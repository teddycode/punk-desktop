...
function extractAndShowNavigation(doc) {
  ...
  try {
    ...
    var items = Array.from(
      doc.querySelectorAll('[class*="menu"] a, [class*="navigation"] a, header li a, [role=tabpanel] a, nav a'),
    )
      .filter((el) => {
        let n = el;
        while (n) {
...
function extractDate(doc) {
  ...
  if (dateItem) {
    try {
      ...
      date = new Intl.DateTimeFormat(navigator.language, { year: 'numeric', month: 'long', day: 'numeric' }).format(
        new Date(d),
      );
    } catch (e) {
      console.warn(e);
...
<style>
  ...
  if (!article) {
    ...
    readerContent +=
      "<div id=\"app\" class='content'><div id='reader-main' class='reader-main' domain='" +
      readerDomain +
      "'>" +
...
function processArticle(data) {
  ...
  data = data
    .replace(/ src="\/\//g, ' src="' + articleLocation.protocol + '//')
    .replace(/"\/\//g, '"' + articleLocation.protocol + '//');
  console.log(data);

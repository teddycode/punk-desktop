...
class GroupModel extends MenuItemModel {
  ...
  _buildCustomIcon() {
    ...
    if (this.group.customIcon) {
      return IconUrlFormat.toDataUrl(this.file.db.meta.customIcons.get(this.group.customIcon.id)?.data);
    }
    return null;
  }
  ...
  matches(filter) {
    return (
      ((filter && filter.includeDisabled) ||
        (this.group.enableSearching !== false && !this.group.uuid.equals(this.file.db.meta.entryTemplatesGroup))) &&

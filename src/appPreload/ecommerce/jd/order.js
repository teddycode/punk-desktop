const { $, ipc, args, clearText, fixProtocol, callback, intervalEvent, closeSelf } = require('../../app.js');
const { getNickname, isLogged, loaded } = require('./util');
  ...
  for (const tip of tips) {
    ...
    window.callbackData.orders.find((order) => {
      if (order.id === orderId) {
        ...
        order.expressNo = order.expressType.substring(
          order.expressType.lastIndexOf('：') + 1,
          order.expressType.length,
        );
      }
    });
    got = true;
...
function getOrderInfo($item) {
  ...
  for (const child of children) {
    ...
    item.cover =
      lazyCover !== 'done' ? fixProtocol(lazyCover) : fixProtocol($(child).find('.goods-item img').attr('src'));

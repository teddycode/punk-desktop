...
const DEFAULT_WEB_PREFERENCE = {
  ...
  enableWebSQL: false,
  autoplayPolicy:
    settings.get('enableAutoplay') || settings.get('enableAutoplay') === undefined
...
class WindowManager {
  ...
  async create(options) {
    ...
    let webContents;
    if (webPreferences) webPreferences = Object.assign(_.cloneDeep(DEFAULT_WEB_PREFERENCE), webPreferences);
    ...
    windowOption = Object.assign(_.cloneDeep(DEFAULT_WINDOW_PREFERENCES), windowOption);
  ...
  createFrameView(options, frameWindow) {
    let { webPreferences, width, height, name, url: loadUrl, onDomReady } = options;
  ...
  createView(options) {
    let { name, viewOption, webPreferences, rememberBounds, defaultBounds, url } = options;
    let webContents;
    if (!webPreferences) {
      webPreferences = {};
    }
    ...
    viewOption.windowOption = Object.assign(_.cloneDeep(DEFAULT_VIEW_PREFERENCES), viewOption);
    if (!webPreferences.additionalArguments) {

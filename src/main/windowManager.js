...
const DEFAULT_WEB_PREFERENCE = {
    ...
    enableWebSQL: false,
    autoplayPolicy:
        settings.get("enableAutoplay") ||
        settings.get("enableAutoplay") === undefined
...
const DEFAULT_VIEW_PREFERENCES = {
    //根据其定义的位置取用这里的width和height，在左右侧的话，只取用width，在上下，则只取height，目前仅支持width
    width: 480,
};
    ...
    static listenIPC() {
        ipc.on("app.ipc", (event, args) => {
            BrowserViewHandler.IPCEvents.forEach((e) => {
                if (e.id === event.sender.id && e.event === args.event) {
                    //找到发送者的id
...
class ViewManager {
    ...
    autoAdjustPosition(pos, width) {
        ...
        Object.keys(viewMap).forEach((key) => {
            ...
            if (
                viewMap[key] === windowManager.attachedView &&
                Object.keys(viewMap).length > 1
            ) {
                //如果是吸附到右侧的tab，且不是唯一的tab，则不需要实际去移动这个view
                return;
            }
            let mainViewPreWidth =
                parentBounds.width - width - viewBounds.x - this.SPLIT_WIDTH; //计算将要调整的宽度。用于限宽
    ...
    resetAttachPosition() {
        ...
        this.autoAdjustPosition(
            "right",
            parentBounds.width + parentBounds.x - cursorPosition.x
        );
    }
    ...
    onSetBounds(bounds) {
        if (windowManager.attachedView) {
            if (
                mainWindow.getBrowserViews().indexOf(windowManager.attachedView) === -1
            ) {
                //如果发现侧边吸附的view被移除了
...
class WindowManager {
    ...
    async create(options) {
        ...
        if (webPreferences)
            webPreferences = Object.assign(
                _.cloneDeep(DEFAULT_WEB_PREFERENCE),
                webPreferences
            );
        ...
        windowOption = Object.assign(
            _.cloneDeep(DEFAULT_WINDOW_PREFERENCES),
            windowOption
        );
    ...
    async createFrameWindow(options) {
        ...
        frameWebPreferences = Object.assign(
            _.cloneDeep(DEFAULT_WEB_PREFERENCE),
            frameWebPreferences
        );
    ...
    createFrameView(options, frameWindow) {
        ...
        appView.webContents.on("did-navigate-in-page", (event, url) => {
            if (frameWindow) {
                if (!frameWindow.webContents.isDestroyed()) updateView(url);
            }
        });
    ...
    createView(options) {
        ...
        viewOption.windowOption = Object.assign(
            _.cloneDeep(DEFAULT_VIEW_PREFERENCES),
            viewOption
        );
        if (!webPreferences.additionalArguments) {
    ...
    syncAttachedBounds() {
        if (this.viewManager) this.viewManager.syncSize();
    }

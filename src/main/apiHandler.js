...
class ApiHandler {
  ...
  static bindIPC() {
    ...
    ipc.on('changeUrl', (e, a) => {
      ApiHandler.onUrlChanged.forEach((instance) => {
        try {
          ...
          //发现已经挂了，就直接delete掉
          ApiHandler.onUrlChanged.splice(
            ApiHandler.onUrlChanged.findIndex((ins) => {
              return ins === instance;
            }),
            1,
          );
    ...
    ApiHandler.on('tabs', 'setOnUrlChanged', (event, args, instance) => {
      //添加一个弹幕观测
      if (instance.type === 'view') ApiHandler.onUrlChanged.push(instance.view);
      else {
      ...
      function bindDownloadListener(webContents) {
        ...
        if (found) {
          ...
          webContents.on('closed', () => {
            ...
            ApiHandler.downloadWebContents.splice(
              ApiHandler.downloadWebContents.findIndex((wc) => {

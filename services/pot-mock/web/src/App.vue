...
import { CONNECTION_CONFIG_EVENT, getConnectionConfig } from "@/services/connection";
...
const resolveWebSocketUrl = () => {
  return import.meta.env.VITE_WS_URL || getConnectionConfig().wsUrl;
};
...
const connectWebSocket = () => {
  ...
  if (getConnectionConfig().useMock) {
    return;
  }
...
const handleConnectionConfigChange = async () => {
  await Promise.all([
...
onMounted(async () => {
  ...
  connectWebSocket();
  window.addEventListener(CONNECTION_CONFIG_EVENT, handleConnectionConfigChange);
});
...
onUnmounted(() => {
  ...
  // 断开 WebSocket
  wsService?.disconnect();
  window.removeEventListener(CONNECTION_CONFIG_EVENT, handleConnectionConfigChange);
});
</script>

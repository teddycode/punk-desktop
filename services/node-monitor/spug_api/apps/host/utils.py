...
def fetch_host_extend(ssh):
    ...
    geoInfo = fetch_location(public_ip_address)
    if 'Error' not in geoInfo:
        response['location'] = geoInfo['location']
...
def fetch_location(ipList):
    if len(ipList) == 0: 
        return  {'Error': 'Empty ip list'}
    for ip in ipList:
        try:
            url = f"https://ipinfo.io/{ip}/json?token=48014d5ea1f7d7"
            response = requests.get(url)
            data = response.json()
            if 'error' not in data:
                location_info = {
                    'location': data.get('region') + '-' + data.get('city') ,

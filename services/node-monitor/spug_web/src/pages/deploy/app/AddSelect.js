...
class AddSelect extends React.Component {
  // 关联脚本功能
associateScript = (app_id, template_id) => {
  axios.post('/api/exec/relatedtemp/', {
    app_id: app_id,
    template_id: template_id,
  })
  .then(response => {
    if (response.outputs === 'success') {
      message.success('脚本关联成功');
      this.closeModal();  // 关闭弹出框
    } else {
      message.error('已添加过脚本');
    }
  ...
  render() {
    ...
    return (
      <Modal
        ...
        {hasPermission('exec.relatedTemp.edit|exec.relatedTemp.add') && (
          <Table.Column title="操作" render={info => (
            <Action>
                <Action.Button auth="exec.relatedTemp.add" onClick={() => this.associateScript(store.currentRecord.id, info.id)}>添加</Action.Button>
            </Action>
          )}/>
        )}
      </TableCard>

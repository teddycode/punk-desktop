...
export class CookiesAPI {
  ...
  private async get(event: ExtensionEvent, details: chrome.cookies.Details): Promise<chrome.cookies.Cookie | null> {
    // TODO: storeId
  ...
  private async getAll(event: ExtensionEvent, details: chrome.cookies.GetAllDetails): Promise<chrome.cookies.Cookie[]> {
    // TODO: storeId
    const cookies = await this.cookies.get({
      url: details.url,
      name: details.name,
  ...
  private async set(event: ExtensionEvent, details: chrome.cookies.SetDetails): Promise<chrome.cookies.Cookie | null> {
    await this.cookies.set(details);
    const cookies = await this.cookies.get(details);
    return cookies.length > 0 ? createCookieDetails(cookies[0]) : null;
  }

  private async remove(event: ExtensionEvent, details: chrome.cookies.Details): Promise<chrome.cookies.Details | null> {
    try {

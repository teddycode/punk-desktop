...
const getFrame = (frameProcessId: number, frameRoutingId: number) => {
  return ('webFrameMain' in electron && (electron as any).webFrameMain.fromId(frameProcessId, frameRoutingId)) || null;
};
...
export class WebNavigationAPI {
  ...
  private onCreatedNavigationTarget = (
    ...
    const { url = args[0] as string, frame = getFrame(args[3], args[4]) as Electron.WebFrameMain } = event;

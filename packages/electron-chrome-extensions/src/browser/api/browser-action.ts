...
export class BrowserActionAPI {
  ...
  constructor(private ctx: ExtensionContext) {
    ...
    const setDetails = ({ extension }: ExtensionEvent, details: any, propName: ExtensionActionKey) => {
      const { tabId } = details;
    ...
    const setter =
      (propName: ExtensionActionKey) => (event: ExtensionEvent, details: chrome.browserAction.TabDetails) =>
    ...
    // to use 'icon'.
    handle('browserAction.setIcon', (event, { tabId, ...details }: chrome.browserAction.TabIconDetails) => {
  ...
  private activateContextMenu(details: ActivateDetails) {
    ...
    const contextMenuItems: MenuItem[] = this.ctx.store.buildMenuItems(extensionId, 'browser_action');

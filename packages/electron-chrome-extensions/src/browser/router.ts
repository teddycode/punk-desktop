...
class RoutingDelegate {
  ...
  private onRemoteMessage = async (
    ...
    const ses = sessionPartition === DEFAULT_SESSION ? event.sender.session : session.fromPartition(sessionPartition);
  ...
  private onAddListener = (event: Electron.IpcMainInvokeEvent, extensionId: string, eventName: string) => {
    const observer = this.sessionMap.get(event.sender.session);
    const listener: EventListener = { host: event.sender, extensionId };
    return observer?.addListener(listener, extensionId, eventName);
  };

  private onRemoveListener = (event: Electron.IpcMainInvokeEvent, extensionId: string, eventName: string) => {
    const observer = this.sessionMap.get(event.sender.session);
    const listener: EventListener = { host: event.sender, extensionId };
...
export class ExtensionRouter {
  ...
  constructor(public session: Electron.Session, private delegate: RoutingDelegate = RoutingDelegate.get()) {

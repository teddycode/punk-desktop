...
const LABELS = {
  openInNewTab: (type: 'link' | Electron.ContextMenuParams['mediaType']) => `Open ${type} in new tab`,
  openInNewWindow: (type: 'link' | Electron.ContextMenuParams['mediaType']) => `Open ${type} in new window`,
...
export const buildChromeContextMenu = (opts: ChromeContextMenuOptions): Menu => {
  ...
  if (params.isEditable) {
    if (params.misspelledWord) {
      ...
      if (app.isEmojiPanelSupported() && !['number', 'tel', 'other'].includes(params.inputFieldType)) {
        append({
          label: labels.emoji,

$(function () {
  ...
  function formType(i) {
    if (i == 0) {
      ...
      $('form button.voice-btn').attr('data-btn', 'sms-voice-btn').text('忘记密码');
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data.CountryNum, function (i, item) {
        ...
        let btns =
          " <button class='phone-btn selectBtn select-option' type='button' data-type='option'" +
          'data-id=' +
          i +
          '>' +
          item.countryName +
  ...
  //失去焦点;获得焦点
  Focuss($('.msgInput'), '输入邮箱验证码');
  Focuss($('.phoneInput'), '邮箱号');
  Focuss($('.accountUsername'), '用户名或手机号');
  Focuss($('.accountPwd'), '密码');

  Blurr($('.phoneInput'), '请输入邮箱号');
  Blurr($('.msgInput'), '请输入邮箱验证码');
  Blurr($('.accountUsername'), '请输入用户名或手机号');

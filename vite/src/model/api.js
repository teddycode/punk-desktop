...
const api = {
  async getCosUpload(data, callback) {
    // 非工作台图片上传请求方法
    let res = await axios.post(url.cosUpload, data, await getConfig());
    result(res, callback);
  },

  async postCosUpload(data, callback) {
    // 工作台图片上传请求方法
    let res = await axios.post(sUrl(url.cosUpload), data, await getConfig());
    result(res, callback);
  },
};
export default api;

<template>
  <div
    class="TUI-search"
    style="padding: 0 16px !important; margin-top: 13px"
    :class="[env.isH5 ? 'TUI-search-H5' : '']"
    ref="dialog"
  >
    <a-dropdown @click="toggleOptionalShow" placement="topLeft" class="ml-4" :trigger="['click']">
      <div
        class="w-11 active-button h-11 rounded-lg pointer flex items-center justify-center"
      ...
      <template #overlay>
        <div class="flex items-center rounded-lg" style="background: var(--modal-bg); padding: 16px !important">
          <div
            ...
            class="flex add-item flex-col items-center justify-center pointer"
            @click="showOpen(item.type, index)"
          >
            <div
              class="flex items-center add-hover rounded-lg justify-center h-11 w-11"

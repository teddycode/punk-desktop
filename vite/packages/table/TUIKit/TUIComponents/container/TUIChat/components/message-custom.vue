  ...
  <div class="custom">
    ...
    <template v-else-if="isCustom.businessID === constant.typeEvaluate">
      <div class="evaluate">
        ...
        <ul>
          <li v-for="(item, index) in ~~isCustom.score" :key="index" class="evaluate-list-item">
    ...
    <template v-else-if="isCustom.businessID === constant.TYPE_CALL_MESSAGE">
      <div :class="`call-${data?.message?.conversationType}`" class="call" @click="handleCallAgain">
        <i :class="handleCallMessageIcon()" class="icon"></i>
    ...
    <template v-else>
      <div v-if="isCustom.type==='game'">
        <!--      {{steam游戏格式}}-->
        <img :src="isCustom.game?.header"
             alt=""
             style="border-radius: 5px;width: 250px;cursor:pointer!important;position: relative;pointer-events: auto;" @click.stop="goGameDetail(isCustom.game?.appid)">
        <div class="mt-2 " style="margin-top: 10px;">
          <div class="truncate" style="max-width: 160px;float: left;margin-right: 10px">
            <a-avatar :size="30" :src="isCustom.game?.icon || '/emoji/game.png'" shape="square"></a-avatar>
            <strong>{{ isCustom.game?.chineseName }}</strong>
          </div>

          <div style="float: right">
            <xt-button :h="35" :w="70" size="mini" @click="goGameDetail(isCustom.game?.appid)">

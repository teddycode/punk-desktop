<template>
  <div class="flex flex-col memberinfo-main">
    ...
    <div class="flex rounded-lg px-4  mb-4" style="height:64px;background: var(--secondary-bg);">
      ...
      <Medal v-for="medal in info.medal" :medal="medal"></Medal>
    </div>

    <div v-if="info.add?.flow === 'in'" class="flex" style="color: var(--secondary-text);">
      <div class="font-16 active-button mr-3 rounded-lg pointer"
           style="background: var(--secondary-bg); padding:11px 55px; color: var(--primary-text);"
           @click="initiateChat">发起聊天
      </div>
      <div class="font-16 active-button  rounded-lg pointer"
           style="background: var(--active-bg); padding:11px 55px; color: var(--active-text);">添加好友
      </div>
    </div>

<template>
  <div class="flex flex-col memberinfo-main">
    <div class="flex" style="margin: 25px !important">
      <FrameAvatar
        :frame="info.eq?.frameDetail"
        class="frame"
        :avatarUrl="info.userinfo?.avatar"
        :avatarSize="50"
      ...
      <div class="flex flex-col" style="margin-left: 30px">
        ...
        <span
          class="font-12"
          style="border-radius: 4px; background: var(--secondary-bg); padding: 0 6px; color: var(--secondary-text)"
          >{{ info.userinfo?.uid }}</span
        >
      </div>
    </div>

    <div class="rounded-lg flex flex-col px-4 py-3 mb-4" style="background: var(--secondary-bg); height: 85px">
      <div class="font-14 mb-4" style="color: var(--primary-text)">全球排名: {{ info.grade?.rank }}</div>
    ...
    <div class="flex" style="color: var(--secondary-text)" v-if="info.add?.flow === 'in'">
      <div
        class="font-16 active-button mr-3 rounded-lg pointer"
        style="background: var(--secondary-bg); padding: 11px 55px; color: var(--primary-text)"
        @click="initiateChat"
      >
        发起聊天
      </div>
      <div
        class="font-16 active-button rounded-lg pointer"
        style="background: var(--active-bg); padding: 11px 55px; color: var(--active-text)"
      >
        添加好友
      </div>
    </div>

...
function handleTipGrpUpdated(message: any) {
  ...
  if ('muteAllMembers' in newGroupProfile) {
    if (newGroupProfile['muteAllMembers']) {
      text = `${t('message.tip.管理员')} ${message.nick} ${t('message.tip.开启全员禁言')}`;
    } else {
      text = `${t('message.tip.管理员')} ${message.nick} ${t('message.tip.取消全员禁言')}`;
    }
  } else if ('ownerID' in newGroupProfile) {
    text = `${message.nick} ${t('message.tip.成为新的群主')}`;
  } else if ('groupName' in newGroupProfile) {
    text = `${message.nick} ${t('message.tip.修改群名为')} ${newGroupProfile['groupName']}`;
...
export function translateGroupSystemNotice(message: any) {
  ...
  switch (message.payload.operationType) {
    ...
    case 5:
      return `${t('message.tip.群')}：${groupName} ${t('message.tip.被')} ${message.nick} ${t('message.tip.解散')}`;
    case 6:

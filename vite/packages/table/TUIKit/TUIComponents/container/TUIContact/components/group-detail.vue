<template>
  <div class="flex  rounded-lg items-center"
       style="height: 54px;background: var(--secondary-bg);padding: 11px 16px;margin-bottom: 16px;">
    <a-avatar :size="32" :src="group.avatar" shape="square"></a-avatar>
  ...
  <div class="rounded-lg flex flex-col" style="background: var(--secondary-bg);margin-bottom: 16px;">
    <div class="flex items-center justify-between py-3 px-4">
      ...
      <div class="flex items-center">
        ...
        <Icon icon="icon_copy" style="color: var(--secondary-text);font-size: 1.5em;cursor: pointer;"
              @click="handleGroupIDCopy"></Icon>
      </div>
    </div>
  ...
  <div v-if="group?.selfInfo?.role !== 'Member'" class="flex">
  ...
  <Modal v-if="groupDetailShow" v-model:visible="groupDetailShow" :blurFlag="true">
    <UserSelect :groupID="group.groupID" :list="memeber" :type="type" @close="close" @closeUser="close"></UserSelect>
  </Modal>
</template>

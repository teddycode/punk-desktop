<template>
  ...
  >
    ...
    >
      ...
      <div class="content">
        <div class="content-header">
          <label>
            <p class="name">
              {{ handleConversation?.name(conversation) }}
              <a-badge
                style="margin-top: -1px; zoom: 0.9"
                :number-style="{ border: 'none !important' }"
                :count="conversation.conversationID === 'GROUP' + liveRoom ? '全员' : ''"
              ></a-badge>
            </p>
          </label>
    ...
    <div class="dialog dialog-item flex flex-col" v-if="toggle" ref="dialog">
      ...
      <div
        class="flex items-center justify-center rounded-lg conversation-options"

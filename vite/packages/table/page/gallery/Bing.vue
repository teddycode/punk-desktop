<template>
  ...
  <vue-custom-scrollbar id="containerWrapper" :settings="settingsScroller" style="margin-top: 1.5em">
    ...
    <div
      ...
      data-control="autofit,page,fullscreen,close,zoom"
      data-play="true"
      data-autoslide="true"
      data-infinite="true"
      id="container"
    ...
    <template v-for="dayImages in bingImages">
      <div
        style="color: var(--primary-text); text-align: left; font-size: 20px; font-weight: bold"
        class="mt-3 mb-3 text-white s-text"
      >
        - 第 {{ dayImages.day }} 天 -
      </div>
...
export default {
  ...
  mounted() {
    ...
    $('#containerWrapper').scroll(() => {
      if (
        $('#containerWrapper').scrollTop() + $('#containerWrapper').height() + 20 >=
          $('#bingImages').prop('scrollHeight') &&

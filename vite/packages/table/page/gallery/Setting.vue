<template>
  <div class="flex items-center justify-between" style="">
    ...
    <div class="pointer s-icon mr-6 float-right" @click="resetPapersSettings">
      <div style="display: inline-block; margin-right: 1em">
        <span> <Icon style="font-size: 2em; vertical-align: top" icon="shuaxin"></Icon> </span
        ><span style="font-size: 1.2em" class="s-text"> 重置全部设置</span>
      </div>
    </div>
  </div>
  <vue-custom-scrollbar
    id="containerWrapper "
    class="gap-4 item-content"
  ...
  >
    <div class="w-full">
      <div class="card s-bg p-4" style="width: 200px; height: 180px">
        ...
        <div class="flex gap-2 item-content">
          ...
          <div
            @click="selectSource('picking')"
            class="select-item"
            :class="{ active: settings.wallSource === 'picking' }"
          >
    ...
    >
      ...
      <div class="line">使用锁屏壁纸：<a-switch v-model:checked="settings.enable"></a-switch></div>
      <div v-if="settings.enable">
        ...
        <div class="line">
          ...
          <a-input-number
            default-value="300"
            v-model:value="settings.lockTimeout"
            style="color: var(--primary-text)"
          ></a-input-number>
          秒
        </div>
        <div class="line">
          在状态栏提示还有多久自动锁屏：<a-switch
            v-model:checked="settings.tipLock"
            style="color: var(--primary-text)"
          ></a-switch>
        </div>
        <div class="line">
          壁纸保存位置：<a-input
            placeholder="请选择壁纸保存地址"
            @click="showSaveDialog"
            style="width: 15em; color: var(--primary-text)"
            v-model:value="settings.savePath"
          ></a-input>
        </div>
    ...
    <div
      ...
      "
    >
      <div class="line-title">锁屏显示设置</div>
      <div class="line">显示时间：<a-switch v-model:checked="settings.showTime"></a-switch></div>
      <div class="line">显示未读消息：<a-switch v-model:checked="settings.showUnreadMessage"></a-switch></div>
      <div class="line">显示天气：<a-switch v-model:checked="settings.showWeather"></a-switch></div>
      <div class="line">显示播放进度：<a-switch v-model:checked="settings.showProgress"></a-switch></div>
    </div>
    <div
      class="card auto-height s-bg"
      v-if="settings.enable"
      ...
      "
    >
      <div class="line-title">轮播设置</div>
      <div class="line">
        默认轮播模式：<a-radio-group v-model:value="settings.playType"
          ><a-radio value="my" style="color: var(--primary-text)">轮播我的收藏</a-radio>
          <a-radio value="active" style="color: var(--primary-text)">轮播激活壁纸</a-radio></a-radio-group
        >
      </div>
      <div class="line">
        切换图片时间间隔：
        <a-input-number default-value="3" v-model:value="settings.interval" style="color: var(--primary-text)">
        </a-input-number>
        秒
      </div>
    </div>

<template>
  <div class="rotate-center" style="margin-bottom: 1em; margin-top: 2em; color: var(--primary-text)">
    ...
    <div class="pointer" style="float: right; font-size: 1em">
      <div
        @click="playAll"
        class="rounded-lg list-hover s-icon"
...
export default defineComponent({
  ...
  methods: {
    ...
    loadStaticPaper() {
      ...
      fs.pathExists(staticDir).then((exists) => {
        ...
        if (exists) {
          ...
          promptPaper.map((el) => {
            this.getFileCreatedTime(path.join(`${staticDir}/${el}`))
              .then((createTime) => {
    ...
    isInActive(image) {
      return (
        this.activePapers.findIndex((img) => {
    ...
    del() {
      ...
      if (this.fileImageExtension(this.currentPaper)) {
        ...
        const findIndex = this.activePapers.findIndex((img) => {
          if (img.src === this.currentPaper.src) return true;
        });
        this.activePapers.splice(findIndex, 1);
        if (
          this.currentPaper.src.split('//')[0] === 'file:' &&
          imageExtensions.indexOf(this.currentPaper.src.split('.')[1]) !== -1
        ) {

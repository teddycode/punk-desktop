<template>
  <div class="flex w-full justify-between items-center" style="margin-bottom: 16px; margin-top: 2em">
    <div class="w-96 h-12 flex items-center">
      <div class="w-80 h-12 bg-opacity-10 rounded-lg flex items-center xt-bg-2">
        <div
          class="w-20 h-12 flex items-center justify-center"
    ...
    <div class="w-48 h-12 flex items-center justify-end" style="margin-right: 1em; color: var(--primary-text)">
      ...
      <div
        class="w-12 h-12 flex items-center rounded-lg cursor-pointer justify-center s-icon"
        @click="openInfo"
  ...
  >
    <div
      class="w-full h-12 flex rounded-lg"
    ...
    <div class="flex w-full justify-center items-center flex-col">
      <div class="flex-col flex justify-start mx-2 my-40">
        <div class="flex" style="margin-bottom: 12px">
          <span style="margin-right: 16px; color: rgba(255, 255, 255, 0.6); font-size: 16px; font-weight: 400"
            >壁纸源</span
          >
        ...
        <div class="flex items-center">
          <span style="margin-right: 30px; color: rgba(255, 255, 255, 0.6); font-size: 16px; font-weight: 400"
            >官网</span
          >
          <span
            class="w-40 h-12 flex items-center rounded-lg cursor-pointer justify-center bg-white bg-opacity-10"
...
export default defineComponent({
  ...
  mounted() {
    $('#pick-wrapper').scroll(() => {
      if (
        $('#pick-wrapper').scrollTop() + $('#pick-wrapper').height() + 20 >= $('#pick-images').prop('scrollHeight') &&
  ...
  methods: {
    ...
    getPickingData(e) {
      ...
      const cate =
        this.pickFilterValue === '/wallhaven/v2'
          ? this.wallValue
          : this.pickFilterValue === '/timeline/v2'
          ? this.classValue
          : '';
      ...
      const url = `${api}${e}?${
        cate === '' ? '' : `cate=${cate}`
      }&order=${order}&no=${no}&date=${dateTime}&score=${score}&client=thisky`;
      if (!this.isLoading) {
        this.isLoading = true;
        axios
          .get(url)

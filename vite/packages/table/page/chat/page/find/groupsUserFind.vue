<template>
      ...
      <template v-else>
        <div class="flex flex-col">
          ...
          <div class="content-list">
            <div v-for="item in recommendData.users"
                 class="flex items-center content-item px-4 justify-between py-5 rounded-xl mb-3"
              ...
              <div class="flex items-center pointer w-full" @click="openUserCard(item.uid)">
                <a-row :gutter="10" class="flex w-full">
                  <a-col flex="50px">
                    <a-avatar :size="48" :src="item.avatar"></a-avatar>
                  </a-col>
                  <a-col flex="auto">
                    <div class=" font-16" style="color:var(--primary-text)">{{ item.nickname }}</div>
                    <div>{{ item.reason }}</div>
                  </a-col>
                  <a-col v-if="item.uid!=userInfo.uid && item.relationship!==undefined" flex="100px" @click.stop>
                    <AddFindButton v-show="item.relationship==='not'"
                                   :key="item.uid" :uid="item.uid"
                                   @relationshipChanged="updateRelationship($event,item)"></AddFindButton>
                    <SendMessageButton v-show="item.relationship==='yes'" :enable="false" :uid="item.uid"
                                       @send="enterGroup(item)"></SendMessageButton>
                  </a-col>
                </a-row>
          ...
          <div class="content-list">
            <div v-for="item in recommendData.groups "
                 class="flex items-center content-item justify-between px-4 py-5 rounded-xl mb-3"
              ...
              <XtButton v-show="item.relationShip ==='not'" style="width: 96px;background: var(--active-bg);"
                        @click="addGroup(item.groupID)">
                加入
              </XtButton>
            </div>
          </div>
  ...
  <teleport to='body'>
    <Modal v-if="cardVisible" v-model:visible="cardVisible" :blurFlag="true">

    ...
    <div class="w-full card-content">
      <div class="flex justify-between mb-2 -mt-3">
        ...
        <div class="flex items-center">
          <a-tooltip placement="bottom" title="更多操作">
            <Icon class="text-xl xt-text pointer active-icon" icon="fluent:more-horizontal-16-filled"/>
          </a-tooltip>
          <a-tooltip placement="bottom" title="刷新">
            <Icon class="ml-3 text-xl xt-text pointer active-icon" icon="akar-icons:arrow-clockwise"
                  @click="refreshDetail"/>
          </a-tooltip>
          ...
          <a-tooltip placement="bottom" title="关闭">
            <Icon class="ml-3 text-xl xt-text pointer active-icon" icon="akar-icons:cross" @click="closeDetail"/>
          </a-tooltip>
        </div>
      </div>
      <div v-if="refreshDetailFlag">
        ...
        <div>
          <div>
            ...
            <div class="mb-2">
              <div>
                <div v-if="cardData.title" id="title" :innerHTML="title"
                     style="color: var(--primary-text); "></div>
                <div id="context" :innerHTML="content"
                     style="color:  var(--secondary-text); word-break: pre-wrap;"></div>
              </div>
        ...
        <div class="flex mt-4 mb-4 ">
          ...
          <div class="flex items-center " style="cursor: pointer;" @click="clickLike">
            <button :class="{ 'xt-bg': !isLike, 'xt-active-bg': isLike }"
                    class="mr-3 reply w-[57px] h-[32px]  pl-5 "
                    style="position: relative;border: none;cursor: pointer;">{{ cardData.support_count }}
            </button>
            <img alt="" class="w-[20px] h-[20px] " src="../../../../../public/icons/like.png"
                 style="position: absolute;left:34px;">
          </div>
          <div class="flex items-center" style="cursor: pointer;" @click="clickCollect">
            <button :class="{ 'xt-bg': !isCollect, 'xt-active-bg': isCollect }"
                    class="reply w-[57px] h-[32px]  pl-5"
                    style="position: relative;border: none;cursor: pointer;">{{ cardData.collect_count }}
            </button>
            <img alt="" class="w-[20px] h-[20px]" src="../../../../../public/icons/collect.png"

<template>
  <div class="w-full ">
    ...
    <MainReplyComment v-for="(item, index) in commentList.list" :key="index" :commentList="item"
                      :uid="props.uid"></MainReplyComment>
    <a-pagination v-if="paginationVisible" v-model:current="current" :total="totalReply" class="xt-text-2" simple
                  @change="changePage"/>
  </div>
</template>

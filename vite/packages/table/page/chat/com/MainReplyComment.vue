<template>
  ...
  <div class="w-full mb-3 box">
    <div class="mb-3">
      <div class="flex">
        <a-avatar
          :src="props.commentList.user.avatar"
          :size="24"
          class="mr-2 pointer"
      ...
      <div
        class="mt-2 ml-8 font-16 xt-text content-image"
        style="user-select: text; text-align: left"
        :innerHTML="content"
      ></div>
        ...
        <viewer :images="commentList.image_pc" :options="options" class="items-center p-0 mb-0">
          <a-row :gutter="[20, 20]" style="margin-right: 1em" wrap="'true">
            <a-col
              ...
              v-for="(img, index) in commentList.image_pc"
              :span="7"
              style=""
            >
              <!-- {{ commentList.image }} -->
      ...
      <replyComments
        ...
        :replyCom="props.commentList"
      />
    </div>
    <div class="ml-8">
      <ReplyComment
        :replyVisible="replyVisible"
        v-for="(item, index) in replyCmmentList"
        :key="index"
        :uid="props.uid"
        :replyCom="item"
      />

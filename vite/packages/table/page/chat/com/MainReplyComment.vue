<template>
  ...
  <div class="w-full mb-3 box">
    <div class="mb-3">
      <div class="flex ">
        <a-avatar :size="24" :src="props.commentList.user.avatar" class="mr-2 pointer"
        ...
        <div class="flex items-center ml-2 text-center">
          ...
          <div v-if="props.uid === commentList.user.uid"
               class="font-12 w-[32px] h-[20px] rounded-lg xt-theme-b xt-theme-text ml-2 mt-1">作者
          </div>
        </div>
      </div>
      <div :innerHTML="content" class="mt-2 ml-8 font-16 xt-text content-image"
           style="user-select: text;text-align: left;"></div>
      <div v-if="commentList.image" class="flex w-full p-0 mt-3 ml-8 -mb-1 whitespace-pre-wrap cover-wrapper">
        ...
        <viewer :images="commentList.image_pc" :options="options" class="items-center p-0 mb-0 ">
          <a-row :gutter="[20, 20]" style="margin-right: 1em" wrap="'true">
            <a-col v-for="(img, index) in commentList.image_pc" :span="7"
                   class="flex flex-wrap mr-2 image-wrapper" style="">
              <!-- {{ commentList.image }} -->
              <img :data-source="commentList.image_artwork_master[index]" :src="img"
      ...
      <div class="flex justify-between  mt-3  h-[20px] xt-text-2 font-14 ml-8">
        <div class="flex items-center justify-center ">
          <div :class="{ 'xt-theme-text': isLike }" class="flex items-center" @click="clickLike">
            ...
            <replyIcon class="mr-1 " icon="akar-icons:thumbs-up" style="font-size: 16px; "></replyIcon>
          ...
          <div class="flex items-center" @click="replyStatus">
            ...
            <replyIcon class="mr-1 " icon="fluent:chat-16-regular" style="font-size: 18px;"></replyIcon>
            <div class="font-14 xt-text-2">回复</div>
          </div>
          <!-- v-if="props.uid === commentList.user.uid" -->
          <div v-if="useUserStore.userInfo.uid === commentList.user.uid" class="flex justify-center ml-1">
      ...
      <replyComments v-if="replyVisible" :replyCom="props.commentList" :userName="props.commentList.user.nickname"
                     @addComment="getReplyText" @changeStatus="getReplyFlag"/>
    </div>
    <div class="ml-8 ">
      <ReplyComment v-for="(item, index) in replyCmmentList" :key="index" :replyCom="item"
                    :replyVisible="replyVisible" :uid="props.uid"/>
      <!-- <replyEmoji/> -->
    </div>

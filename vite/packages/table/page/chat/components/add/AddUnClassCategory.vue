  ...
  <teleport to="body">
    <Modal v-if="unClassVisible" v-model:visible="unClassVisible" :blurFlag="true">
        ...
        <div class="flex flex-col px-6">
          ...
          <div class="flex flex-col mt-4">
            <div
              ...
              class="flex items-center xt-bg-2 pointer rounded-lg px-6 py-5 mb-4"
      ...
      <ChannelLink
        :id="item.id"
        :no="no"
        v-else-if="selectIndex === 'link' && detailShow === true"
        @back="detailShow = false"
        @close="unClassVisible = false"
      />
      <ForumChannel
        :id="item.id"
        :no="no"
        v-else-if="selectIndex === 'community' && detailShow === true"
        @back="detailShow = false"
        @close="unClassVisible = false"
      />
      <ChannelGroup
        :id="item.id"
        :no="no"
        v-else-if="selectIndex === 'group' && detailShow === true"
        @back="detailShow = false"
        @close="unClassVisible = false"
      />
    </Modal>

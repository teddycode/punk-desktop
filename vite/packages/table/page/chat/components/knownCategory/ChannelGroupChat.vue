  ...
  <div class="flex flex-col my-3" style="width:667px;">
    <div class="flex w-full mb-5 h-10 items-center justify-center" style="position: relative;">
      ...
      <div class="close-channel w-10 h-10 flex items-center rounded-lg pointer active-button justify-center"
           style="background: var(--secondary-bg);" @click="closeGroup">
    ...
    <div class="flex px-6">
      <div class="flex flex-col" style="width: 293px;">
        <a-input ref="searchRef" v-model:value="searchKeyWord" class="h-10 search" placeholder="搜索群名称、ID"
                 spellcheck="false" style="border-radius: 12px;" @input="searchGroup"
                 @pressEnter="searchGroup">
          <template #suffix>
            <ChatIcon class="pointer" icon="fluent:search-20-filled"
                      style="font-size: 1.25rem;color:var(--secondary-text);" @click="searchGroup"/>
          </template>
        </a-input>
        ...
        <vue-custom-scrollbar :settings="settingsScroller" style="height:335px;">
          <div class="flex flex-col">
            <div v-for="(item,index) in filterList" :class="{'select-bg':isSelected(index)}"
                 class="flex rounded-lg items-center px-4 py-3 mb-3" @click="leftListClick(item)">

<template>
  <div v-if="isNextShow === false" class="flex flex-col" style="padding: 12px; width: 650px; height: 530px">
    <!-- 顶部关闭 -->
    ...
    <div class="flex justify-between">
      <div class="left-group">
        <div class="font-16" style="color: var(--secondary-text); padding-left: 16px; margin-bottom: 16px">
          我的好友
        </div>
      ...
      <a-divider style="height: 430px; border-color: var(--divider)" type="vertical" />
      ...
      <div class="right-group">
        ...
        <div class="flex justify-end" style="height: 48px">
          <div
            class="flex items-center rounded-md pointer font-16 active-button justify-center"
  ...
  <div v-else-if="classShow === false" class="flex flex-col" style="padding: 12px; width: 650px; height: 530px">
    <div class="top-close">
      <div
        class="flex rounded-lg active-button pointer items-center justify-center"
        style="background: var(--secondary-bg); width: 48px; height: 48px"
        @click="goBack"
      >
        <NewGroupIcon icon="fluent:chevron-left-16-filled" style="font-size: 2em" />
      </div>
      <div class="font-16 flex items-center justify-center" style="color: var(--primary-text); width: 84%">
        创建群聊
      </div>
      <div
        class="flex rounded-lg active-button pointer items-center justify-center"
    ...
    <div
      ...
      style="height: 415px; color: var(--primary-text); position: relative"
    >
      <div class="flex items-center justify-center pointer" style="position: relative" @click="updateGroupAvatar">
        <a-avatar :size="64" :src="groupTypeData.icon" shape="square"></a-avatar>
        <div
          class="flex items-center rounded-full p-3 justify-center"
          style="
            width: 24px;
            height: 24px;
            position: absolute;
      ...
      <div
        v-if="validateChinese"
        :style="validateChinese === true ? { marginBottom: '16px' } : { marginBottom: '0' }"
        class="font-12"
      ...
      <a-select
        v-model:value="public.type"
        :bordered="false"
        :dropdownStyle="{ boxShadow: 'none !important', borderRadius: '12px', color: 'var(--secondary-text)' }"
        :showArrow="true"
        style="width: 320px; border-radius: 12px; color: var(--secondary-text)"
        @change="getGroupType($event)"
      >
        <a-select-option v-for="(item, index) in groupType" :key="index" :value="item.type"
          >{{ item.text }}
        </a-select-option>
      </a-select>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const selectUser = (item) => {
      // 选中用户
      const index = _.findIndex(data.selectList, function (o) {
    ...
    const getGroupType = (evt) => {
      // 下拉获取选择后的群组类型
      const index = _.find(data.groupType, function (o) {
        return o.type === evt;
      });
    ...
    const groupTypeData = computed(() => {
      // 根据计算属性获取群组类型的描述
      const index = _.find(data.groupType, function (o) {
        return o.type === data.public.type;
      });
      return index;
    });

<template>
    ...
    <div class="flex flex-col px-6">
      ...
      <div class="flex flex-col mt-4">
        <div
          ...
          :class="{ 'select-bg': selectIndex === item.type }"
          class="flex items-center pointer rounded-lg px-6 py-5 mb-4"
          style="background: var(--secondary-bg)"
  ...
  <ChannelLink
    v-if="selectIndex === 'link' && nextShow === true"
    :id="id"
    :no="no"
    @back="nextShow = false"
    @close="closeChannel"
  />
  <ForumChannel
    v-if="selectIndex === 'community' && nextShow === true"
    :id="id"
    :no="no"
    @back="nextShow = false"
    @close="closeChannel"
  />
  <ChannelGroup
    v-if="selectIndex === 'group' && nextShow === true"
    :id="id"
    :no="no"
    @back="nextShow = false"
    @close="closeChannel"

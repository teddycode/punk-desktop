<template>
  <div v-if="classShow === false" class="flex flex-col my-3" style="width:500px;">
    ...
    <div class="flex flex-col px-6">
     ...
     输入带有“https://”或“http://”前缀的，完整的网页地址。支持选择点击链接后的打开方式，默认使用工作台设置。
      ...
      <a-input ref="linkNameRef" v-model:value="linkName" class="h-10" placeholder="链接名称"
               spellcheck="false" style="border-radius: 8px; margin-bottom: 12px;"/>
      ...
      <a-input-group compact>
        ...
        <a-input ref="linkRef" v-model:value="link" placeholder="请输入" spellcheck="false"
                 style="width: 80%;height:40px !important;border-top-right-radius: 8px !important;border-bottom-right-radius:8px;"/>
      </a-input-group>

      <span class="font-16-400 my-4" style="color:var(--primary-text);">链接打开方式</span>
      <RadioTab v-model:selectType="defaultOpen" :navList="openType" class="my-3"></RadioTab>
      <template v-if="defaultOpen.openMethod === 'outerOpen'">
        <RadioTab v-model:selectType="defaultType" :navList="linkType"></RadioTab>
      </template>
      ...
      <div class="flex items-center justify-end my-3">
        ...
        <XtButton style="width: 64px;height:40px; background: var(--active-bg);color:var(--active-text);"
                  @click="submitSelect">选择
        </XtButton>
      </div>
    </div>
  </div>

  <SelectClassification v-else :data="{name:linkName,props:webLink}" :no="no" :type="type"
                        @classBack="classShow = false"></SelectClassification>
</template>

<template>
  <div v-if="classShow === false" class="flex flex-col my-3" style="width:500px;">
    ...
    <div class="flex flex-col px-6">
      ...
      <vue-custom-scrollbar :settings="settingsScroller" style="height: 300px;">
        <div v-for="(item,index) in forumList" :class="{'select-bg':selectIndex === index}"
             class="flex items-center mb-3 pointer p-5 rounded-lg"
             style="background: var(--secondary-bg);" @click="selectForum(index,item)"
        >
          <a-avatar :size="40" :src="item.logo" shape="square"></a-avatar>
  ...
  <ChannelClassification v-else :data="selectForumItem" :no="no" type="forum" @classBack="classShow = false"/>
</template>

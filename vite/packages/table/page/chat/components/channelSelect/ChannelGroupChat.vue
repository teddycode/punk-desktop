<template>
  <div class="flex flex-col my-3" style="width: 500px" v-if="chatShow === false">
    ...
    <div class="px-6 flex-col mt-4">
      <div
        v-for="item in list"
        class="flex flex-col xt-bg-2 p-4 mb-4 rounded-lg pointer"
        :class="{ 'select-bg': listIndex === item.type }"
  ...
  <CreateNewGroup
    v-else-if="listIndex === 'create' && chatShow === true"
    :no="no"
    @close="closeChannel"
    @backButton="chatShow = false"
  />
  <ChannelKnownGroup
    :no="no"
    v-else-if="listIndex === 'related' && chatShow === true"
    @backButton="chatShow = false"
    @close="closeChannel"
  />
</template>

  ...
  <div class="flex flex-col my-3" style="width: 500px">
    ...
    <div class="px-6 flex flex-col">
      <vue-custom-scrollbar :settings="settingsScroller" style="height: 350px">
        ...
        <xt-button
          class="my-4"
          style="width: 100%; border: 1px dashed var(--disable-text); background: transparent !important"
          @click="addClassItem"
        >
        ...
        <div class="flex flex-col" ref="classSortTab">
          <div
            v-for="(item, index) in categoryClass"
            class="flex pointer xt-bg-2 rounded-lg items-center px-5 mb-4"
            :class="{ 'select-bg': statusIndex === index }"
            @click.stop="listClick(item, index)"
            :key="index"
            style="height: 60px"
          >
            <template v-if="isEditing && editIndex === index">
              <div class="flex w-full justify-between items-center">
                <a-input
                  ...
                  :bordered="false"
                  @pressEnter.stop="createClass(item)"
                >
                  <template #suffix>
                    <div class="flex">
                      <ClassIcon
...
export default {
  ...
  methods: {
    ...mapActions(communityStore, ['removeCategory', 'createChannel', 'getChannelList', 'updateCategoryClass']),
    // 拖拽排序
    onSortEnd(evt) {

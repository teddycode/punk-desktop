  ...
  <div class="flex flex-col my-3" style="width:500px;">
    <div class="flex w-full mb-5 h-10 items-center justify-center" style="position: relative;">
      <div class="back-button w-10 h-10 flex items-center rounded-lg pointer active-button justify-center"
           style="background: var(--secondary-bg);" @click="backButton">
    ...
    <div class="px-6 flex flex-col">
      <vue-custom-scrollbar :settings="settingsScroller" style="height:350px;">
        <div class="px-4 py-3 flex items-center font-14-400 mb-4 justify-center rounded-lg"
             style="background: var(--secondary-bg);color:var(--secondary-text);">
          不选择分类默认会将频道添加在最外层第一个位置；支持拖拽排序。
        </div>
        <div class="flex items-center rounded-lg py-3 pointer mb-4 active-button justify-center"
             style="border: 1px dashed var(--divider);" @click="addClassItem">
        ...
        <div ref="classSortTab" class="flex flex-col">
          <!-- id="classSortTab" -->
          <div v-for="(item,index) in categoryClass" :key="index"
               :class="{'select-bg':statusIndex === index}" class="flex pointer  rounded-lg items-center px-5 mb-4"
               style="background:var(--secondary-bg);height: 60px;" @click.stop="listClick(item,index)"
          >

            <template v-if="isEditing && editIndex === index">
              <div class="flex w-full justify-between items-center">
                <a-input ref="classInputRef" v-model:value="item.name" :bordered="false" spellcheck="false"
                         style="padding: 0;" @pressEnter.stop="createClass(item)">
                  <template #suffix>
                    <div class="flex">
                      <ClassIcon class="pointer" icon="fluent:checkmark-16-filled" style="font-size: 1.5em;"
                                 @click.stop="saveEdit(item)"></ClassIcon>
                      <ClassIcon class="ml-4 pointer" icon="fluent:dismiss-16-filled" style="font-size: 1.5em;"
                                 @click.stop="exitEdit"></ClassIcon>
                    </div>
                  </template>
                </a-input>
              </div>
            ...
            <template v-else>
              <div class="flex w-full justify-between">
                ...
                <div class="flex">
                  <ClassIcon class="pointer" icon="akar-icons:edit" style="font-size: 1.5em;"
                             @click.stop="edit(index)"></ClassIcon>
                  <ClassIcon class="ml-4 pointer" icon="akar-icons:trash-can" style="font-size: 1.5em;"
                             @click.stop="deleted(item)"></ClassIcon>
                </div>
              </div>
            </template>

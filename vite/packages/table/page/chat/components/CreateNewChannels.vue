<template>
  <div v-if="nextShow === false" class="flex flex-col my-3" style="width:500px;">
    ...
    <div class="flex flex-col px-6">
     ...
     你可以选择添加桌面，关联工作台内部的社区，也可以选择关联你的群聊，还支持自定义添加网页应用链接，把需要的资源全部整合到社群当中，与你的小伙伴们一起分享和协作。
      ...
      <div class="flex flex-col mt-4">
        <div v-for="item in channelList" :class="{'select-bg':selectIndex === item.type}"
             class="flex items-center pointer rounded-lg px-6 py-5 mb-4" style="background: var(--secondary-bg);"
  ...
  <SelectChannel v-if="selectIndex === 'community' && nextShow === true" :no="no" @back="nextShow = false"
                 @close="closeChannel"></SelectChannel>
  <SelectGroupChat v-if="selectIndex === 'group' && nextShow === true " :no="no" type="group" @back="nextShow = false"
                   @close="closeChannel"></SelectGroupChat>
  <SelectWebLink v-if="selectIndex === 'link' && nextShow === true " :no="no" type="link" @back="nextShow = false"
                 @close="closeChannel"></SelectWebLink>

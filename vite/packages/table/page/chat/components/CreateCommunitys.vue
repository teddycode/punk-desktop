<template>
  <div class="flex flex-col justify-between px-3 pt-3 pb-4" style="width:500px;">
    <div class="flex w-full h-10 items-center justify-center" style="position: relative;margin-bottom: 46px;">
      <!-- 左侧返回按钮 -->
      <div v-if="id === 'chat'" class="flex items-center pointer active-button rounded-lg justify-center back h-10 w-10"
           @click="backCreate">
        <!-- <LeftOutlined  /> -->
    ...
    <div class="flex items-center flex-col">
      ...
      <a-input v-model:value="communityName" class="h-12 search" placeholder="输入社群名称" spellcheck="false"
               style="width: 340px; margin-bottom:46px; border-radius: 12px;text-align: center;"></a-input>

      <a-input :disabled="true" class="h-12 search" hidden="" placeholder="ID:UH7631"
               style="width: 340px;margin-top: 12px; margin-bottom:46px; border-radius: 12px;text-align: center;"></a-input>

    </div>

    <div class="flex items-center justify-end ">
      <XtButton class="h-10 font-16-400" style="width:64px;color:var(--secondary-text);" @click="closeCreateCom">取消
      </XtButton>
      <XtButton class="h-10 ml-3" style="width:64px;background: var(--active-bg);color:var(--active-text);"
                @click="finshCreateCommunity">确定
      </XtButton>
    </div>

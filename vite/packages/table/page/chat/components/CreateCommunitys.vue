<template>
  <div class="flex flex-col justify-between px-3 pt-3 pb-4" style="width: 500px">
    <div class="flex w-full h-10 items-center justify-center" style="position: relative; margin-bottom: 46px">
      ...
      <div
        v-if="id === 'chat'"
        class="flex items-center pointer active-button rounded-lg justify-center back h-10 w-10"
        @click="backCreate"
      >
        <!-- <LeftOutlined  /> -->
      ...
      <div
        class="flex items-center pointer active-button rounded-lg justify-center close h-10 w-10"
    ...
    <div class="flex items-center flex-col">
      <div class="flex items-center flex-col justify-center" style="margin-bottom: 24px">
        <!-- 替换成图标选择器 -->
        <div
          class="rounded-lg flex pointer items-center justify-center"
        ...
        >
          ...
          <div class="overflow-hidden">
            <a-avatar
              ...
              :style="{
                filter: bgColor ? `drop-shadow(#${bgColor} 80px 0)` : '',
                transform: bgColor ? 'translateX(-80px)' : '',
              }"
            ></a-avatar>
          </div>
          <communityIcon
      ...
      <a-input
        v-model:value="communityName"
        spellcheck="false"
        placeholder="输入社群名称"
        class="h-12 search"
        style="width: 340px; margin-bottom: 46px; border-radius: 12px; text-align: center"
      ></a-input>

      <a-input
        hidden=""
        :disabled="true"
        placeholder="ID:UH7631"
        class="h-12 search"
        style="width: 340px; margin-top: 12px; margin-bottom: 46px; border-radius: 12px; text-align: center"
      ></a-input>
    </div>

    <div class="flex items-center justify-end">
      <XtButton style="width: 64px; color: var(--secondary-text)" class="h-10 font-16-400" @click="closeCreateCom"
        >取消</XtButton
      >
      <XtButton
        class="h-10 ml-3"
        style="width: 64px; background: var(--active-bg); color: var(--active-text)"
        @click="finshCreateCommunity"
        >确定</XtButton
      >
    </div>
  </div>

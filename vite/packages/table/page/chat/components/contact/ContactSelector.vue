  ...
  <div class="flex px-6">
      ...
      <template v-if="inviteMode === 'direct'">
        <template v-if="crumbsShow === false">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <a-checkbox
              class="custom-checkbox-font"
              v-model:checked="settings.isAllSelected"
          ...
          >
            ...
            >
              <template v-if="item.userInfo">
                ...
                <span class="font-16 font-400 ml-4" style="color: var(--primary-text)">{{
        ...
        <FriendCrumb
          v-else-if="crumbsShow && crumbsType === 'myFriend'"
          :selectList="selectedList"
          @currentClick="selectCurrentContact"
          @updateList="updateList"
          @back="crumbsShow = false"
        />
      </template>
      <template v-else>
    ...
    <div style="width: 293px" class="flex flex-col">
      ...
      <div class="flex justify-end mt-4">
        ...
        <div
          class="flex items-center justify-center pointer category-button rounded-lg"
          style="width: 64px; background: var(--active-bg)"
        >
          确定
        </div>
      </div>
      ...
      if (this.inviteMode === 'direct') {
        ...
        if (this.searchKeyword !== '') {
          ...
          const filterLists = arrMerge.filter((item) => {
            return (
              regex.test(item.groupID) || regex.test(item.userID) || regex.test(item.nick) || regex.test(item.name)
            );
          });

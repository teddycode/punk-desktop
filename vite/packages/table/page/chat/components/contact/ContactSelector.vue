<template>
  <div class="flex px-6">
    <div class="flex flex-col" style="width:293px;">
      ...
      <template v-if="inviteMode === 'direct'">
        <template v-if="crumbsShow === false">
          <div v-if="inviteMode === 'direct'" class="flex my-3">
            <div v-for="item in joinCategory" class="flex items-center pointer" @click="openCrumbs(item)">
              <div :style="{background:`${item.bgColor}`}"
                   class="flex items-center rounded-lg w-10 h-10 justify-center">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <a-checkbox v-model:checked="settings.isAllSelected" class="custom-checkbox-font"
                        @change="handleSelectAllChange($event)">全选
            </a-checkbox>
          </div>

          <div v-if="teamList?.length === 0" class="flex items-center h-full justify-center">
            <EmptyStatus/>
          </div>

          <vue-custom-scrollbar v-else :settings="settingsScroller" class="flex flex-col" style="height:320px;">
            ...
            >
              ...
              <template v-if="item.userInfo">
                <a-avatar :size="32" :src="item.userInfo.avatar" shape="circle"></a-avatar>
                <span class="category-16-400 ml-4" style="color:var(--primary-text);">{{
                    item.userInfo.nickname
                  }}</span>
              </template>

              <template v-else>
                <a-avatar :shape="item.nick ? 'circle' : 'square'" :size="32" :src="item.avatar"></a-avatar>
                <span class="category-16-400 ml-4"
                      style="color:var(--primary-text);">{{ item.nick ? item.nick : item.name }}</span>
              </template>
            </div>
          </vue-custom-scrollbar>
        </template>

        <FriendCrumb v-else-if="crumbsShow && crumbsType === 'myFriend'" :selectList="selectedList"
                     @back="crumbsShow = false" @currentClick="selectCurrentContact" @updateList="updateList"/>
      ...
      <template v-else>
        ...
        <vue-custom-scrollbar v-else :settings="settingsScroller" class="flex flex-col" style="height:430px;">
          <div v-for="(item,index) in teamList" :class="{'select-bg':isSelect(index)}"
               class="flex rounded-lg items-center pointer mb-4 p-3"
    ...
    <a-divider style="margin: 0 16px; height: 530px; background-color:var(--divider);" type="vertical"/>

    <div class="flex flex-col" style="width:293px">
      <div class="mb-4">已选({{ selectedList.length }}个)</div>


      <vue-custom-scrollbar :settings="settingsScroller" class="flex flex-col" style="height:440px;">
        <div v-for="item in selectedList" class="flex rounded-lg items-center justify-between pointer mb-2 p-3">
          <div v-if="item.userInfo" class="flex items-center">
            <a-avatar :size="32" :src="item.userInfo.avatar" shape="circle"></a-avatar>
            <span class="category-16-400 ml-4" style="color:var(--primary-text);">{{ item.userInfo.nickname }}</span>
          </div>
      ...
      <div class="flex justify-end mt-4">
        ...
        <div class="flex items-center justify-center pointer category-button rounded-lg"
             style="width: 64px;background: var(--active-bg);">确定
        </div>
      </div>
  ...
  methods: {
    ...
    selectCurrentContact (item) {
      if (item.userInfo) {
        ...
        if (item.groupID) {
          const findIndex = this.selectedList.findIndex((listItem) => {
            return String(listItem.groupID) === String(item.groupID)
          })
          if (findIndex === -1) {
            this.selectedList.push(item)
          } else {
          ...
          const findIndex = this.selectedList.findIndex((listItem) => {
            return String(listItem.userID) === String(item.userID)
          })
          if (findIndex === -1) {
            this.selectedList.push(item)
          } else {

<template>
  <div class="flex flex-col justify-between px-3 pt-3 pb-4" style="width:500px;">
    <div class="flex w-full h-10 items-center justify-center" style="position: relative;margin-bottom:16px;">
      ...
      <div class="flex items-center pointer active-button rounded-lg justify-center close h-10 w-10"
           @click="closeJoinCom">
    ...
    <div class="w-full flex items-center justify-center" style="margin: 0 0 16px 0;">
      <div style="width:452px;">
        <a-input v-model:value="searchId" :spellcheck="false" class="h-10" placeholder="搜索"
                 style="border-radius: 10px;" @input="searchCommunity" @keyup.enter="searchCommunity">
          <template #suffix>
    ...
    <div class="w-full flex items-center justify-center mb-6">
      <div v-if="searchId === ''" class="font-14-400" style="width:452px; color:var(--secondary-text);">
        推荐社群
      </div>
      <div v-else class="font-14-400" style="width:452px; color:var(--secondary-text);">
        搜索结果
      </div>
    </div>
    ...
    <vue-custom-scrollbar :settings="settingsScroller" style="height:466px;">
      <div class="w-full flex items-center justify-center">
        <div v-if="searchId === ''" class="flex grid grid-cols-2 gap-3" style="width:452px;">
          <div v-for="item in filterRecommendList" class="flex flex-col rounded-lg p-4"
            ...
            <div class="flex justify-between">
              ...
              <span class="font-12-400" style="color:var(--secondary-text);">{{
                  item.joinOption === 'freeAccess' ? '公开加入' : '审核加入'
                }}</span>
            </div>

            <a-divider style="height: 2px; background-color: var(--divider);margin: 16px 0;"/>

            <XtButton v-if="isJoin(item)"
                      style="background:var(--active-secondary-bg);color:var(--secondary-text);height:40px;width:100%;"
                      @click="closeJoinCom">已加入
            </XtButton>
            <XtButton v-else style="background:var(--active-bg);color:var(--active-text);height:40px;width:100%;"
                      @click="nowJoin(item)">立即加入
            </XtButton>
          </div>
        </div>

        <div v-else class="flex grid grid-cols-2 gap-3" style="width:452px;">
          <div v-for="item in searchResult" class="flex flex-col rounded-lg p-4"
               style="background: var(--secondary-bg);">
            <div class="flex items-center mb-3">
              <a-avatar :size="40" :src="item.icon" shape="square"></a-avatar>
              <span class="font-16-500 pl-3 truncate" style="color:var(--primary-text);">{{ item.name }}</span>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    // 判断是否已经加入社群
    const isJoin = (data) => {
      const index = _.find(myCom.communityList, function (item) {

<template>
  <div style="width: 500px" class="flex flex-col justify-between px-3 pt-3 pb-4">
    ...
    <div class="w-full flex items-center justify-center mb-6">
      <div style="width: 452px; color: var(--secondary-text)" class="font-14-400" v-if="searchId === ''">推荐社群</div>
      <div style="width: 452px; color: var(--secondary-text)" class="font-14-400" v-else>搜索结果</div>
    </div>
    ...
    <vue-custom-scrollbar :settings="settingsScroller" style="height: 466px">
      <div class="w-full flex items-center justify-center">
        <div class="flex grid grid-cols-2 gap-3" style="width: 452px" v-if="searchId === ''">
          <div
            v-for="item in filterRecommendList"
          ...
          >
            ...
            <XtButton
              v-else
              style="background: var(--active-bg); color: var(--active-text); height: 40px; width: 100%"
              @click="nowJoin(item)"
              >立即加入</XtButton
            >
          </div>
  ...
  setup(props, ctx) {
    ...
    const filterRecommendList = computed(() => {
      const list = Object.values(
        myCom.recommendCommunityList.reduce((acc, curr) => {
          if (!acc[curr.id]) {
            acc[curr.id] = curr;
          }
          return acc;

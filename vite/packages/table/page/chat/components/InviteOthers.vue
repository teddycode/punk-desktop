<template>
  <div v-if="inviteShow === false" class="flex flex-col mb-6" style="width: 500px">
    <div class="flex w-full items-center justify-center h-10 m-3">
      ...
      <div
        class="flex items-center category-button pointer rounded-lg justify-center w-10 h-10"
    ...
    <div class="px-6 flex flex-col">
      <div
        v-for="item in inviteMethod"
        class="invite-bg p-4 flex flex-col pointer mb-4"
        @click.stop="inviteEnter(item)"
      >
      ...
      <div class="invite-bg p-4 flex flex-col">
        ...
        <div class="flex mb-6">
          <a-input v-model:value="linkValue" class="h-10" disabled="true" style="width: 82%; border-radius: 12px" />
          <div
            class="rounded-md px-4 h-10 ml-3 category-button pointer flex items-center justify-center"
  ...
  <AddDirectly v-if="inviteShow && type === 'directly'" :no="no" @back="inviteShow = false" @close="closeButton" />
  <InvitationJoin v-if="inviteShow && type === 'inviteJoin'" :no="no" @back="inviteShow = false" @close="closeButton" />
</template>
  ...
  data() {
    return {
      ...
      inviteMethod: [
        ...
        {
          title: '邀请加入',
          description: '支持批量发送邀请信息到群聊或好友，对方接受邀请后即可加入。',
          type: 'inviteJoin',
        },
      ],
      type: '',

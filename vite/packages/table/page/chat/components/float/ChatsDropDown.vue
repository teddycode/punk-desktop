<template>
  <a-dropdown :overlayStyle="{ zIndex:'10000 !important'}" :trigger="['click']" placement="bottomLeft">
    ...
    <template #overlay>
      <a-menu class="custom-dropdown-menu flex-col flex items-center justify-center"
        ...
        <template v-for="(item,index) in dropDownList">
          ...
          <a-menu-item v-if=" index <  dropDownList.length - 2 "
                       class="rounded-lg flex items-center h-11 drop-item"
                       style="color: var(--secondary-text);width:184px;margin-bottom: 8px;"
          ...
          <a-divider v-if="index ===  dropDownList.length - 2 && dropDownList.length === 5"
                     style="height: 1px; margin: 0 0 8px 0;  background-color: var(--divider);"></a-divider>
          ...
          <a-menu-item v-if=" index >= dropDownList.length - 2 "
                       ...
                       @click="selectMenuItem(item,index)">
            <DorpIcon :icon="item.icon" style="font-size: 1.25rem;"/>
            <span :style="item.type === 'deletePacket' ? { color:'var(--error)' } : { color:'var(--primary-text)' }"
                  class="pl-4 font-16"> {{
                item.title
              }}</span>
          </a-menu-item>
  ...
  <teleport to='body'>
    <Modal v-if="categoryShow" v-model:visible="categoryShow" :blurFlag="true" style="z-index:1000 !important;">
      ...
      <PacketSetting v-if="type === 'packetSet'" :item="data" :no="no" @close="categoryShow = false"></PacketSetting>
      <AddLeftChildChannel v-if="type === 'addNewApp'" :id="data.id" :no="no"
                           @close="categoryShow = false"></AddLeftChildChannel>
    </Modal>
  </teleport>

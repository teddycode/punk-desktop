<template>
  <a-dropdown :overlayStyle="{ zIndex: '10000 !important' }" :trigger="['click']" placement="bottomLeft">
    ...
    <template #overlay>
      <a-menu
        class="custom-dropdown-menu flex-col flex items-center justify-center"
        style="background: var(--secondary-bg)"
      >
        <template v-for="(item, index) in dropDownList">
          <a-menu-item
            v-if="index < dropDownList.length - 2"
          ...
          <a-divider
            v-if="index === dropDownList.length - 2 && dropDownList.length === 5"
            style="height: 1px; margin: 0 0 8px 0; background-color: var(--divider)"
          ></a-divider>
          ...
          >
            <DorpIcon :icon="item.icon" style="font-size: 1.25rem" />
            <span
              :style="item.type === 'deletePacket' ? { color: 'var(--error)' } : { color: 'var(--primary-text)' }"
              class="pl-4 font-16"
            >
              {{ item.title }}</span
            >

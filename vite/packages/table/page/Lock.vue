<template>
  ...
  <div id="displayMiddle" class="pointer item-content" @click="enter" style="">
    ...
    <div class="mt-2 card half count-downtime item-content" v-if="countDowntime.hours">
      <div class="left-time">
        ...
        <Icon
          ...
          v-show="countDownBtn"
        ></Icon>
        <Icon style="width: 2em; height: 2em; cursor: pointer" icon="guanbi1" @click="deleteCountDown"></Icon>
      </div>
      <div class="right-time">
        <span style="color: #fbae17; text-align: center; font-size: 1.5em; margin-right: 1em">计时</span>
        <span style="color: #fbae17; font-size: 4em; font-weight: bolder">
          {{ countDowntime.hours + ':' + countDowntime.minutes + ':' + countDowntime.seconds }}</span
        >
      </div>
    </div>
  </div>
  ...
  <div class="fixed inset-0 home-blur" style="z-index: 999999999999" v-if="visible">
    ...
    >
      <div>{{ clockEvent[0].dateValue.hours }}:{{ clockEvent[0].dateValue.minutes }}</div>
      <div>
        {{ clockEvent[0].eventValue }}
      </div>
      <div
        style="width: 100px; height: 48px"
        class="flex justify-center items-center text-base bg-blue-500 rounded-lg pointer"
        @click="handleOk"
      >
        好的
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      visible: false,
      wallPaper:
        'https://ts1.cn.mm.bing.net/th/id/R-C.653b2eb4ae081875675c6d25a69834b0?rik=p%2fCn01vlMrCUxQ&riu=http%3a%2f%2fimg.pconline.com.cn%2fimages%2fupload%2fupc%2ftx%2fwallpaper%2f1208%2f02%2fc0%2f12659156_1343874598199.jpg&ehk=d8OPA9%2bWy7YX9FLF95st3Rmd8lG6XtopCz0uNZAbebs%3d&risl=&pid=ImgRaw&r=0',
      lively: [
        {
      ...
      handler(newVal, oldVal) {
        try {
          if (
            ...
            this.clockEvent[0].flag === undefined
          ) {
            this.visible = true;
            setTimeout(() => {

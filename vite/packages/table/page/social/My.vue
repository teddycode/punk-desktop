<template>
  <vueCustomScrollbar :settings="scrollbarSettings" style="padding: 15px;white-space: nowrap;height: 100%"
    ...
    <div class="card half mr-3"
         style="width:330px;background: var(--primary-bg);color: var(--primary-text);padding:0;position: relative">
      <userCard :key="key" :uid="userInfo.uid" :userInfo="userInfo">
      </userCard>
      <span class="px-3 py-1 xt-active-bg rounded-full  pointer" style="position: absolute;right: 20px;top: 40px;"
            @click="toggleFrameStore()"><icon icon="gift" style="font-size: 18px"></icon> 头像框</span>
    </div>
    ...
    <div class="mr-3" style="width: 400px;display: inline-block;vertical-align: top;white-space: pre-wrap">
      <!-- 快速搜索 用户->我的 -->
      <div class="card half"
           style="width: 400px;padding: 1em;display: inline-block;margin-bottom: 1em;background: var(--primary-bg);color: var(--primary-text);">
        <ComPanel></ComPanel>
      </div>
      <div class="card half"
           style="width: 400px;padding: 1em;display: inline-block;background: var(--primary-bg);color: var(--primary-text);">

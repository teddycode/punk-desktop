    ...
    >
      ...
      <a-col :md="24" style="flex: 1">
        ...
        <HorizontalPanel
          @changed="navChanged"
          :navList="category"
          v-model:selectType="categoryType"
            ...
            <vueCustomScrollbar :settings="scrollbarSettings" class="pt-4" style="height: calc(100vh - 18em)">
              <div class="sub-tab" style="text-align: center">
                <a-row :gutter="20" style="width: 300px; margin: auto">
                  ...
                  >
                  <a-col
                    @click="setCurrentSubTab('comment')"
                    :class="{ active: currentSubTab === 'comment' }"
                    :span="6"
                    class="item"
                    >评论</a-col
                  >
                  <a-col
                    @click="setCurrentSubTab('support')"
                    :class="{ active: currentSubTab === 'support' }"
                    :span="6"
                    class="item"
                    >点赞</a-col
                  >
                  <a-col
                    ...
                    :class="{ active: currentSubTab === 'mention' }"
                    :span="6"
                    class="item"
                    >@</a-col
                  >
              ...
              <div v-else v-for="item in interact" class="px-4 interact-hover rounded-lg py-4 flex items-center">
                <div>
                  <a-avatar
                    @click.stop="showCard(item)"
                    v-if="item.user"
                    :size="50"
                    :src="item.user.avatar_128"
                    class="avatar-list pointer"
                  ></a-avatar>
                </div>
                <div class="flex flex-col ml-4" style="flex: 1">
                  <div class="flex">
                    <span
                      ...
                      class="pr-1 truncate pointer interact-name"
                      style="max-width: 84px; font-size: 16px; color: var(--primary-text); font-weight: 600"
                    >
                      {{ item.title }}
                    </span>
                    <span
                      class="pr-1 truncate dynamic"
                      style="font-size: 16px; color: var(--primary-text); font-weight: 400"
                    >
                      {{ item.dynamic }}
                    </span>
                  </div>
                  <div @click="clickInteract(item)" class="truncate pointer" style="width: 98%">
                    <span class="w-100 interact-content" style="color: var(--primary-text)">{{ item.content }}</span>
                  </div>
            ...
            >
              ...
              <div class="flex flex-col ml-4">
                <div class="flex">
                  ...
                  <span @click="showCard(item)" class="pr-1 truncate dynamic pointer"> 关注了你 </span>
...
export default {
  ...
  data() {
    return {
      ...
      category: [
        { title: '互动', name: 'interact', state: false },
        { title: '关注', name: 'attention', state: false },
  ...
  computed: {
    ...
    interact() {
      ...
      return sorted.filter((item) => {
        if (this.currentSubTab === 'all') return true;

<template>
    ...
    <a-table v-else :columns="checkColumns" :data-source="testData" :pagination="{ pageSize: 10 }" :scroll="{ y: 400 }">
      <template #bodyCell="{ column,index,record}">
        <template v-if="column.dataIndex === 'controls'">
          <div v-if="index === 0" class="rounded-xl pointer button-active h-10 flex items-center justify-center"
  ...
  <Modal v-show="checkDetailShow" v-model:visible="checkDetailShow" :blurFlag="true">
    <div class="flex flex-col" style="width: 480px;">
      <div class="w-full flex p-3 mb-6">
        ...
        <div class="p-3 flex items-center pointer justify-center button-active rounded-lg"
             style="background: var(--secondary-bg);" @click="checkDetailShow = false">
      ...
      <div class=" w-full px-10 pb-10">
        <div class="flex items-center rounded-xl p-4 mb-6" style="width: 400px;background: var(--secondary-bg);">
          <div class="w-16 h-16 mr-4">
            <img :src="checkDetailsData.frame_url" alt="" class="w-full h-full object-cover">
          </div>

<template>
      ...
      <div class="w-full flex flex-col  top-card mr-4" style=" max-width: 572px;">
        ...
        <div class="rounded-lg flex-1 w-full  tip-card px-5 text-white" style="background: var(--primary-bg);">
          <div class="flex items-center -ml-4 mt-3">
            <Icon class="mx-2 text-white" icon="-dengpao" style="width: 24px;height: 24px;"></Icon>
            Tips
          </div>
      ...
      <div class="grade-card p-6 rounded-lg  mr-4" style=" background:var(--primary-bg)">
        ...
        <div class="flex flex-row">
          <div class="mt-6 mr-4" style="width: 160px">
            <a-steps ref="step" :current="abilityGrade" :progressDot="true" direction="vertical">
              <a-step v-for="(item, index) in leveList" :title="'Lv ' + item" class="left-box" description="null"
                      style="color:var(--primary-text);" @click="clickStep(index)"/>
            </a-steps>
          </div>
          <div class="flex flex-wrap justify-between  bottom-card w-full">
            ...
            <div v-for="item in powerList"
              ...
              <Icon :icon="item.icon" class="mr-4 ml-2 text-white"
                    style="width: 24px;height: 24px;color: var(--primary-text);"></Icon>
              <div style="width: calc(100% - 50px)">
                ...
                <div v-if="item.detail[[leveList[clickGrade]]]" class=" text-more w-full xt-text-2">
                  {{ item.detail[[leveList[clickGrade]]] }}
                </div>
              </div>
              <div v-if="item.noOnline" class="grade-tip" style="color: rgba(255, 255, 255, 0.85);">即将上线</div>
            </div>
          </div>
        </div>
      </div>
  ...
  <div v-if="tipFlag" class="home-blur fixed inset-0 float-tip" style="z-index: 999" @click.stop="closeTip">

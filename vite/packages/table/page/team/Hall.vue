<template>
  <a-row :gutter="20">
    ...
    <a-col :span="3">
      <div
        ...
        class="s-bg pointer h-12 w-24 rounded-lg flex justify-center items-center px-3"
      >
  ...
  <a-drawer placement="right" v-model:visible="actionVisible">
    ...
    <div
      ...
      class="ml-5 bg-mask p-5 rounded-lg"
      v-if="selectTeam"
    >
      ...
      <div class="mt-3" v-if="selectTeam.leaderInfo">
        ...
        <div class="mt-2 ml-2">
          <a-avatar
            @click.stop="showUserCard(selectTeam.leaderInfo.data.uid, selectTeam.leaderInfo.data.userInfo)"
            class="mr-3 pointer"
            :size="40"
            :src="selectTeam.leaderInfo.data.userInfo.avatar"
          ></a-avatar
      ...
      <div v-if="selectTeam.members.data.length">
        <div class="mb-3 mt-3">队员：</div>
        <div class="mb-3 ml-3" :span="4" v-for="member in selectTeam.members.data">
          <a-avatar
            class="mr-2 pointer"
            @click="showUserCard(member.userInfo.uid, member.userInfo)"
            v-if="member.userInfo"
...
export default {
  ...
  data() {
    return {
      ...
      list: [],
      teamNavList: [
        { title: '活跃小队', name: 'active' },
        { title: '最新创建', name: 'latest' },
      ],

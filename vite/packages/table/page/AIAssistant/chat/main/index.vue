...
export default {
  computed: {
    ...mapWritableState(aiStore, ['selectTopicIndex', 'topicList', 'chatList', 'url', 'key', 'count', 'searchState']),
    currentList() {
  ...
  methods: {
    ...
    async onSearch(search) {
      ...
      this.chatList[this.selectTopicIndex].slice(-1 * this.count).forEach((item) => {
        arr.push({
          role: item.role,
    ...
    async processGPTResults(arr) {
      ...
      for await (const result of getStreamData(arr)) {
        ...
        const index = this.chatList[this.selectTopicIndex].findIndex((item) => item.id === result.id);

        if (index !== -1) {
          this.chatList[this.selectTopicIndex][this.chatList[this.selectTopicIndex].length - 1].content +=
            result.content;
        } else {
          let assistant = {

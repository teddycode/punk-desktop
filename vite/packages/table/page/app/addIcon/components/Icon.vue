...
export default {
  ...
  computed: {
    ...
    // 全选状态
    selectAll() {
      if (this.selectAppsLength && this.selectAppsLength == this.appsLenght && this.isSelectedArr[this.name] !== true) {
    ...
    // 选中数组长度
    selectAppsLength() {
      return this.selectApps[this.name] ? this.selectApps[this.name].length : '';
    },
    // 数组总长度
    appsLenght() {
  ...
  methods: {
    ...
    toggleSelect(index) {
      if (this.isSelected(index)) {
        // 二维数组不存在则开辟一个数组
        // if (!this.selectApps[this.name]) this.selectApps[this.name] = [];
        this.selectedIndexes[this.name] = this.selectedIndexes[this.name].filter((i) => i !== index); // 取消选中
      } else {
    ...
    renderIcon,
    isSelected(index) {
      if (!this.selectedIndexes[this.name]) this.selectedIndexes[this.name] = [];

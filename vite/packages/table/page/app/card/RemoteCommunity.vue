<template>
  <div v-if="!openShare" class="fixed inset-0 home-blur xt-mask" style="z-index: 99999;">
    <div
      ...
      <dvi v-if="selectNav.name === 'share'" class="share-list">
        <Market :desk="desk" setCard="tip" @closeMarket="close" @getCard="getCard"></Market>
      </dvi>
      <div v-if="selectNav.name !== 'share' && !shareList.length" class="body-box">
        <div class="box">
          <div class="box-block">
            <img alt="" src="/img/state/init.png">
      ...
      <div v-if="selectNav.name !== 'share' && shareList.length" class="share-list">
        <Market :desk="desk" listType="my" setCard="my" @closeMarket="close" @closeMy="closeMy"
                @shareNow="shareNow"></Market>
      </div>
    </div>
  </div>
  <div v-else>
    <RemoteShare :cardId="cardId" :desk="desk" :direct="direct" :openShare="openShare"
                 @closeShare="closeShare"></RemoteShare>
  </div>
  ...
  <Modal v-if="promptVisible" v-model:visible="promptVisible" blurFlag="true" style="z-index:99999;">
    <div class="p-5 xt-modal flex flex-col justify-center items-center" style="border-radius:16px">
      ...
      <span class="my-5"
            style="font-size: 16px;color: var(--secondary-text);">选择覆盖当前小组件的数据或在桌面上新建一个组件。</span>
      <div class="modal-btn">

<template>
  <div class="p-3">
              ...
              <a-dropdown-button :type="!task.running ? 'primary' : ''" @click="handleButtonClick">
                ...
                <template #overlay>
                  <a-menu @click="handleMenuClick">
                    <a-menu-item v-if="task.running" @click="refresh" key="1"> 立即刷新 </a-menu-item>
                    <a-menu-item v-if="task.running" @click="showTask" key="1"> 显示运行中网页 </a-menu-item>
                    <a-menu-item @click="openUrl" key="2"> 浏览器打开源网页 </a-menu-item>
                    <a-menu-item key="3"> 编辑任务 </a-menu-item>
                  </a-menu>
                </template>
              </a-dropdown-button>
            </a-col>
    ...
    <vue-custom-scrollbar :settings="outerSettings" style="position: relative; height: calc(100vh - 21em)">
      <div
        id="report"
        style="max-width: 900px; margin: auto; background: var(--primary-bg); color: var(--primary-text)"
      >
        <div class="p-3">
          <a-row class="mb-5" :gutter="[20, 10]">
            ...
            <a-col :md="18">
              <div class="mb-2" style="">
                <span data-html2canvas-ignore>任务：{{ task.title }} </span>&nbsp; &nbsp;发布于 {{ data.pudate || '-' }}
              </div>
              <div>
                <a-row>
              ...
              <div data-html2canvas-ignore>
                ...
                <a-row>
                  ...
                  <a-col :span="14">
                    <div>
                      <div>
                        {{ stage.data.name }}流量池：<span class="text-white"
                          >{{ formatWan(stage.data.show[0]) }}-{{ formatWan(stage.data.show[1]) }}</span
                        >
                      </div>
                      <div>
...
8.全站推荐
                ...
                <div class="text-lg mt-2">
                  <span v-if="stage.condition === 'pre'">
                    <Icon style="font-size: 40px; vertical-align: middle" icon="gaoxingneng" /> 正在
                    <strong class="text-white">预热</strong> 阶段</span
                  >
                  <span v-if="stage.condition === 'in'"
                    ><Icon style="font-size: 40px; vertical-align: middle; color: white" icon="xingneng" /> 正在
                    <strong class="text-white">推送</strong> 阶段</span
                  >
                </div>
          ...
          <a-row :gutter="40">
            ...
            <a-col :span="12">
              ...
              <a-row>
                <a-col :span="24">
                  <div>
                    <div v-if="suggestions.length === 0">暂无评价</div>
                    <div v-else>

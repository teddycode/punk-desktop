  ...
  <div class="cpu-index">
      ...
      <div class="right-content">
        <div class="cpu">
          <div class="cpu-number">
            <span>温度</span>
            <span style="font-weight: 700">{{ CPUData.TCPUPKG.value }}℃</span>
          </div>
        </div>
        <a-progress
          :showInfo="false"
          :status="CPUData.TCPUPKG.value === 0 ? '' : 'active'"
          :percent="CPUData.TCPUPKG.value"
          :stroke-color="{
            '0%': '#60BFFF',
            '100%': '#348FFF',
          }"
    ...
    <div class="content-small">
      <a-progress
        type="circle"
        stroke-color="#FF9C00"
        :percent="GPUData.SGPU1UTI.value"
        strokeWidth="11"
        :width="160"
        class="left-content"
      >
      ...
      <div class="right-content">
        <div class="cpu">
          <div class="cpu-number">
            <span>温度</span>
            <span style="font-weight: 700">{{ GPUData.TGPU1DIO.value }}℃</span>
          </div>
        </div>
        <a-progress
          :showInfo="false"
          :status="GPUData.TGPU1DIO.value === 0 ? '' : 'active'"
          :percent="GPUData.TGPU1DIO.value"
          :stroke-color="{
            '0%': '#60BFFF',
            '100%': '#348FFF',
          }"
      ...
      handler(newVal, oldVal) {
        ...
        if (this.GPUData.TGPU1DIO.value === 0) {
          for (let i = 0; i < Object.keys(this.aidaData).length; i++) {
            if (Object.keys(this.aidaData)[i] === 'TCPUGTC') this.GPUData.TGPU1DIO.value = this.aidaData.TCPUGTC.value;
          }
        }

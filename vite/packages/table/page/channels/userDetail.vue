  ...
  <div class="flex justify-center h-full ml-3">
    <div class="pl-3 pr-3" style="width: 95%">
      <div class="flex justify-between h-[40px] mb-3">
        <div @click="OnBack" class="xt-text font-16 hover_underline" style="line-height: 40px">返回</div>
      </div>
          ...
          <a-card style="margin-bottom: 30px" :bordered="false">
            <template #cover>
              <div class="image-container">
                <img :src="UserDetail.backgroundImg" class="centered-image" />
              </div>
            </template>
            <div class="spacebetween">
              <a-card-meta :title="UserDetail.nickname">
                <template #description>
                  <u-fold v-if="UserDetail && UserDetail?.description" line="1">
              ...
              <div style="width: 600px; display: flex; justify-content: space-around; align-items: center">
                ...
                <div style="padding: 10px 0; text-align: center">
                  <p
                    ...
                    @click="router.push({ name: 'userFollowers', query: { id: UserDetail.id, activeKey: '1' } })"
                ...
                <div style="padding: 10px 0; text-align: center">
                  <p
                    class="hover_underline"
                    @click="router.push({ name: 'userFollowers', query: { id: UserDetail.id, activeKey: '2' } })"
                    style="font-weight: bold; margin: 0; padding: 0"
                  >
                    正在关注
                  </p>
                ...
                <a-button
                  ...
                  size="large"
                  danger
                  @click="store._userUnFollow(UserDetail)"
                  >取消关注</a-button
                >
                <a-button
                  v-else
                  ...
                  @click="store._userFollow(UserDetail)"
                  >关注</a-button
                >
              </div>
            </div>
            ...
            <div class="icon-text">
              <CalendarOutlined style="margin-right: 10px" />注册于：{{ UserDetail.createTime }}
            </div>
            <div>
              <a-tabs v-model:activeKey="activeKey" size="large" centered>
                <a-tab-pane key="1" tab="帖子">
                  <div class="flex flex-col justify-center content">
                    ...
                    <!-- 循环渲染多个 ComCard -->
                    <a-empty
                      v-if="UserForums?.length === 0"
                      description="暂无内容"
                      image="/img/test/load-ail.png"
                      style="margin-top: 30%"
                    ></a-empty>
                    <template v-else>
                      <ComList v-for="(card, index) in UserForums" :key="index" :cardData="card" class="xt-bg">
                      </ComList>
                    </template>
                  </div>
                </a-tab-pane>
                <a-tab-pane key="2" tab="点赞" force-render>
                  <div class="flex flex-col justify-center content">
                    ...
                    <!-- 循环渲染多个 ComCard -->
                    <a-empty
                      v-if="UserLoves?.length === 0"
                      description="暂无内容"
                      image="/img/test/load-ail.png"
                      style="margin-top: 30%"
                    ></a-empty>
                    <template v-else>
                      <ComList v-for="(card, index) in UserLoves" :key="index" :cardData="card"> </ComList>
                    </template>
                  </div>
                </a-tab-pane>
                <a-tab-pane key="3" tab="收藏" force-render>
                  <div class="flex flex-col justify-center content">
                    ...
                    <!-- 循环渲染多个 ComCard -->
                    <a-empty
                      v-if="UserCollects?.length === 0"
                      description="暂无内容"
                      image="/img/test/load-ail.png"
                      style="margin-top: 30%"
                    ></a-empty>
                    <template v-else>
                      <ComList v-for="(card, index) in UserCollects" :key="index" :cardData="card"> </ComList>
                    </template>
                  </div>
                </a-tab-pane>
              </a-tabs>

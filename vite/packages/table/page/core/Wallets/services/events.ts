...
import { RequestLoginNonce, UserDoAuthReq } from '@package/../src/api/users.ts';
...
export const setupWalletListener = (callback): void => {
  const app = appStore();
  // 监听钱包事件
  const w3mEvent = useWeb3ModalEvents();
  watch(w3mEvent, () => {
    console.log(w3mEvent.data);
      ...
      case 'MODAL_CLOSE': // 只能通过监听close事件实现
        ...
        if (app.userInfo.value) {
          ...
          if (connectStatus) {
            ...
            // 向后端请求一个随机数
            RequestLoginNonce(useWeb3ModalAccount().address.value).then((res) => {
              console.log('获取后端数据：', res);
              if (res?.code === 200 && res?.data !== null) {
                // 向钱包请求签名
                toast.success('正在请求签名');
                signMessage(res?.data).then((data: SignMessage): void => {
                  console.log('获取返回数据：', data);
                  // 返回签名到后端并获取登录结果
                  toast.success('请求验证');
                  UserDoAuthReq(data).then((resp) => {

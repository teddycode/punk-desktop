<template>
  <div class="px-5 pt-3.5">
    ...
    <div class="h-20 set-bg rounded-lg flex flex-row items-center px-6 mb-4" style="width: 572px">
      ...
      <a-select
        ...
        class="w-56 h-auto rounded-lg text-xs s-item"
        size="large"
        :bordered="false"
        v-model:value="area"
      >
  ...
  <Modal
    v-model:visible="proxyVisibility"
    v-show="proxyVisibility"
  ...
  >
    <div class="p-10" style="width: 400px">
      ...
      <div class="line">
        <a-row>
          <a-col :span="6" class="text-left"> 类型： </a-col>
      ...
      <div class="line">
        <a-row>
          <a-col :span="5" class="text-left"> 地址： </a-col>
          <a-col :span="19">
      ...
      <div class="line">
        <a-row>
          <a-col :span="5" class="text-left"> 端口： </a-col
          ><a-col :span="19"> <a-input class="w-full" v-model:value="settings.proxy.port"></a-input></a-col
        ></a-row>
      </div>
      <div class="line">
        <a-row>
          <a-col :span="5" class="text-left"> 用户名： </a-col
          ><a-col :span="19"><a-input v-model:value="settings.proxy.userName"></a-input></a-col>
        </a-row>
      </div>
      <div class="line">
        <a-row>
          <a-col :span="5" class="text-left"> 密码： </a-col
          ><a-col :span="19"><a-input-password v-model:value="settings.proxy.password"></a-input-password></a-col>
        </a-row>
      </div>
    </div>
  </Modal>
  ...
  >
    <div class="flex flex-col p-6 xt-text" @click.stop>
      ...
      <HorizontalPanel
        :navList="loginTypeList"
        v-model:selectType="loginType"
        class="mt-4 mx-auto"
      ...
      <div class="mt-3 mb-0 pl-2">
        ...
        网络不好建议设置代理！ steam302用户请看-><a
          style="color: var(--secondary-text)"
          target="_blank"
          href="https://www.yuque.com/tswork/mqon1y/kvinb8xbzw2eaa2e"
          >技术说明</a
        >
      </div>
      <div class="mt-3">
        <a-input
          ...
          placeholder="用户名（非邮箱）"
          class="no-drag rounded-lg h-12 mx-auto"
          spellcheck="false"
          style="width: 328px; background: var(--secondary-bg)"
          @click.stop
        />
      </div>
      <div class="mt-6">
        <a-input-password
          @keyup.enter="bindSteam"
      ...
      <div class="mt-6" v-show="loginType.name === 'phone'">
        <a-input
          ...
          class="no-drag rounded-lg h-12 mx-auto"
          placeholder="令牌"
          style="width: 328px; background: var(--secondary-bg); text-transform: uppercase"
          @click.stop
        />
      </div>
      <div class="mt-6" v-show="mailBoxShow && loginType.name !== 'phone'">
        <a-input
          v-model:value="mailBoxAuthCode"
          class="no-drag rounded-lg h-12 mx-auto"
          placeholder="邮箱验证码"
          style="width: 328px; background: var(--secondary-bg)"
          @click.stop
      ...
      <div class="flex justify-between mt-6">
        <div
          class="s-item h-12 rounded-lg flex justify-center items-center pointer w-40"
...
export default {
  ...
  data() {
    return {
      ...
      mailBoxShow: false,
      loginTypeList: [
        { title: '普通登录', name: 'mailBox' },
        { title: '手机令牌离线登录', name: 'phone' },
      ],
      ...
      switch (this.loginType.name) {
        case 'mailBox':
          if (this.mailBoxShow !== true) {
            ...
            window.steamSession
              .submitSteamGuardCode(this.mailBoxAuthCode)
              .then((res) => {
                message.info({
                  content: '登录成功',
                });
        ...
        case 'phone':
          window.steamSession
            ...
            .then((res) => {
              window.steamSession
                .submitSteamGuardCode(this.authCode)
                .then((res) => {})
                .catch((err) => {
                  console.error(err);
                  message.error({

    ...
    <div class="pt-3.5">
      <div class="flex flex-row items-center game-page-nav">
        ...
        <div class="flex flex-row ml-3">
          <div @click="openDrawer" class="my-bg my-title pointer h-12 w-12 rounded-lg flex justify-center items-center">
      ...
      >
        <div class="flex flex-row flex-wrap -ml-3" v-if="gameType.name === 'other' && localGameList.length > 0">
          ...
          <div class="pb-3 pl-3 game-list-local flex-shrink-0 my-game-content" v-for="(item, index) in displayGameList">
            <div
              class="my-bg h-full pointer w-full rounded-lg"
              style="padding-top: 30px"
              @click="openMyGame(item)"
              @contextmenu="openOtherDetail(item)"
            >
              <div
                class="relative mx-auto"
                style="height: 65px; width: 65px"
                :class="hoverIndex === index ? 'fly' : ''"
        ...
        >
          ...
          <!--  </div>-->
          <div
            class="pb-3 pl-3 game-list-item flex-shrink-0 my-game-content"
            v-for="(item, index) in displaySteamGameList"
          >
            <div
              class="relative w-auto h-full my-bg pointer flex flex-col"
            ...
            >
              ...
              <div :style="showTime ? 'height: 96px' : 'height: 50px'" class="p-3 flex flex-col justify-between">
                <div class="truncate text-white text-base my-item" style="font-weight: 400">
                  <a-tag color="red" v-if="getHide('steam', item.appid)">隐</a-tag>

                  {{ item.chineseName }}
                </div>
                <span :style="showTime ? '' : 'display:none'" class="text-xs my-num"
                  >过去两周：{{ twoWeekTime(item.time) }}小时</span
                >
                <span :style="showTime ? '' : 'display:none'" class="text-xs my-num"
                  >总数：{{ totalTime(item.time) }}小时</span
                >
              </div>
            </div>
          </div>
      ...
      <div class="p-6 my-bg" @click.stop>
        ...
        <div class="flex flex-row mt-4">
          <div
            ...
            class="pointer h-12 w-48 rounded-lg flex justify-center items-center"
            @click.stop="openLocal"
          >
            选择游戏
          </div>
          <div
    ...
    <Modal v-model:visible="steamShow" v-if="steamShow" animationName="bounce-in" :blurFlag="true">
      <div class="flex flex-col" style="border-radius: 6px; overflow: hidden" v-if="currentSteamGame">
        <div class="relative" style="height: 188px; width: 400px" v-if="currentSteamGame">
          <img
            :src="'https://cdn.cloudflare.steamstatic.com/steam/apps/' + currentSteamGame.appid + '/header.jpg'"
            style=""
            class="w-full h-full object-cover"
            alt=""
          />
        ...
        <div class="p-4">
          <div style="user-select: text" class="text-white flex">
            <a-avatar
              shape="square"
              :size="24"
              :src="getClientIcon(this.currentSteamGame.appid, this.currentSteamGame.clientIcon)"
            ></a-avatar>
            <div class="xt-text ml-2 truncate" style="max-width: 300px">
              {{ currentSteamGame.chineseName }} <span class="xt-text-2">{{ currentSteamGame.appid }}</span>
            </div>
          </div>
          ...
          <div class="flex flex-row justify-between mt-3 text-white">
            <div
              v-if="runningGame.appid !== currentSteamGame.appid"
    ...
    <Modal v-model:visible="otherShow" v-show="otherShow" animationName="bounce-in" :blurFlag="true">
      ...
      <div class="pt-12 pb-8 px-8">
        ...
        <div class="flex flex-col justify-between">
          ...
          <div
            @click="openMyGame(currentLocalGame)"
...
export default {
  ...
  data() {
    return {
      ...
      selectName: '',
      gameNavList: [
        { title: 'Steam游戏', name: 'steam' },
        { title: '其他游戏', name: 'other' },
      ],
  ...
  computed: {
    ...
    selectSteamList() {
      ...
      } else return this.steamGameList;
    },
    shareContent() {
      return {
        description: 'steam游戏',

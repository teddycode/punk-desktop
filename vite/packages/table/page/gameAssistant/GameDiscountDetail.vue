  ...
  <div class="flex">
    <div class="flex-grow">
      <div class="left-flex flex items-center">
        <div
          class="w-12 h-12 flex items-center button-active cursor-pointer detail-bg justify-center rounded-lg"
          @click="goBack"
        >
          <Icon icon="xiangzuo" style="font-size: 1.5em"></Icon>
        </div>
        <span style="font-size: 20px; font-weight: 500" class="ml-4 px-2 py-2 rounded-lg detail-name">{{
          discountDetail.name
        }}</span>
      </div>
    </div>
    <div class="flex-grow-0">
      <div class="right-flex flex items-center justify-center mr-5">
        ...
        <span class="rounded-lg px-10 py-2 button-active detail-bg pointer mr-3 min-store" @click="enterSteamStore"
          >前往购买</span
        >
        <span class="px-10 py-2 detail-bg button-active rounded-lg pointer" @click="openDetailDrawer"
          >地区:{{ defaultDetailRegion.name }}</span
        >
      </div>
    </div>
  </div>
    ...
    <div class="detail-container">
      ...
      <div class="flex flex-col min-width detail-bg ml-4 rounded-lg px-4 py-4">
        ...
        <div class="flex items-center mt-1">
          <span
            v-for="item in discountDetail.genres"
            class="rounded-lg mr-1"
            style="background: rgba(255, 255, 255, 0.4); padding: 2px 6px 1px 6px"
        ...
        <div style="padding: 12px 13px" class="detail-bg flex rounded-lg items-center justify-between mt-6">
          <div class="flex flex-col mt-1">
            ...
            <span style="font-size: 14px; max-width: 120px; font-weight: 400; color: var(--secondary-text)"
              >特价促销！{{ acquisitionDate(exTime) }}截止</span
            >
          </div>
          <div class="flex flex-col">
            <span
              ...
              >{{ discountDetail.oldPrice }}</span
            >
            <div>
              <span
                class="mr-2 rounded-lg oswald-font"
                style="color: var(--active-text); padding: 2px 4px 5px 4px; background: rgba(255, 77, 79, 1)"
              >
                -{{ discountDetail.percent }}%</span
              >

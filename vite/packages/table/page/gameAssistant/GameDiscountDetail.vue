  ...
  <div class="flex">
    <div class="flex-grow">
      <div class="left-flex flex items-center">
        <div class="w-12 h-12 flex items-center button-active cursor-pointer detail-bg justify-center rounded-lg "
             @click="goBack">
          <Icon icon="xiangzuo" style="font-size:1.5em;"></Icon>
        </div>
        <span class="ml-4 px-2 py-2 rounded-lg detail-name"
              style="font-size:20px;font-weight: 500;">{{ discountDetail.name }}</span>
      </div>
    </div>
    <div class="flex-grow-0">
      <div class="right-flex flex items-center justify-center mr-5">
        <xt-button :h="40" :w="40" class="mr-3" @click="shareSteamGame">
          <Icon icon="fenxiang" style=""></Icon>
        </xt-button>
        <span class="rounded-lg px-10 py-2 button-active detail-bg pointer mr-3 min-store"
              @click="enterSteamStore">前往购买</span>
        <span class="px-10 py-2 detail-bg button-active rounded-lg pointer"
              @click="openDetailDrawer">地区:{{ defaultDetailRegion.name }}</span>
      </div>
    </div>
  </div>
  <vue-custom-scrollbar :settings="settingsScroller" class="mt-3 mr-3" style="height: calc(100vh - 15.8em)">
    <div class="detail-container">
      <div v-if="loading">
      </div>
      <WheelCastingUnit v-else :loading="loading" :wheelList="wheelData"></WheelCastingUnit>
      <div class="flex flex-col min-width detail-bg ml-4 rounded-lg px-4 py-4">
        ...
        <div class="flex justify-between items-center mt-4">
          ...
          <span style="font-size: 16px; font-weight: 400; color: var(--primary-text);">{{
              discountDetail.IssueDate
            }}</span>
        </div>
        <div class="flex justify-between items-center mt-4">
        ...
        <div class="flex items-center mt-1">
          <span v-for="item in discountDetail.genres" class="rounded-lg mr-1"
                style="background: rgba(255, 255, 255, 0.4);padding: 2px 6px 1px 6px;">{{ item.description }}</span>
        </div>
        ...
        <div class="detail-bg flex rounded-lg items-center justify-between mt-6" style="padding: 12px 13px;">
          <div class="flex flex-col mt-1">
            <span style="font-size: 16px; font-weight: 400; color: var(--secondary-text);">价格</span>
            <span style="font-size: 14px; max-width: 120px; font-weight: 400; color: var(--secondary-text);">特价促销！{{
                acquisitionDate(exTime)
              }}截止</span>
          </div>
          <div class="flex flex-col">
            <span class="line-through mr-2"
                  style="text-align: right; color:rgba(255, 255, 255, 0.4);font-size: 14px; font-weight: 400;">{{
                discountDetail.oldPrice
              }}</span>
            <div>
    ...
    <div class="flex items-center justify-center flex-col mt-5">
      <div class="rounded-lg px-4 py-4 min-introduction detail-bg" style="width: 960px;">
        ...
        <div class="game-intro" v-html="discountDetail.gameIntroduction"></div>
      </div>
    </div>
  </vue-custom-scrollbar>

  <HorizontalDrawer
      ref="regionDrawer" :drawerTitle="drawerTitle" :rightSelect="rightSelect"
      @getArea="getArea"
  ></HorizontalDrawer>
</template>

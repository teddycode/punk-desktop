<template>
  <div class="mx-7 pt-3.5 flex flex-col">
    ...
    <div class="flex justify-between h-12 items-center mb-4">
      ...
      <div
        class="rounded-lg button-active p-3 place-name pointer"
        @click="openRightDrawer"
        v-if="leftTitleType.name === 'steam'"
      >
        地区: {{ placeName.name }}
      </div>
    </div>
    ...
    <template v-if="leftTitleType.name === 'steam'">
      ...
      >
        <div class="steam-top-content w-full mb-4">
          <div
            v-for="item in steamList.slice(0, 2)"
          ...
          >
            ...
            <div class="px-2.5 mb-3" style="color: var(--secondary-text)">
              优惠截止时间: {{ deadline(item.discount_expiration) }}
            </div>
        ...
        <div class="steam-bottom-content">
          <div
            v-for="item in steamList.slice(2)"
            @click="enterDiscountDetail(item)"
    ...
    <template v-else>
      <vue-custom-scrollbar
        ...
        @touchend.stop
        :settings="settingsScroller"
        style="height: calc(100vh - 15.8em)"
      >
        <div class="flex epic-container">
          <div
            v-for="item in epicList"
            @click="openEpicStore(item)"
            class="mr-4 mb-4 discount-bg pointer flex flex-col rounded-lg"
          >
            <div class="epic-image">
              <img
                :src="getSpecifiedPicture(item.keyImages).url"
            ...
            <div class="p-3">
              <div v-if="item.promotions.promotionalOffers.length !== 0" class="flex item-center">
                ...
                <span class="py-1 time-text"
                  >截止时间: {{ deadline(item.promotions.promotionalOffers[0].promotionalOffers[0].endDate) }}</span
                >
              </div>
              <div v-if="item.promotions.upcomingPromotionalOffers.length !== 0" class="flex item-center">
                ...
                <span class="py-1 time-text"
                  >开始时间:
                  {{ deadline(item.promotions.upcomingPromotionalOffers[0].promotionalOffers[0].startDate) }}</span
                >
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      leftTitle: [
        { title: 'Steam折扣', name: 'steam' },
        { title: 'Epic喜加一', name: 'epic' },
      ],
  ...
  methods: {
    ...
    getSteamData(v) {
      if (!this.isLoading) {
        ...
        sendRequest(url, {}, { localCache: true, localTtl: 60 * 12 * 60 })
          .then((result) => {

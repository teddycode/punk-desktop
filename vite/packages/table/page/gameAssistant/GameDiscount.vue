<template>
  <div class="mx-7 pt-3.5 flex flex-col">
    ...
    <div class="flex justify-between h-12 items-center mb-4">
      <HorizontalPanel v-model:selectType="leftTitleType" :navList="leftTitle"></HorizontalPanel>
      <div v-if="leftTitleType.name === 'steam' " class="rounded-lg button-active p-3 place-name pointer"
           @click="openRightDrawer">地区: {{ placeName.name }}
      </div>
    </div>
    ...
    <template v-if="leftTitleType.name === 'steam' ">
      ...
      <vue-custom-scrollbar v-else :settings="settingsScroller" class="mt-3 px-3" style="height: calc(100vh - 16.5em)"
        ...
        <div class="steam-top-content w-full mb-4">
          ...
          >
            <img :src="item.header_image" alt="" class="rounded-t-lg"
                 style="width:100%;height: 100%;object-fit: cover;">
            <div class="p-2.5 name-text">{{ item.name }}</div>
            <div class="px-2.5 mb-3" style="color: var(--secondary-text);">优惠截止时间:
              {{ deadline(item.discount_expiration) }}
            </div>
      ...
      <vue-custom-scrollbar :settings="settingsScroller" style="height: calc(100vh - 15.8em)" @touchstart.stop
        ...
        <div class="flex epic-container">
          <div v-for="item in epicList" class="mr-4 mb-4 discount-bg pointer flex flex-col rounded-lg"
               @click="openEpicStore(item)">
            <div class="epic-image">
              <img :src="getSpecifiedPicture(item.keyImages).url" alt=""
                   class="w-full h-full rounded-t-lg object-cover">
            </div>
            <div class="p-3 name-text epic-title truncate">{{
                item.title === 'Mystery Game' ? '神秘游戏' : item.title
              }}
            </div>
            <div class="p-3">
              <div v-if="item.promotions.promotionalOffers.length !== 0" class="flex item-center">
                ...
                <span class="py-1 time-text">截止时间: {{
                    deadline(item.promotions.promotionalOffers[0].promotionalOffers[0].endDate)
                  }}</span>
              </div>
              <div v-if="item.promotions.upcomingPromotionalOffers.length !== 0" class="flex item-center">
                ...
                <span class="py-1 time-text">开始时间: {{
                    deadline(item.promotions.upcomingPromotionalOffers[0].promotionalOffers[0].startDate)
                  }}</span>
              </div>
            </div>
          </div>
  ...
  <HorizontalDrawer ref="regionDrawer" :drawerTitle="drawerTitle" :rightSelect="rightSelect"
                    @getArea="getArea"></HorizontalDrawer>
</template>

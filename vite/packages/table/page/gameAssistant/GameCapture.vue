      ...
      <div style="width: 490px; height: 100%">
        <div v-if="step === 1" style="height: 100%; display: flex; flex-direction: column">
          ...
          <HorizontalCapture
            @click="refreshSource"
            :navList="captureType"
            v-model:selectType="defaultRecordingType"
            class="mb-4"
          ></HorizontalCapture>
          ...
          <div v-else style="flex: 1; height: 0">
            <template v-if="defaultRecordingType.name === 'recordGame'">
              ...
              >
                ...
                <div class="flex justify-between flex-wrap" v-else>
                  <div
                    v-for="(item, index) in recordGameData"
            ...
            <template v-if="defaultRecordingType.name === 'recordFullScreen'">
              ...
              >
                ...
                <div class="flex justify-between flex-wrap" v-else>
                  <div
                    v-for="(item, index) in deskSource"
            ...
            <template v-if="defaultRecordingType.name === 'logger'">
              ...
              >
                ...
                <div class="flex justify-between flex-wrap" v-else>
                  <div
                    v-for="(item, index) in windowSource"
        ...
        <div v-if="step === 2" style="height: 100%">
          <vue-custom-scrollbar :settings="settingsScroller" style="height: 100%">
            ...
            <div style="height: 5em; position: relative">
              <back-btn
                @click="step = 1"
                class="capture-bg capture-title"
                style="margin-top: -30px; margin-left: -20px"
              ></back-btn>
              <div class="capture-title" style="position: absolute; left: 70px; top: 10px; font-size: 16px">
                已选择：
                <a-avatar
                  v-if="currentSource.type === 'window'"
                  :size="24"
                  shape="square"
                  :src="'file://' + currentSource.icon"
                ></a-avatar>
                {{ currentSource.name }}
              </div>
            ...
            <div class="flex mb-3">
              <div
                ...
                class="cp-w cp-orange-1 flex-col rounded-lg pointer mr-3 hover-btn"
                @click="screenshot"
              >
            ...
            <div class="capture-bg rounded-md p-4 mb-3" v-if="isHeight === true">
              <div class="flex items-center mb-5">
                <div class="pointer" @click="clickMute">
                  <Icon
                    icon="yinliang"
                    style="font-size: 1.5em; color: var(--primary-text)"
                    v-if="!systemSound.muted"
                  ></Icon>
            ...
            <div style="text-align: center" v-if="recording">
              ...
              正在录制视频：已录制{{ recordedTimeStr }}， 码率：{{
                (this.settings.videoBitsPerSecond / 1024 / 1024).toFixed(2)
              }}Mbps 预估大小：{{
                ((this.settings.videoBitsPerSecond / 1024 / 1024 / 8) * this.recordedSeconds).toFixed(2)
              }}MB
            </div>
            <!--  捕获预览        -->
            <div v-if="recentFileName && !recording">
              <div v-if="recentType === 'video'">
                ...
                <VideoItem
                  :key="recentKey"
                  :options="{ controls: true, loop: false }"
                  :url="'file://' + this.getRecentVideoPath()"
                ></VideoItem>
              </div>
              <div v-else>
                <div class="mb-2 truncate">
    ...
    <div class="ml-3 rounded-md w-full flex flex-col cap-right">
      ...
      <div class="flex-1 flex capture-bg rounded-md p-3 flex-col" style="height: 0">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center justify-center">
            <Icon icon="video" style="font-size: 1.75em; color: var(--primary-text)"></Icon>
            <span class="ml-3" style="font-size: 16px; color: var(--primary-text); font-weight: 400"
              >最近截屏和录制</span
            >
          </div>
          <div class="flex items-center pointer justify-center" @click="openRecordSet">
        ...
        <div class="flex-1" style="height: 0">
          <template v-if="defaultLastCap.name === 'screenCap'">
            ...
            >
              <div class="flex flex-row flex-wrap content-game">
                <div
                  class="game-list-item my-game-content my-image"
          ...
          <template v-else>
            ...
            >
              <div class="flex flex-row flex-wrap content-game">
                <div class="game-list-item px-1.5 pb-4 flex-shrink-0 my-game-content" v-for="item in pagedVideos">
                  <div
                    class="relative w-auto h-full s-item rounded-md overflow-hidden pointer flex flex-col"
    ...
    <div class="flex flex-col scroll-container">
      <div class="flex flex-col rounded-md p-4 mb-3" style="background: var(--secondary-bg)">
        <div class="flex items-center mb-3">
          <div class="pointer" @click="clickMute">
            <Icon icon="yinliang" style="font-size: 1.5em; color: var(--primary-text)" v-if="!systemSound.muted"></Icon>
            <Icon icon="jingyin" v-else style="font-size: 1.5em; color: var(--primary-text)"></Icon>
          </div>
          <span class="mx-3" style="color: var(--primary-text)">系统声音</span>
      ...
      <span
        ...
        >{{ settings.imageSavePath }}</span
      >
      <span class="mb-3 fps-t">我的录制保存地址</span>
      <span
        @click="setVideoSavePath"
        style="background: var(--secondary-bg)"
        class="text-center mb-3 py-3 rounded-lg"
        >{{ settings.videoSavePath }}</span
      >
      <span class="mb-3 fps-t">视频码率</span>
      <a-select v-model:value="settings.videoBitsPerSecond">
        <a-select-option :value="1.5e6"> 1.5Mbps </a-select-option>
        <a-select-option :value="2.5e6"> 2.5Mbps </a-select-option>
        <a-select-option :value="5.0e6"> 5.0Mbps </a-select-option>
        <a-select-option :value="8.0e6"> 8.0Mbps </a-select-option>
        <a-select-option :value="10.0e6"> 10.0Mbps </a-select-option>
      </a-select>
  ...
  <a-drawer width="500" title="设置" :placement="right" v-model:open="setShow">
    <div class="flex flex-col">
      <div
        ...
        class="h-12 mb-2 flex rounded-lg pointer items-center justify-center"
        style="background: var(--secondary-bg)"
        @click="selectMonitorItem(item)"
        :class="{ 'xt-active-bg cap-active': this.settings.inspectorIndex === item.id }"
      >
        {{ item.name }}
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    callback(image) {
      ...
      const filename =
        this.filterName(this.currentSource.name) +
        '_' +
        time.year +
        '年' +
        time.month +

...
export const teamStore = defineStore('teamStore', {
  ...
  getters: {
    hasTeam: (state) => {
      if (state.my.created || state.my.joined) return true;
      else return false;
    },
    myTeamNo(state) {
      let teamNo = 0;
  ...
  actions: {
    ...
    async getTeamLeader(no, cache = 1, userCache = 1) {
      let teamLeaderResult = await axios.post(
        getTeamLeaderUrl,
        { no: no, cache: cache, userCache: userCache },
        await getConfig(),
      );
    ...
    async getTeamMembers(no, cache = 1, userCache = 1) {
      let teamMembersResult = await axios.post(
        getTeamMembersUrl,
        { no: no, cache: cache, userCache: userCache },
        await getConfig(),
      );
    ...
    async getMemberDevote() {
      let result = await axios.get(getMemberDevote, await getConfig());
      if (result.code === 1000) {
    ...
    async exchangeDevote(from_uid = 0) {
      let result = await axios.post(exchangeDevoteUrl, { from_uid: from_uid }, await getConfig());
      if (result.code === 1000) {
        let data = result.data;
        if (data) {
          return {
    ...
    async joinByNo(no) {
      try {
        let rs = await axios.post(joinByNoUrl, { no: no }, await getConfig());
        return rs;
      } catch (e) {
        return {
          status: 0,
    ...
    async quitByNo(no, uid) {
      try {
        let rs = await axios.post(quitByNoUrl, { no: no, uid: uid }, await getConfig());
        return rs;
      } catch (e) {
        return {
          status: 0,
    ...
    async disbandByNo(no) {
      try {
        let rs = await axios.post(disbandByNoUrl, { teamNo: no }, await getConfig());
        return rs;
      } catch (e) {
        return {
          status: 0,

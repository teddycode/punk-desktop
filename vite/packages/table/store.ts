...
// @ts-ignore
        state: () => ({
            ...
            fullScreen: true, //是否是全屏模式
            searchFullScreen: true, // 聚合搜索全屏
        ...
        actions: {
            ...
            setUser(userInfo) {
                userInfo.onlineGradeExtra.cumulativeMinutes =
                    Number(userInfo.onlineGradeExtra.minutes) -
                    Number(userInfo.onlineGradeExtra.cumulativeHours) * 60;
                ...
                function handleGrade(name) {
                    for (let i = 0; i < userInfo.onlineGrade[name]; i++) {
                        userInfo.onlineGradeIcons[name].push({
                            icon:
                                "file://" +
                                window.globalArgs["app-dir_name"] +
            ...
            setSearchFullScreen(value: boolean) {
                // 设置聚合搜索全屏
            ...
            setSearchUrlOpenType(value: object) {
                // 设置搜索路径打开方式类型

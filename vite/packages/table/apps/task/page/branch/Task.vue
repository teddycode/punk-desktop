<template>
  <div>
    <xt-text class="mb-3">
      <xt-new-icon icon="fluent:chevron-left-16-filled" @click="back()" bgClass="xt-bg-2" class="mr-3" w="40" />
      {{ task.chapter }}
      <template #right> <Progress style="width: 68px; height: 20px" :task="task" /></template>
    </xt-text>
    <xt-collapse v-for="data in task.tasks">
      ...
      <xt-button v-if="taskState(data)" @click="getReceive(data)" style="background: #faad14 !important; width: 100%"
        >领取奖励</xt-button
      >
      <xt-button v-else @click="startTak(data)" type="theme" style="width: 100%">开始任务</xt-button>
    </xt-collapse>
  </div>
</template>

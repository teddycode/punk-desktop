<template>
  <div class="w-full h-full flex justify-center items-center">
    <div class="w-full center overflow-hidden" style="max-width: 800px;max-height: 660px;height: 90%;">
      <!-- 顶部 -->
      <div class="flex w-full" style="max-width: 800px;justify-content: space-between;">
        <div class="flex items-center">
          <div
              class="flex justify-center items-center mr-3 pointer"
              style="background-color: var(--secondary-bg);border-radius:10px;width: 107px;height: 40px;"
              @click="selDesk"
          >#{{ deskName }}
          </div>
        ...
        <div class="flex" style="position: relative;">
          ...
          <!-- 颜色选择 -->
          <div v-show="isColor"
               class="flex flex-wrap rounded p-3"
               style="justify-content: space-around;position: absolute;width: 168px;height: 120px;top: 46px;left: -118px;background-color: #2A2A2A;z-index: 100;">
            <div v-for="(item,index) in this.noteBgColor" :key="index"
                 :style="{backgroundImage:item}" class="flex rounded-lg pointer" style="height:40px;width:40px;"
                 @click="changeBgColor(index)"></div>
          </div>
          <a-dropdown :trigger="['click']">
            <div class="flex items-center pointer justify-center"
                 style="width: 40px;height:40px;background: rgba(0,0,0,0.30);border-radius: 10px;">
      ...
      <!-- 主体 -->
      <div>
        <div :style="{backgroundImage:backgroundImage}" class="mt-4 shadow"
             style="height: 600px;border-radius: 12px;padding: 24px;">
          <!-- 标题 -->
          <a-input

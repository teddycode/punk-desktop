...
<template>
  <div
    style="width: 338px; height: 420px; flex-shrink: 0"
    v-if="tab === 'item'"
    class="flex flex-col rounded-lg justify-between"
    @contextmenu="switchTab('menu')"
  >
    <!-- 文本卡片顶部标题开始 -->
  ...
  <div v-else-if="tab === 'menu'" style="width: 338px; height: 420px">
    <div class="flex s-item flex-col rounded-lg px-4 py-3 h-full default-content">
      <div class="flex mb-3">
        <div
          class="w-12 s-item button-active bt-default h-12 rounded-lg pointer flex items-center justify-center"
          @click="backClip"
        >
  ...
  <slot v-else-if="tab === 'other'" name="otherTabs"> </slot>
</template>

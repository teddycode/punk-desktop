<template>
  ...
  <div v-if="isWin()">
    <div class="xt-bg-2 rounded-lg p-4 px-5 w-auto m-auto" v-if="!settings.enable">
      ...
      <xt-button @click="settings.enable = true" type="theme" size="mini" :w="70" :h="35">开启</xt-button>
    </div>
  </div>
  <template v-else>
  ...
  <div class="text-center">{{ clipContents.length }} 条记录</div>
  <!-- 剪切板列表展示区域结束 -->
  ...
  <a-drawer :width="500" v-model:visible="drawerVisible" title="搜索" placement="right">
    <div class="flex mb-3">
      <a-input
        ...
        allow-clear
        placeholder="输入关键词"
        class="no-drag h-10 w-full"
        v-model:value="searchWords"
        style="background: var(--secondary-bg)"
      ></a-input>
      <div
        class="h-10 w-24 ml-3 s-item pointer flex button-active items-center justify-center rounded-lg"
        @click="clickSearch"
        style="background: var(--secondary-bg)"
      >
        搜索

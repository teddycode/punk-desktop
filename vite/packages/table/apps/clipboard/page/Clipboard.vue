<template>
  <div class="flex items-center clip-container justify-between mx-4 my-4">
    <div class="flex">
      <!-- tab切换开始 -->
      <HorzontanlPanelIcon v-model:selectType="tab" :navList="clipType" class="mr-3"></HorzontanlPanelIcon>
      <!-- tab切换结束 -->

      <!-- 导航栏筛选开始 -->
      <TabSwitching v-model:activeType="currentFilter" :navList="filterList"></TabSwitching>
  ...
  <div v-if="this.searchWords" class="xt-bg pointer rounded-full p-2 text-center mx-10" @click="drawerVisible=true">
    关键词：{{ this.searchWords }}
    <close-circle-filled class="ml-3" @click.stop="this.searchWords='';this.doSearch()"/>
  </div>
  <!-- 剪切板列表展示区域开始 -->
  <div v-if="isWin()">
    <div v-if="!settings.enable" class="xt-bg-2 rounded-lg p-4 px-5 w-auto m-auto">
      <div class="float-left mr-2 pt-1">
        <iconify icon="akar-icons:info-fill" style="font-size: 18px;vertical-align: text-bottom"></iconify>
        当前剪切板功能未开启，请在设置中开启，或者直接点击
      </div>
      <xt-button :h="35" :w="70" size="mini" type="theme" @click="settings.enable=true">开启</xt-button>
    </div>
  </div>
  <template v-else>
    <div class="xt-bg-2 rounded-lg p-4 px-5 w-auto m-auto">
      <div class=" mr-2 pt-1 ">
        <iconify icon="akar-icons:info-fill" style="font-size: 18px;vertical-align: text-bottom"></iconify>
        暂不支持在非Windows平台上使用剪切板功能。请耐心等待版本更新。
      </div>
    </div>
  </template>

  <vue-custom-scrollbar ref="wrapper" :settings="settingsScroller" class="mx-4 my-2 py-4 h-full " style="width: 100%"
                        @ps-x-reach-end="doLoadNextPage" @touchstart.stop @touchmove.stop
                        @touchend.stop>
  ...
  <!-- 搜索右侧抽屉开始 -->
  <a-drawer v-model:visible="drawerVisible" :width="500" placement="right" title="搜索">
    <div class="flex mb-3">
      <a-input v-model:value="searchWords" allow-clear class="no-drag h-10 w-full" placeholder="输入关键词"
               style="background: var(--secondary-bg);"
               @keydown.enter="clickSearch"></a-input>
      <div class="h-10 w-24 ml-3 s-item pointer flex button-active items-center justify-center rounded-lg"
           style="background: var(--secondary-bg);" @click="clickSearch">
        搜索
      </div>
    </div>

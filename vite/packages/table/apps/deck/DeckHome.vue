<template>
  <div v-if="!isWin()" class="xt-bg-2 mt-10 rounded-lg p-4 px-5 w-auto m-auto">
    <div class="mr-2 pt-1">
      <iconify style="font-size: 18px; vertical-align: text-bottom" icon="akar-icons:info-fill"></iconify>
    ...
    >
      <div style="width: auto; white-space: nowrap">
        <div
          ...
          :style="{ width: getWidth(grid.cols) }"
        ...
        >
          ...
          <template v-else>
            <h3 class="pointer text-more" v-if="editing">
              <span v-if="editing"
                ><span style="margin-left: 0.8em"
                  ><left-square-outlined v-if="index !== 0" @click.stop="moveGrid(-1, index)" class="mr-3" />
                  <right-square-outlined v-if="index !== this.grids.length - 1" @click.stop="moveGrid(1, index)"
                /></span>
            ...
            <h3
              ...
              style="padding-left: 0.8em; margin-bottom: 0em"
              v-else
          ...
          <div>
            ...
            <div
              ...
              v-if="grid.children.length === 0 && editing === false"
              class="grid"
            >
            ...
            <vuuri
              v-show="grid.children.length !== 0 || editing"
              :style="{ width: getVuuriWidth(grid.cols) }"
              :key="key"
              ...
              item-key="id"
              class="grid s-bg"
              :get-item-width="getIconSize"
              :get-item-height="getIconSize"
              v-model="grid.children"
            >
              <template #item="{ item }">
                <Widget
                  @contextmenu.stop="showMenu(item.id, { item, grid }, 'item')"
                  :item="item"
  ...
  <a-drawer title="修改组设置" placement="right" :closable="true" v-model:open="editGridVisible" @close="onClose">
    ...
    <div class="line">
      ...
      <a-input-number
        ...
        step="1"
        addon-before="列数"
        v-model:value="currentGrid.cols"
        :defalut-value="2"
      ></a-input-number>
    </div>
  </a-drawer>
  <a-drawer title="导入方案" placement="right" :closable="true" v-model:open="importMenuJsonVisible" @close="onClose">
    ...
    <div class="line">
      <a-textarea
        placeholder="在此处粘贴分享代码，点击导入分享代码即可完成导入。"
        style="width: 100%; height: 22em"
        v-model:value="importJsonTxt"
      ></a-textarea>
    </div>
    <div class="line">
      <a-button
        :disabled="this.importJsonTxt === ''"
        class="mr-5"
        type="primary"
        @click="importCode"
        style="background: var(--primary-bg); color: var(--primary-text)"
      ...
      <a-button @click="importFile" style="background: var(--primary-bg); color: var(--primary-text)"
        >导入文件</a-button
      >
    </div>
  </a-drawer>
      ...
      <a-col>
        <div @click="addBoard" class="btn relative">
          ...
          <div
            ...
            style="border-radius: 6px; background: rgba(42, 42, 42, 0.6)"
            v-show="superiorLimit <= grids.length"
            @click.stop="limitTip"
          ></div>
  ...
  <Prompt
    @cancel="this.visiblePromptTitle = false"
    content="请输入分组标题"
    title="编辑组标题"
    placeholder="输入标题"
    :visible="visiblePromptTitle"
  ...
  methods: {
    ...
    remove() {
      Modal.confirm({
        ...
        okText: '确认删除',
        onOk: () => {
          this.currentGrid.children.splice(
            this.currentGrid.children.findIndex((item) => {

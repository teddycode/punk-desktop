    ...
    <div class="add-content" v-if="defaultNavType.name === 'info'">
      ...
      <div class="flex">
        ...
        <div class="ml-10 xt-text-2" style="font-size: 16px">
          ...
          >
            <div class="flex mt-2">
              <xt-button class="mr-2" type="theme" @click.stop="imageSelect" style="width: 120px; height: 48px"
                >自定义上传
              </xt-button>
              <xt-button @click.stop="resetIcon" style="width: 120px; height: 48px">同软件图标 </xt-button>
            </div>
          </a-upload>
        </div>
      </div>
      ...
      <div
        @click="nextStep"
        class="pointer flex items-center rounded-lg justify-center mr-3 mt-6 xt-active-btn"
      ...
      <vue-custom-scrollbar :settings="settingsScroller" :style="closePrompt ? 'height:90%' : 'height:100%'">
        <div @click.self="completeEdit" class="key-box" :style="keyBoxStyle" id="keyBox">
          <div v-for="(item, index) in keyList" :key="item.id">
            ...
            <!-- 快捷键 -->
            <div
              :style="{ backgroundColor: !item.isEdit ? getColor(index) : '' }"
              v-else
              ...
              <div class="flex">
                ...
                <div v-else class="flex items-center mr-3" @click="openKeyBoard(item, 'key')">
                  ...
                  >
                    <template #suffix>
                      <div
                        class="w-8 h-8 flex rounded-lg justify-center items-center"
                        style="background: rgba(80, 139, 254, 0.2)"
                      >

  ...
  <div class="flex flex-col rounded-lg w-full px-2 h-full">
    ...
    <div class="flex flex-1 justify-between px-4 h-0">
      <vue-custom-scrollbar :settings="settings" class="h-full w-full">
        ...
        ></div>
        <div v-else class="main-part item-content" style="flex: 1">
          <div
            :class="{ selectable: selecting, selected: isSelected(item.id) }"
            v-for="item in list"
          ...
          >
            ...
            <span class="xt-text truncate" style="max-width: 180px" :title="item.name">
              {{ item.name }}

  ...
  methods: {
    ...
    async importSchemes() {
      let openPath = await tsbApi.dialog.showOpenDialog({
        ...
        properties: ['openFile', 'createDirectory', 'showOverwriteConfirmation', 'multiSelections'],
      });
      if (!openPath) {
        return;
      }
  ...
  <div class="flex flex-col rounded-lg w-full px-2 h-full">
    <div class="m-2 my-1 flex">
      <div v-if="!exeName" class="flex-1">
        ...
        <p class="xt-text-2">
          <span v-if="selecting"
            >已选中 <strong class="xt-active-text">{{ selectedCount }}</strong> 个方案，
          </span>
          共 {{ shortcutSchemeList.length }} 个方案
        </p>
      </div>
      ...
      <div>
        <div class="btn-item mt-3">
          ...
          <!--            </span>-->
          <xt-button @click="createScheme"> <Icon class="icon" icon="akar-icons:edit"> </Icon> 创建 </xt-button>
          ...
          <template v-if="!selecting">
            <xt-button @click="edit"> <Icon class="icon" icon="akar-icons:chevron-left"> </Icon> 管理 </xt-button>
          </template>
        </div>
      </div>
    </div>
    <div class="flex flex-1 justify-between px-4 h-0">
      <vue-custom-scrollbar :settings="settings" class="h-full w-full">
        <div
          v-if="shortcutSchemeList.length === 0 && loading === false"
          class="w-full"
          style="flex: 1; justify-items: center; justify-content: center"
        >
          <NotShortcutKey @createScheme="createScheme"></NotShortcutKey>
        </div>
        <div v-else class="scheme-container" style="flex: 1">
          <div
            :class="{ selectable: selecting, selected: item.selected }"
            v-for="item in shortcutSchemeList"
            class="scheme-item items-center pointer xt-bg-2"

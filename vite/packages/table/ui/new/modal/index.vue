<template>
  <Teleport to="body">
    ...
    <div
      ...
      style="border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0px 0px 10px 0px rgba(0, 0, 0, 0.5)"
      :style="{
        'z-index': index,
      }"
    >
          ...
          <header class="flex items-center mb-4" v-if="header">
            ...
            <div class="flex items-center">
              <slot name="header-left">
                <template v-if="back">
                  <xt-button w="32" h="32" radius="8" class="mr-4" @click="onBack()">
                    <xt-new-icon icon="fluent:chevron-left-16-filled" size="16" class="xt-text-2" />
                  </xt-button>
                </template>
              </slot>
            </div>
            ...
            <div class="flex items-center">
              ...
              <xt-button class="ml-3" w="32" h="32" radius="8" @click="onNo()">
                <xt-new-icon icon="fluent:dismiss-16-filled" size="16" class="xt-text-2" />
              </xt-button>
            </div>
          </header>
          <!-- 内容区 -->
      ...
      <slot name="footer">
        <footer class="flex justify-end mt-4 items-center" v-if="footer">
          ...
          <xt-button w="64" h="40" class="ml-3" type="theme" @click="onOk()">{{ okName }}</xt-button>
        </footer>
      </slot>
    </div>
    <!-- 遮罩 -->

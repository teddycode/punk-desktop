<template>
  ...
  <div
    ...
    style="
      ...
      z-index: 1000;
    "
    class="float-icon fixed top-1/2 left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2"
    id="selectIcon"
  >
    <!-- 选择列表 -->
    <div class="top-icon flex">
      <div class="type-select flex" style="color: var(--primary-text)">
        <span v-for="(item, index) in menus" @click="onSelChange(index)" :class="selIndex == index ? 'active' : ''">{{
          (item, index)
        }}</span>
        <!-- <span @click="onSelChange(1)" :class="selIndex == 1 ? 'active':''">Emojis</span>
            ...
            -->
        <span @click="onSelChange(menus.length)" v-if="isUpload" :class="selIndex == menus.length ? 'active' : ''"
          >自定义</span
        >
      </div>
    ...
    <!-- {{ list }} -->
    <div
      v-for="(value, key) in handleList"
      v-show="key == menus[selIndex]"
      :key="key"
    ...
    >
      ...
      >
        <a-tooltip>
          <template #title>{{ item.alias }}</template>
          <a-avatar
            :src="`https://a.apps.vip/icons/iconSelect/${menus[selIndex]}/${item.name}.svg`"
            :alt="item.alias"
            width="32"
            height="32"
            :style="{
              filter: menus[selIndex] == 'icon' ? `drop-shadow(${bgColor[selBgColor]} 80px 0)` : '',
              transform: menus[selIndex] == 'icon' ? 'translateX(-80px)' : '',
            }"
          ></a-avatar>
        </a-tooltip>
      </div>

          ...
          messageExamine: async function () {
            ...
            (window as any).$TUIKit.TUIServer.TUIConversation.getConversationProfile(conversationID).then(
              (imResponse: any) => {
                ...
                (window as any).$TUIKit.TUIServer.TUIConversation.handleCurrentConversation(
  ...
  public async receiveNotification(msg: any) {
    ...
    if (isText) {
      ...
      // 群聊消息
        ...
        const groupResult = await (window as any).$TUIKit.tim.getGroupList();
        const groupList = groupResult?.data?.groupList;
        const findItem = groupList.find((item: any) => {
          return String(item.groupID) === String(data.to);
        });
        const textContent = {

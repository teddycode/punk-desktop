...
<template>
    ...
    >
      <div :id="'frameTop_' + item.nanoid" class="avatar-top flex mb-4">
        ...
        <div class="flex flex-col justify-center ml-4">
          ...
          <span class="get-way-font"> 获得途径：{{ avatarGainMethodText(item.detail.gainMethod) }} </span>
        </div>
      </div>
      <a-row :gutter="10">
        <a-col :span="12">
        ...
        <a-col :span="12">
          <a-button class="rounded-xl h-12 w-full xt-bg xt-text-2" @click="download(item)"> 下载头像 </a-button>
        </a-col>
      </a-row>
    </div>
  </vue-custom-scrollbar>
  <Modal v-if="downloadVisible" v-model:visible="downloadVisible" blur-flag="true">
    <div class="p-5">
      ...
      <div class="mt-2">
        <a-button block class="rounded-full" type="primary" @click="doDownload"> 下载头像 </a-button>
      </div>
    </div>
  </Modal>
</template>

<template>
  <div style="padding-top: 20px; padding-left: 20px">
    <a-row :gutter="20">
      ...
      <a-col :span="18">
        <div class="mb-1 2 font-bold truncate">{{ userInfo.nickname }}</div>
        <div>
  ...
  <div class="mb-0 pd-0 m-3 p-3 mt-0" style="margin-bottom: 0">成就勋章</div>
  ...
  <div class="mb-0 pd-0 m-3 p-3 mt-0">小队贡献</div>
...
export default {
  ...
  methods: {
    ...
    async updateUserInfo() {
      ...
      this.joined_days = Number(
        ((new Date().getTime() - new Date(this.joinedTime).getTime()) / (24 * 60 * 60 * 1000)).toFixed(0),
      );

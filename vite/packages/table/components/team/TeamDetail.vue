<template>
  <div
    class="bg-mask rounded-lg m-3 p-3"
    style="line-height: 2; background: var(--primary-bg); color: var(--primary-text)"
  >
    升级效率：<strong class="mr-3" style="color: #48ef48">{{ effect }} % </strong>
    <span v-if="online - 1 > 0">{{ online - 1 }} 位队友在线 *5%</span><span v-else>无队友在线</span>
    <br />
    <a-row>
      <a-col> 小队等级：{{ team.rankInfo.grade.level }}级 </a-col>
      <a-col>
    ...
    升级剩余时长：{{
      parseHours(getRemain(team.rankInfo.grade.level, team.rankInfo.onlineInfo.total_time), 1)
    }}
    小时<br />
  ...
  <div style="position: absolute; bottom: 0; margin-left: 34px; color: var(--primary-text)">
    <a-row :gutter="16" class="m-5">
      <a-col>
        <div
          v-if="teamLeader.userInfo.uid !== userInfo.uid"
...
export default {
  ...
  methods: {
    ...
    async dismiss() {
      Modal.confirm({
        content:
          '您确定要解散团队？此操作不可撤销。此操作将清退小队全部队员，且未领取的小队收益将被清空。建议先提醒小队成员领取后再解散。',
        okText: '解散团队',
        centered: true,
        onOk: async () => {

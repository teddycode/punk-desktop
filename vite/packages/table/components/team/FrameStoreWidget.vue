<template>
  <div class="pl-4 pt-4 h-full flex flex-col" style="height: 100%">
    <HorizontalPanel v-model:selectType="listItem" :navList="avatarList" class="mb-5"></HorizontalPanel>
    <template v-if="listItem.name === 'shop_store'">
      ...
      >
        <div
          ...
          class="w-full mb-3 rounded-lg flex flex-col"
        >
          <div class="avatar-top flex" style="position: relative">
            <div style="width: 100px; height: 100px">
              <RayMedal
                v-if="item.frame.rarity >= 4"
                :size="120"
                :src="item.cover"
              ...
              <div v-if="getFramePrice(item)?.originPrice" class="xt-text-2 text-center" style="margin-top: -8px">
                <a-badge
                  :count="getFramePrice(item)?.discountReason"
                  :numberStyle="{ backgroundColor: '#52c41a', borderColor: 'transparent' }"
                ></a-badge>
              </div>
            </div>
            <div class="flex flex-col justify-center ml-4">
              ...
              <span class="get-way-font mt-1"> 获得途径：{{ avatarGainMethodText(item.frame.gainMethod) }} </span>
            ...
            <div
              class="xt-bg px-3 py-1 rounded-full pointer"
          ...
          <div v-if="item.prices.length !== 0" class="avatar-bottom flex">
            <a-button
              ...
              style="color: var(--active-text); height: 44px"
              type="primary"
              @click="buyNow(item)"
            >
              赞助 ￥ {{ getFramePrice(item).price }}
              <template v-if="+getFramePrice(item).originPrice"
                ><span class="line-through ml-2">￥{{ getFramePrice(item).originPrice }}</span>
                <a-badge
                  :count="getDiscount(getFramePrice(item))"
            ...
            <a-button
              ...
              type="default"
            >
              已有 <span class="ml-2">￥ {{ getFramePrice(item).price }}</span>
              <template v-if="getFramePrice(item).originPrice"
                ><span class="line-through ml-2">￥{{ getFramePrice(item).originPrice }}</span>
              </template>
            </a-button>
            <a-button
              v-if="getFrameScore(item)"
              class="mr-3 rounded-xl avatar-font flex items-center justify-center"
              hidden=""
              style="color: var(--active-text); height: 44px"
              type="primary"
              @click="scorePay(item)"
  ...
  >
    <div v-if="isPay === false" class="w-full flex items-center mb-6">
      ...
      <div
        class="w-12 flex items-center justify-center h-12 rounded-lg active-button pointer"
  ...
  >
    <div class="flex mb-6">
      ...
      <div
        class="w-12 h-12 flex items-center pointer justify-center rounded-lg active-button"
        style="color: var(--secondary-text); background: var(--secondary-bg)"
  ...
  <Modal v-if="tryFrameVisible" v-model:visible="tryFrameVisible" :blur-flag="true">
    <div class="p-8 text-center flex flex-col justify-center" style="width: 200px; height: 200px">
      <FrameAvatar
        :avatar-size="50"
        :avatar-url="userInfo.avatar"
        :frame-url="tringFrame"

<template>
  <div
    ...
    :class="{ active: isActive(action.name) }"
    :style="{ 'border-left-color': group.color }"
    class="action"
    @click="setActive(action)"
  >
    {{ action.title }}
  </div>
  <div v-for="action in actions">
    <div v-if="this.isActive(action.name)">
      <div v-if="action.summary" class="line" v-html="action.summary"></div>
      <template v-for="input in action.inputs">
        <div v-if="getShow(action, input, 'input')" class="line">
          {{ input.title }}：
          <a-input
            v-model:value="action.data[input.name]"
            :placeholder="input.placeholder"
            :style="{ width: input.width }"
          ></a-input>
        </div>
        <div v-if="getShow(action, input, 'switch')" class="line">
          {{ input.title }}：
          <a-switch v-model:checked="action.data[input.name]"></a-switch>
        </div>
        <div v-if="getShow(action, input, 'radio')" class="line">
          {{ input.title }}：
        ...
        <div v-if="getShow(action, input, 'file')" class="line">
          {{ input.title }}：
          <a-input
            v-model:value="action.data[input.name]"
            :placeholder="input.placeholder"
            style="width: 15em"
            @click="showOpenFileDialog(action, input)"
          ></a-input>
        </div>
        <template v-if="getShow(action, input, 'textarea')" class="line">
          <div class="line">{{ input.title }}：</div>
          <div class="line">

  ...
  <div style="height: 100%; display: flex; justify-content: center; align-content: center; flex-direction: column">
    ...
    >
      ...
      >
        ...
        ></a-image>
        <span v-if="screen.key !== 'main'" class="p-2 screen-title" style="">{{ screen.title }}</span>
        <span
          v-else
      ...
      <div class="text-right">
        <a-button class="mr-3 no-dark" style="color: var(--primary-text)" @click="addScreen">创建 </a-button>
        <a-button style="color: var(--primary-text)" @click="tagScreen">辨别 </a-button>
      </div>
    </div>
    <div class="p-3" @click.stop>
      <div class="line-title text-center" style="font-size: 24px">
        <span v-if="!editing" :style="{ cursor: currentScreen.key !== 'main' ? 'text' : '' }" @click="toggleEdit"
          ><edit-outlined v-if="currentScreen.key !== 'main'" /> {{ currentScreen.title }}</span
        >
        ...
        <a-row :gutter="[20, 20]" style="font-size: 1.2em; text-align: center">
          <a-col :span="5">
            <div class="btn relative">
              <a-switch v-model:checked="currentScreen.running" @change="toggleScreen"></a-switch>

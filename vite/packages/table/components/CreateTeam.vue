  ...
  <div class="flex flex-row items-center justify-center w-full h-full">
    ...
    >
      <div>
        ...
        <div class="mb-5">
          <a-input :value="no" class="w-1/4 h-10 rounded-lg text-center mr-3" disabled placeholder="团队号"></a-input>
          <a-input v-model:value="name" class="w-2/3 h-10 rounded-lg text-center" placeholder="团队名"></a-input>
        </div>
        <div class="mb-5">
          <strong class="text-white">注意：团队号为有限资源，一旦选定，终身有效，除非解散小队，</strong>
        </div>
        <a-row :gutter="10">
          <a-col :span="4">
            <div
              class="rounded-lg h-10 w-4/5 flex justify-center items-center mt-4 pointer text-white"
          ...
          <a-col :span="20">
            <div
              ...
              style="background: rgb(41, 109, 241)"
              @click="doCreate"
            >
              立即创建
            </div>
...
const names =
  '发际线跟我作对,汤臣一品业组,我们做的都队,卷死其他组,精神小伙成双队,清华在校生组,素质教育漏网之鱼队,裸考过线组,下岗工人再就业队,光宗耀组,铁道游击队,没有这组,好好好你说的都队,花开富贵组,蒙的全队,所以爱会消失对不队,治安大队,校外施工队,斗地组,六神五组,重案六组,基因重组';
...
export default {
  ...
  methods: {
    ...
    async doCreate() {
      ...
      this.create(this.name, this.no, this.avatar)
        .then((rs) => {
          if (rs.code === 1000) {
            if (rs.data.status) {

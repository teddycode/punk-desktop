  ...
  <div class="flex-col" style="max-width: 100%; width: 360px">
    <div class="flex items-center my-3 mx-3">
      <div class="pt-2 ml-2" style="width: 70px; height: 70px; position: relative; text-align: center">
        ...
        <a-tooltip
          v-if="displayUserInfo.certification && displayUserInfo.certification.length > 0"
          :title="displayUserInfo.certification[0].name"
        >
          <a-avatar
            :src="displayUserInfo.certification[0].attestation_icon"
            style="position: absolute; width: 20px; height: 20px; right: -15px; bottom: 0; z-index: 999"
          ></a-avatar>
        </a-tooltip>
      </div>
      <div class="flex flex-col ml-2" style="flex: 1">
        <div class="mb-1 xt-text font-bold truncate">
        ...
        <a-dropdown overlayClassName="xt-text " overlayStyle="background:var(--primary-bg-solid)">
          ...
          <template #overlay>
            <a-menu style="background: none" @click="handleMenuClick">
              ...
              >
                <icon class="mr-1" icon="chengyuan" style="font-size: 16px"></icon>
                拉黑
              </a-menu-item>
              <a-menu-item
    ...
    <div class="flex flex-col mb-4">
      ...
      >
        个性签名：
        <div class="px-2" style="word-break: break-all; white-space: pre-wrap">
    ...
    <div class="flex flex-col">
      <div class="mb-4 pd-0 m-3 p-1 mx-5 mt-0" style="color: var(--primary-text)">成就勋章</div>
      <div
    ...
    <div v-if="uid !== myUserInfo.uid" v-show="relationship !== 'unload'" class="px-5 pb-4">
      <a-row v-if="relationship !== 'unknown'" :gutter="12">
        <a-col :span="12">
          <XtButton v-if="relationship === 'not'" class="rounded-full w-full" style="width: 100%" @click="addFriend">
        ...
        <a-col :span="12">
          <XtButton :type="relationship === 'yes' ? 'theme' : 'default'" style="width: 100%" @click="sendMessage"
            >发消息
          </XtButton>
        </a-col>
      </a-row>
...
export default {
  ...
  methods: {
    ...
    addToBlacklist() {
      let promise = window.$chat.addToBlacklist({

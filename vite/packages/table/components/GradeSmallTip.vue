<template>
  <div v-if="blocking" class="grade-tip small-tip" style="background:#52C41A;">{{ tipTitle }}</div>
  <div v-else class="absolute inset-0" style="border-radius: 6px;background: rgba(42, 42, 42, 0.6);"
       @click.stop="clickTip">
    <div class="grade-tip small-tip" style="color: rgba(255, 255, 255, 0.85);">{{ tipTitle }}</div>
  </div>
  <a-modal v-model:visible="clickTipShow" :bodyStyle="{ padding: 0 }" :centered="true" :closable="false" :footer="null"
           :maskClosable="false"
           style="font-size: 8px;" title="" @ok="() => { }">
    <div class="flex flex-col items-center rounded-lg h-56 w-full justify-evenly text-sm text-white "
      ...
      <div v-else-if="lastPowerType === ''">当前「 等级{{ lv }} 」，解锁{{ powerAlias }}功能需要达到「 等级 {{ powerLv }}
        」
      </div>
...
export default {
  ...
  mounted() {
    ...
    const {
      ...
      powerAlias,
      superiorLimit
    } = this.powerState(this.powerType, lv, this.lastPowerType)

<template>
      ...
      >
        <div class="">
          ...
          <div class="w-full mt-4 mb-3">
            <div class="mt-4 mb-3 font-16 xt-text">重复</div>
            <RadioTab
    ...
    >
      ...
      >
        <div class="">
          ...
          <div>
            <div
              class="w-full font-16 xt-text h-[40px] text-center xt-bg mt-2 rounded-lg pointer"
            ...
            <div
              class="w-full font-16 xt-text h-[40px] text-center xt-bg mt-3 rounded-lg pointer"
              style="line-height: 40px"
              @click="onCountDown(5)"
            >
              5分钟
        ...
        >
          <div>
            <clockIcon
              ...
              class="xt-text clock-icon pointer"
            ...
            <clockIcon
              class="ml-2 xt-text clock-icon pointer"
              icon="fluent:dismiss-16-filled"
              style="font-size: 24px; vertical-align: sub"
          ...
          <div>
            ...
            <a-tooltip autoAdjustOverflow title="设置定时器">
              <button class="mr-2 xt-bg-2" style="border: none; width: 25px">
                <clockIcon class="xt-text font-20 clock-icon" icon="fluent:clock-12-regular" @click="settingCountDown">
                </clockIcon>
              </button>
            </a-tooltip>
      ...
      >
        <div class="pl-4 pr-4">
          <div
            ...
            class="flex items-center mt-3 overflow-hidden"
            @contextmenu="deleteClock(index)"
          >
            <div
              class="flex items-center rounded-lg event-list"
        ...
        <div style="display: flex; flex-direction: column; align-items: center">
          ...
          <a-space :popupStyle="{ zIndex: 9999999999999 }" direction="vertical" style="margin: 14px">
            <a-time-picker
              ...
              :popupStyle="{ zIndex: 9999999999999 }"
              :showNow="false"
              popupClassName="popupClass"
              size="large"
            />
          </a-space>
          <xt-button style="margin: 14px; background: var(--active-bg)" type="primary" @click="addCustom"
            >开始倒计时
          </xt-button>
        </div>
      </a-modal>
  ...
  <div style="position: relative">
    <xt-button
      v-if="useCountDownStore.countDowntime.hours !== undefined"
      :style="{
        background: `linear-gradient(to-right, var(--secondary-bg) ${
          100 - useCountDownStore.progress
        }%, var(--warning) ${useCountDownStore.progress}%)  `,
      }"
    ...
    <xt-button
      v-else
      class="flex items-center justify-center mr-3 rounded-md clock-timer top-bar"
      style="width: 82px; height: 32px; background: rgba(80, 139, 254, 0.2)"
      @click="closeDetail"
    >
      <div class="flex items-center">
        <clockIcon class="mr-1 text-base" icon="fluent:clock-alarm-16-filled"></clockIcon>
        <!-- <div class="mr-2 xt-text font-14">闹钟</div> -->
        <div v-if="firstClockTime?.hours !== undefined" class="xt-text font-14">
...
onMounted(() => {
  if (
    useCountDownStore.countDowntime.seconds == '00' &&
    useCountDownStore.countDowntime.minutes == '00' &&
    useCountDownStore.countDowntime.hours == '00'
  ) {
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    if (
      useTimerStore.appDate.minutes === firstClockTime.value?.minutes &&
      useTimerStore.appDate.hours === firstClockTime.value?.hours &&
      clockEvent.value[0].flag === undefined
    ) {
      const key = `open${Date.now()}`;

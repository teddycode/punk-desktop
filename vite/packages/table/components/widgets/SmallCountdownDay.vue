  ...
  <a-drawer v-model:visible="settingVisible" :width="500" placement="right">
    ...
    <div>
      ...
      <div class="flex flex-row items-center w-full justify-center mt-4">
        <div class="rounded-lg h-10 w-24 flex justify-center items-center mr-4 pointer"
             style="background:var(--primary-bg) ;color: var(--primary-text);" @click="closeSettingVisible">取消
        </div>
        <div class="rounded-lg h-10 w-24 flex justify-center items-center pointer"
             style="background:var(--primary-bg) ;color: var(--primary-text);" @click="addEvent">确定添加
        </div>
      </div>
    </div>
  </a-drawer>

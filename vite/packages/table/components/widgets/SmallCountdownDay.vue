<template>
  <Widget :customIndex="customIndex" :desk="desk" :options="options">
    <div v-if="myCountdownDay.length <= 0" class="content">
      <div style="text-align: center">暂无纪念日</div>
      <a-empty :description="null" :image="simpleImage" style="margin: 15px" />
      <div class="item-content">
        <xt-button :h="40" :w="100" size="mini" @click="onSetup">立即添加</xt-button>
      </div>
    </div>
    <div v-else class="content">
      <div style="text-align: center; display: flex; font-size: 1.2em; max-width: 80%; margin: 0 auto">
        距离「
        <span class="text-more" style="flex: 1">{{ myCountdownDay[0].eventValue }}</span
        >」<span v-if="myCountdownDay[0].type">还有</span>
        <span v-else>已过</span>
      </div>
      <div style="font-size: 3em; margin-top: 0.1em">{{ differenceDay }}天</div>
      <div style="font-size: 1.5em; margin-top: 0.5em">
        {{ myCountdownDay[0].dateValue.year }}年{{ myCountdownDay[0].dateValue.month }}月{{
  ...
  <a-drawer v-model:open="settingVisible" :width="500" placement="right">
    ...
    <div>
      <div>事件名称</div>
      <a-input v-model:value="eventValue" class="rounded-lg h-10 mt-4 xt-text" placeholder="请输入" />
      ...
      <div class="flex flex-row items-center w-full justify-center mt-4">
        <div
          class="rounded-lg h-10 w-24 flex justify-center items-center mr-4 pointer"
          style="background: var(--primary-bg); color: var(--primary-text)"
          @click="closeSettingVisible"
        >
          取消
        </div>
        <div
          class="rounded-lg h-10 w-24 flex justify-center items-center pointer"
          style="background: var(--primary-bg); color: var(--primary-text)"
          @click="addEvent"
        >
          确定添加

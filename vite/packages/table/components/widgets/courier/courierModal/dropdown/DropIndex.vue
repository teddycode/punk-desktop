    ...
    <template #overlay>
      <a-menu class="flex flex-col xt-bg-2" style="width: 200px; border-radius: 8px; padding: 8px">
        <a-menu-item
          ...
          class="flex flex-col w-full rounded-lg xt-text nav-item"
          @click.prevent="item.callBack"
        >
          <div class="flex" v-if="item.name === 'tb'">
            <div
              class="flex justify-center w-5 h-5 rounded-md xt-font font-14 font-500 items-enter"
              style="background: #fa5000; line-height: 20px; color: var(--active-text)"
            >
              淘
            </div>
            <span
              class="ml-3"
              :style="
                storeInfo.tb.nickname !== null
                  ? { color: 'var(--secondary-text) !important' }
                  : { color: 'var(--primary-text) !important' }
              "
              >{{ item.title }}</span
            >
            <span class="ml-4" v-if="storeInfo.tb.nickname !== null" style="color: var(--secondary-text) !important"
              >已关联</span
            >
          ...
          <div class="flex" v-if="item.name === 'jd'">
            <div
              class="flex justify-center w-5 h-5 rounded-md xt-font font-14 font-500 items-enter"
              style="background: #e12419; line-height: 20px; color: var(--active-text)"
            >
              JD
            </div>
            <span
              class="ml-3"
              :style="
                storeInfo.jd.nickname !== null
                  ? { color: 'var(--secondary-text) !important' }
                  : { color: 'var(--primary-text) !important' }
              "
              >{{ item.title }}</span
            >
            <span class="ml-4" v-if="storeInfo.jd.nickname !== null" style="color: var(--secondary-text) !important"
              >已关联</span
            >

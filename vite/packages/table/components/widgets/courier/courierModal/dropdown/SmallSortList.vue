      ...
      <div :style="settings.tagVisible ? { height: '380px' } : { height: '450px' }" ref="dropRef">
        <div v-for="(item, index) in list" class="rounded-lg">
          <xt-menu name="name" @contextmenu="revID = { item, index }" :menus="menus">
            <div
              :class="{ select: sortItem === index }"
              class="flex p-3 mb-3 flex-col rounded-lg xt-text pointer xt-bg-2 courier-item"
              @click="sortDetail(item, index)"
            >
              <div class="flex">
                <div
                  class="flex items-center justify-center mr-4 rounded-lg"
                  style="width: 52px; height: 52px; background: var(--mask-bg)"
                >
                ...
                <div class="flex items-center justify-between" style="width: 362px">
                  <div class="flex flex-col">
                    <div class="flex items-center mb-1.5">
                      ...
                      <SmallIcon
                        ...
                        v-else
                        style="color: var(--warning); font-size: 1.25rem"
                      />
                    </div>
                    <div class="flex">
                      ...
                      <div v-if="false" class="flex items-center justify-center rounded-md w-6 h-6 xt-text-2 xt-bg">
                        拆
                      </div>
                    </div>
                  </div>
                  <div class="flex flex-col items-center justify-end">
                    <div
                      class="flex items-center ml-8 mb-2 pl-1 pr-1 rounded-md"
                      :style="{ background: stateColor(item, index), color: 'var(--active-text)' }"
                    >
                      {{ switchState(item) }}
                    </div>
              ...
              <div class="flex flex-col">
                <div class="my-1.5 font-14 font-400 xt-text-2">
                  {{ item.Traces[item.Traces.length - 1]?.AcceptTime }}
                </div>
                <div class="summary">

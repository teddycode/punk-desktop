<template>
  <div class="flex flex-col px-6 xt-text">
    <div class="w-full p-4 mb-3 rounded-lg xt-bg-2">
      <div class="flex items-center justify-between h-8 mb-4">
        <div class="flex items-center">
          <div
            class="flex justify-center w-8 h-8 rounded-lg xt-font font-19 font-500 items-enter"
            style="background: #e12419; line-height: 32px; color: var(--active-text)"
          >
            JD
          </div>
        ...
        <xt-button
          w="32"
          v-if="storeInfo.jd.nickname === null"
          style="color: var(--active-bg) !important"
          @click="bindJd"
          >关联</xt-button
        >
      ...
      <div class="flex items-center justify-between h-8 mt-4">
        <div class="flex items-center">
          <div
            class="flex justify-center w-8 h-8 rounded-lg xt-font font-19 font-500 items-enter"
            style="background: #fa5000; line-height: 32px; color: var(--active-text)"
          >
            淘
          </div>
        ...
        <xt-button
          w="32"
          v-if="storeInfo.tb.nickname === null"
          style="color: var(--active-bg) !important"
          @click="bindTb"
          >关联</xt-button
        >
    ...
    <div class="flex flex-col w-full p-4 mb-4 rounded-lg xt-bg-2 items-enter">
      ...
      <a-select
        :bordered="false"
        class="rounded-lg xt-bg"
        v-model:value="settings.courierRefresh.autoTime"
        :options="autoRefreshTime"
      >
      </a-select>

      <span class="mt-4 xt-text-2 xt-font font-14 font-400"
        >仅对电商平台快递订单有效。（自定义添加的快递订单默认会定期自动更新，不需要设置）</span
      >

      <xt-button @click="clear" :radius="6" type="error" size="mini" :h="34" class="mt-2" style="width: 100%">
        清空全部数据</xt-button
      >
    </div>
    ...
    <div class="flex flex-col w-full p-4 rounded-lg xt-bg-2 items-enter">
      ...
      <div class="flex flex-col">
        ...
        <a-select
          ...
          v-model:value="settings.courierStatus.currentStatus"
        ></a-select>
      </div>
      ...
      <div class="flex flex-col">
        ...
        <a-select
          class="rounded-lg xt-bg"
          :bordered="false"
          :options="autoCancelTime"
          v-model:value="settings.courierSigned.courierTime"
        ></a-select>

        <span class="xt-text-2 mt-2.5 xt-font font-14 font-400"
          >已签收的订单到达以上时间后自动取消关注该订单，并设置为隐藏。</span
        >
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      statusSelect: [
        { value: 'unFinished', label: '未完成订单' },
        { value: 'followed', label: '关注的订单' },
      ],
      autoRefreshTime,
      autoCancelTime,
      dealType: '',

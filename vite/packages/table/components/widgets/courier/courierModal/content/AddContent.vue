<template>
  <div class="flex flex-col" style="width: 500px; height: 500px">
    <div class="flex items-center justify-center w-full py-5" style="position: relative">
      ...
      <div
        class="flex items-center justify-center p-2 close-button pointer category-button rounded-xl xt-bg-2"
        @click="close"
      >
    ...
    <div class="flex-col px-6" style="height: calc(100% - 131px)">
      <div class="px-4 py-3 mb-4 font-normal rounded-lg font-14 xt-text-2 xt-bg-2">
        「顺丰快递」和「菜鸟橙运」需要填写额外信息。
      </div>
      <vue-custom-scrollbar :settings="settingsScroller" class="h-full" style="height: 79%">
        <div class="flex items-center mb-4" v-for="(item, index) in courierLists">
          <a-input
            ref="numRef"
            spellcheck="false"
            style="width: 280px; margin-right: 12px; border-radius: 8px"
            v-model:value="item.orderNum"
            class="h-10 xt-bg-2"
            :style="item.code === 'SF' ? { width: '148px !important' } : { width: '280px !important' }"
            placeholder="输入快递单号"
            @input="getCourierNumber(item.orderNum, index)"
            allowClear
          />

          <a-input
            v-model:value="item.phoneLastNum"
            v-if="item.code === 'SF' || item.code === 'CNCY'"
            style="width: 120px; border-radius: 8px; margin-right: 12px"
            class="h-10 xt-bg-2"
            placeholder="手机尾号后4位"
          />
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const optionList = computed(() => {
      const list = expressList.map((item) => {
    ...
    const isOrderNumEmpty = computed(() => {
      return courierLists.value.some((item) => {
    ...
    const getCourierNumber = (item, index) => {
      if (item !== '') {
        ...
        courier.updateNewCourierInfo(
          { code: result.code, label: result.name, orderNum: item, phoneLastNum: '' },
          index,
        );
      } else {
    ...
    const addCourierData = (evt) => {
      if (courierLists.value.length === 1) {
        if (courierLists.value[0].orderNum === '') {
          ...
          addCourier.putCourierInfo(
            courierLists.value[0].code,
            courierLists.value[0].orderNum,
            courierLists.value[0].phoneLastNum,
          );

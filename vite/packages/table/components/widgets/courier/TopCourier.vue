<template>
    ...
    <div class="w-[572px] h-[420px] rounded-xl p-4" style="background-color: var(--secondary-bg)">
      <div class="flex justify-between h-[32px]">
        ...
        <div class="flex">
          <DropIndex
            :navList="addList"
            dropStyle="var(--primary-bg) !important"
            mClass="mr-2"
            @open="addCourier"
          ></DropIndex>
            ...
            >
              <div class="flex items-center justify-center">
                <newIcon
                  ...
                  style="vertical-align: sub; font-size: 1.25rem"
                ></newIcon>
              </div>
            </xt-button>
          </a-tooltip>
  ...
  <xt-button
    :w="60"
    :h="27"
    v-if="this.settings.courierStatus.statusBar"
    style="
  ...
  <teleport to="body">
    <xt-old-modal
      v-if="showCourierDetail"
      v-model:visible="showCourierDetail"
      title=""
      :isFooter="false"
      zIndex="9"
      :isHeader="false"
...
export default {
  ...
  methods: {
    ...
    backAllCoutiers() {
      (this.showSmallDetail = true), (this.showCourierDetail = false);
    },
    smallDetailsVisible() {
  ...
  computed: {
    ...
    config() {
      return {
        ...
        jdLength:
          this.storeInfo?.jd?.order?.orders?.length !== undefined ? this.storeInfo?.jd?.order?.orders?.length : 0,
        tbLength:
          this.storeInfo?.tb?.order?.orders?.length !== undefined ? this.storeInfo?.tb?.order?.orders?.length : 0,
      };
    },
    filterType() {

<template>
  <Widget :customData="customData" :customIndex="customIndex" :desk="desk" :options="options">
    ...
    <div class="w-full flex mt-3.5 mb-3">
      <HorizontalPanel v-model:selectType="strategy" :navList="strategyType" style="width: 23.5rem; height: 44px">
      </HorizontalPanel>
      <xt-new-icon
        bgClass="xt-bg  category-button mx-3"
      ...
      <template v-if="strategy.name === 'video'">
        <xt-new-icon
          v-if="biliVideoList.length !== 0"
          :style="
            isTop
              ? { color: 'var(--disable-text) !important', cursor: 'not-allowed !important' }
              : { color: 'var(--primary-text) !important' }
          "
          bgClass="xt-bg  category-button mr-3"

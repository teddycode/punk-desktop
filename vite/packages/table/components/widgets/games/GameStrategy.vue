<template>
  <Widget :customData="customData" :customIndex="customIndex" :desk="desk" :options="options">
    <div class="flex items-center" style="position: absolute; top: 0.85rem; left:1rem;">
      ...
      <div
          class="flex items-center justify-center category-button ml-2 py-0.5 px-2 xt-divider-bg xt-text rounded-lg pointer"
    ...
    <div class="w-full flex mt-3.5 mb-3">
      <HorizontalPanel v-model:selectType="strategy"
                       :navList="strategyType" style="width: 23.5rem;height: 44px;">
      </HorizontalPanel>
      <xt-new-icon bgClass="xt-bg  category-button mx-3" icon="fluent:search-20-filled" type="theme" w="44"
                   @click="openSearch"></xt-new-icon>
  ...
  <HorizontalDrawer ref="strategyDrawer" v-model:selectRegion="customData.id"
                    :rightSelect="recentGameList"
                    drawerTitle="游戏名称" @getArea="getArea"
  >
  </HorizontalDrawer>

  <a-drawer v-model:visible="searchShow" placement="right" title="搜索" width="500">
    <div class="line">
      <a-input v-model:value="searchWord" class="w-full h-10 no-drag" placeholder="搜索" spellcheck="false"
               style="border-radius: 12px;background: var(--primary-bg); color:var(--primary-text);"

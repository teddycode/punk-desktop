<template>
  <Widget :custom-data="customData" :customIndex="customIndex" :options="options">
    <div v-if="isLoading" class="example">
      <a-spin />
    </div>
    <div v-else>
      <div v-if="pageToggle" class="flex" style="margin-top: 13px; color: var(--primary-text)">
        <div v-if="fatherWidth == 2" class="cursor-pointer" style="width: 240px; height: 354px; margin: 0 16px 0 5px">
          <a-image
            :src="detailMovie.img"
            alt=""
            class="rounded-lg"
        ...
        <div :class="fatherWidth == 2 ? 'size-max' : 'size-min'">
          <div style="height: 304px">
            <div :class="fatherWidth == 2 ? 'size-max' : 'size-min'" class="film-content">
              <div
                style="
              ...
              <div>
                主演：<span>{{ detailMovie.members }}</span>
              </div>
              <div>
                导演：<span>{{ detailMovie.dir }}</span>
              </div>
              <div>
                类型：<span>{{ detailMovie.filmType }}</span>
              </div>
              <div>
                制片国家/地区: <span>{{ detailMovie.src }}</span>
              </div>
              <div>
                语言：<span>{{ detailMovie.language }}</span>
              </div>
              <div>
                上映：<span>{{ detailMovie.pubDesc }}</span>
              </div>
            </div>
            <div
              v-if="detailMovie.sc"
              :class="fatherWidth == 2 ? 'size-max' : 'size-min'"
              class="items-center s-item mt-2 mb-3 rounded-lg pl-4"
              style="display: flex; height: 55px"
            >
              <span v-if="fatherWidth == 2" class="mr-4" style="font-size: 16px; color: var(--secondary-text)"
                >猫眼</span
              >
              <span
                style="font-size: 32px; color: #ffffff; letter-spacing: 4px; font-weight: 500; margin-right: 11px"
                >{{ detailMovie.score }}</span
              >
...
export default {
  ...
  methods: {
    ...
    async getfilmDetail() {
      let detail = await cacheRequest(
        `https://m.maoyan.com/ajax/detailmovie?movieId=${this.detailId}`,
        {},
        {
          localCache: true,
          localTtl: 60 * 60 * 12,

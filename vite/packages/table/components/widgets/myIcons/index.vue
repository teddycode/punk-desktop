...
<template>
  ...
  <Widget
    ...
    :options="options"
  >
    <div ref="iconRef" :style="dragStyle" class="icon-box box-border">
  ...
  <XtDrawer v-model="iconsSetVisible" placement="right">
    <div class="xt-bg-2 p-4 rounded-xl">
      ...
      <div class="xt-text-2 text-sm">该模式下可以自由拖动文件夹大小，直接点击打开文件夹内的应用或连接。</div>
    </div>
  </XtDrawer>
  <!-- 底部导航 -->
  <XtDrawer v-model="menuVisible">
    <BottomEdit :copy="customData.copy" :menuList="menuList" :merge="customData.merge"></BottomEdit>
  </XtDrawer>
</template>
<script>
// components
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      Object.keys(this.edit).forEach((k) => (this.customData.iconList[this.index][k] = this.edit[k]));

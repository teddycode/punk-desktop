<template>
  ...
  <div class="relative items-center">
    ...
    >
      <img v-if="edit.src" :src="renderIcon(edit.src)" alt="avatar" style="width: 100px; height: 100px" />
      <div
        v-if="edit.src"
        class="xt-bg-2 flex justify-center items-center xt-hover absolute"
        style="border-radius: 50%; padding: 5px; cursor: pointer; left: 90px; top: -10px"
        @click.stop="clear()"
      >
    ...
    <div class="absolute" style="top: 50%; transform: translateY(-50%); left: 120px">
      <div class="xt-text-2" style="font-size: 16px">推荐图片尺寸：128*128，不能超过2MB</div>
      <XtButton class="btn no-drag xt-bg-2 mt-3" @click="imgUp()">自定义上传 </XtButton>
      <input ref="imgRef" accept="image/jpg,image/jpeg,image/png" style="display: none" type="file" />
    </div>
  </div>
</template>

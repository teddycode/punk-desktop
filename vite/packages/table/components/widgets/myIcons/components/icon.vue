<template>
  <div :data-index="index" :style="[iconSize]"
       class="cursor-pointer rounded-xl xt-hover black xt-base-btn flex-col justify-around" @click.stop="iconClick($event)">
    <div :data-index="index"
         :style="[bgSize, backgroundState]" class="xt-text overflow-hidden no-drag flex items-center justify-center rounded-xl w-full">
      <img v-if="src && src.length > 0" :data-index="index" :src="renderIcon(src)" :style="[imgSize, radiusState, imgStateStyle]"
           alt=""/>
    </div>

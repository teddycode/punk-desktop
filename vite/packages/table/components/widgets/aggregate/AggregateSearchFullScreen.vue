<template>
  <div class="flex flex-col">
    <div class="flex p-4">
      <div class="flex flex-col mr-4">
        ...
        <vue-custom-scrollbar :settings="settingsScroller" class="left-tab-container" style="max-height:400px;">
          <div v-for="(item,index) in list" :key="index" :class="{'active-bg': selectIndex === index}"
               class="flex items-center pointer rounded-lg mb-3 py-3 px-3" @click="selectAggSearch(item,index)"
          >
      ...
      <div class="flex flex-col ">
        <div class="flex items-center justify-center rounded-lg h-12 pl-3 pr-1  mb-5"
          ...
          <a-input ref="searchRef" v-model:value="searchKeyWords" :bordered="false" allowClear class="search"
                   placeholder="搜索" spellcheck="false" @input="dataSearch" @pressEnter.stop="enterSearch"></a-input>
          <a-dropdown v-model:visible="isDropdownVisible" placement="bottomRight" trigger="click">
            <div class="flex pointer items-center justify-center active-button p-2 rounded-lg"
            ...
            <template #overlay>
              <a-menu class="custom-dropdown-menu flex items-center flex-col justify-center">
                <a-menu-item v-for="(item,index) in linkType"
                             :key="index" class="rounded-lg"
                             style="color: var(--primary-text);" @click="changeOpenType(item,index)"
            ...
            >
              ...
              <div v-else-if="suggestion.value" class="flex">
                <span class="ping-title" style="color: var(--secondary-text);"
                      v-html="matchingKey(suggestion.value)"></span>
              </div>

              <!-- 豆瓣、微博搜索 -->
              <div v-else-if="suggestion.suggestion">
                <span class="ping-title" style="color: var(--secondary-text);"
                      v-html="matchingKey(suggestion.suggestion)"></span>
              </div>

              <!-- github、优酷搜索 -->
              <div v-else-if="suggestion.name">
                <span class="ping-title" style="color: var(--secondary-text);"
                      v-html="matchingKey(suggestion.name) "></span>
              </div>

              <!-- 知乎搜索 -->
              <div v-else-if="suggestion.query">
                <span class="ping-title" style="color: var(--secondary-text);"
                      v-html="matchingKey(suggestion.query)"></span>
              </div>
          ...
          <div v-else class="flex flex-col">
            <template v-if="getClipBoardData.length !== 0">
              <div class="flex flex-col" style="width:483px;">
                ...
                <div v-for="(item,index) in getClipBoardData" :class="{'active-bg':clipboardIndex === index}"
                     class="flex primary-title rounded-lg flex-col p-3 pointer" style="color: var(--secondary-text);"
    ...
    <div class="rounded-b-lg flex items-center h-12 px-3 py-3" style="background: var(--secondary-bg);">
      <div class="px-4 py-2.5 w-12 h-7 flex items-center justify-center primary-title rounded-lg mr-2 search-tag"
           style="color: rgba(0, 0, 0, 0.65);">Tab
      </div>
      ...
      <div class="px-4 py-2.5 w-12 h-7 flex items-center justify-center primary-title rounded-lg mx-2 search-tag"
           style="color: rgba(0, 0, 0, 0.65);">Ctrl
      </div>
      <div class="px-4 py-2.5 w-12 h-7 flex items-center justify-center primary-title rounded-lg mr-2 search-tag"
           style="color: rgba(0, 0, 0, 0.65);">Tab
      </div>

<template>
  <div class="flex flex-col">
    <div class="flex p-4">
      <div class="flex flex-col mr-4">
        ...
        <vue-custom-scrollbar :settings="settingsScroller" class="left-tab-container" style="max-height: 400px">
          <div
            ...
            :key="index"
            :class="{ 'active-bg': selectIndex === index }"
            class="flex items-center pointer rounded-lg mb-3 py-3 px-3"
      ...
      <div class="flex flex-col">
        <div
          class="flex items-center justify-center rounded-lg h-12 pl-3 pr-1 mb-5"
          style="border: 1px solid var(--divider); width: 480px; background: var(--secondary-bg)"
        >
          <div class="flex items-center justify-center" style="width: 20px; height: 20px">
          ...
          <a-input
            ...
            spellcheck="false"
            @input="dataSearch"
            @pressEnter.stop="enterSearch"
          ></a-input>
          <a-dropdown v-model:visible="isDropdownVisible" placement="bottomRight" trigger="click">
            <div
              class="flex pointer items-center justify-center active-button p-2 rounded-lg"
              style="width: 210px; background: var(--active-secondary-bg)"
            >
            ...
            >
              ...
              <div v-else-if="suggestion.value" class="flex">
                <span
                  ...
                  v-html="matchingKey(suggestion.value)"
                ></span>
              </div>
              ...
              <div v-else-if="suggestion.suggestion">
                <span
                  class="ping-title"
                  style="color: var(--secondary-text)"
                  v-html="matchingKey(suggestion.suggestion)"
                ></span>
              </div>
              ...
              <div v-else-if="suggestion.name">
                <span
                  class="ping-title"
                  style="color: var(--secondary-text)"
                  v-html="matchingKey(suggestion.name)"
                ></span>
              </div>
              ...
              <div v-else-if="suggestion.query">
                <span
                  class="ping-title"
                  style="color: var(--secondary-text)"
                  v-html="matchingKey(suggestion.query)"
                ></span>
              </div>
          ...
          <div v-else class="flex flex-col">
            <template v-if="getClipBoardData.length !== 0">
              <div class="flex flex-col" style="width: 483px">
                ...
                <div
                  v-for="(item, index) in getClipBoardData"
                  :class="{ 'active-bg': clipboardIndex === index }"
                  class="flex primary-title rounded-lg flex-col p-3 pointer"
                  style="color: var(--secondary-text)"
                  @click="selectClipboardItem(item, index)"
                >
                  {{ item }}
                </div>
              </div>
    ...
    <div class="rounded-b-lg flex items-center h-12 px-3 py-3" style="background: var(--secondary-bg)">
      <div
        class="px-4 py-2.5 w-12 h-7 flex items-center justify-center primary-title rounded-lg mr-2 search-tag"
      ...
      <div
        class="px-4 py-2.5 w-12 h-7 flex items-center justify-center primary-title rounded-lg mx-2 search-tag"
        style="color: rgba(0, 0, 0, 0.65)"
      >
        Ctrl
      </div>
...
export default {
  ...
  methods: {
    ...
    getDataFields(res) {
      if (this.list[this.selectIndex].id === 3 && res.data[1]) {
        // 必应搜索建议
        this.searchSuggestionList = res.data[1];
      } else if (this.list[this.selectIndex].id === 1 && res.data.g) {
        // 百度搜索建议
        this.searchSuggestionList = res.data.g;
      } else if (this.list[this.selectIndex].id === 4 && res.data.suggest) {
        // 知乎搜索建议
        this.searchSuggestionList = res.data.suggest;
      } else if (this.list[this.selectIndex].id === 5 && res.data.items) {
        // github搜索建议
        this.searchSuggestionList = res.data.items;
      } else if (this.list[this.selectIndex].id === 6 && res.data) {
        // bilibili搜索建议
        this.searchSuggestionList = res.data;
      } else if (this.list[this.selectIndex].id === 8 && res.data.data) {
        // 优酷搜索建议
        this.searchSuggestionList = res.data.data;
      } else if (this.list[this.selectIndex].id === 9 && res.data.words) {
        // 豆瓣搜索建议
    ...
    openSearchSuggest(words) {
      // 回车或者点击其他后根据不同打开方式类型进行打开

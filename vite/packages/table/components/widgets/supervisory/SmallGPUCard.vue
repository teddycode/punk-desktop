      ...
      <div class="right-content">
        <div class="cpu">
          <div class="cpu-number">
            <span>温度</span>
            <span style="font-weight: 700">{{ GPUData.warmGPU.value }}℃</span>
          </div>
        </div>
        <a-progress
          :percent="GPUData.warmGPU.value"
          :showInfo="false"
          :status="GPUData.warmGPU.value === 0 ? '' : 'active'"
          :stroke-color="{
            '0%': '#60BFFF',
            '100%': '#348FFF',
          }"
        ...
        <div class="cpu" style="margin-top: 13px">
          <div class="cpu-number">
            <span>显存</span>
            <span style="font-weight: 700">{{ GPUStorage }}GB</span>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    GPUStorage() {
      return this.GPUData.videoStorage.value > 0
        ? (this.GPUData.videoStorage.value / 1000).toFixed(2)
        : this.GPUData.videoStorage.value;
    },
  },
  mounted() {
    this.startInspect();

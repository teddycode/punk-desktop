<template>
  <div class="xt-bg-2 rounded-xl p-3 mb-3 text-base">
    ...
    <template v-if="freeLayoutSwitch">
      ...
      <div class="xt-text-2 text-sm my-3">开启后组件不可以叠加摆放，通过右键菜单层叠位置来设置层级</div>
      <hr class="my-3" />
      <div class="flex justify-between mb-3">
        <div>卡片吸附</div>
        <a-switch v-model:checked="getFreeLayoutState.option.magnet" />
      </div>
      <div class="xt-text-2 text-sm my-3">开启后会在组件拖拽时自动吸附到组件边缘</div>
      <hr class="my-3" />
      <div class="flex justify-between mb-3">
        <div>基于网格移动</div>
      ...
      <div class="flex mb-3 justify-between">
        <xt-button class="mr-3 xt-bg" w="204" h="40" @click="freeDeskResize()"> 回到中心点</xt-button>
      ...
      <div class="xt-text-2 text-sm my-3">拖动小组件进行排列时自动对齐到网格。</div>
      <hr class="my-3" />
      <div class="flex justify-between mb-3">
        <div>无限画布</div>
      ...
      <div class="flex mb-3 justify-between">
        <xt-button class="xt-bg" w="204" h="40" :copy="copyData"> 分享自由布局</xt-button>
...
// 缩放比例
const zoom = ref(getFreeLayoutState?.value ? getFreeLayoutState?.value.canvas.zoom * 100 : 100);
watch(zoom, (newV) => {
...
watch(stack, (newV) => {
  getFreeLayoutState.value.option.collision = !getFreeLayoutState.value.option.collision;
});

watch(
  () => [getFreeLayoutState.value?.canvas.width, getFreeLayoutState.value?.canvas.height],
  (newV) => {
    debounceScrollbarUpdate();
  },
);

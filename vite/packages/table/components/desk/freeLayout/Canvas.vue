  ...
  >
    ...
    <template v-if="getFreeLayoutState.line.isCenterLine">
      <div
        ...
        :style="{
          top: getFreeLayoutState.line.centerLine.y / getFreeLayoutState.canvas.zoom + 'px',
        }"
      />
      <div
        ...
        :style="{
          left: getFreeLayoutState.line.centerLine.x / getFreeLayoutState.canvas.zoom + 'px',
        }"
      />
    </template>
    <slot></slot>
...
const { getFreeLayoutData, getFreeLayoutState, getFreeLayoutMargin }: any = storeToRefs(freeLayoutStore);
// 辅助线生成
...
const auxLineWidth = computed(() => {
  return auxLine(getFreeLayoutState.value.canvas.width, 140 - 6 + getFreeLayoutMargin.value * 2);
});

const auxLineHeight = computed(() => {
  return auxLine(getFreeLayoutState.value.canvas.height, 102 - 6 + getFreeLayoutMargin.value * 2);
});
</script>

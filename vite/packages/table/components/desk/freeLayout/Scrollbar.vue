...
<template>
  <div
    ...
    style="padding-right: 10px; padding-bottom: 10px; margin-bottom: 12px; height: 100%"
...
const { getFreeLayoutState, dragData, freeLayoutEnv, getFreeLayoutData, getCurrentDesk } = storeToRefs(freeLayoutStore);
...
function redirect() {
  scrollbar.value.scrollLeft = getFreeLayoutState.value.line.centerLine.x - width.value / 2;
  scrollbar.value.scrollTop = getFreeLayoutState.value.line.centerLine.y - height.value / 2;
  return;
...
function handleMouseMove(event) {
  ...
  if (scrollbar.value && Object.keys(dragData.value).length !== 0) {
    ...
    const { left, top, right, bottom } = scrollbar.value.getBoundingClientRect();

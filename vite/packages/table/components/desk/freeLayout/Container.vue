...
const { freeLayoutData, getCurrentDeskId, getFreeLayoutData, freeLayoutEnv, getFreeLayoutState } =
...
async function getPosition(item) {
  ...
  const { left, top } = childDiv[cards.value.length - 1].getBoundingClientRect();

  const { left: waterfallFlowLeft, top: waterfallFlowTop } = waterfallFlow.value.getBoundingClientRect();
  const scrollTop = freeLayoutEnv.value.scrollTop;
  const scrollLeft = freeLayoutEnv.value.scrollLeft;
  return {

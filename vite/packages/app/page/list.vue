      ...
      <template v-if="this.searchWords === ''" style="position: relative">
        <a-tabs v-model:value="tabIndex" type="card">
                ...
                <div style="width: 20%" v-for="app in appsRecently">
                  <div style="margin-bottom: 15px; text-align: center">
                    <AppItem
                      @uninstall="
                        () => {
                          this.uninstall(app, appsRecently);
                        }
                      "
                      @favUpdated="refresh"
                      :app="app"
          ...
          <a-tab-pane key="fav" tab="收藏">
            <a-empty
              description="暂无收藏，您可在应用上「右键」-「添加为收藏」"
              v-if="this.appsFav.length === 0"
          ...
          <a-tab-pane key="all" tab="全部">
            <div style="position: relative; margin-bottom: 15px">
              <a-select v-model:value="type" size="small" style="width: 80px" :options="options"></a-select>
                ...
                <div style="width: 20%" v-for="app in filteredApps">
                  <div style="margin-bottom: 15px; text-align: center">
                    <AppItem
                      @uninstall="
                        () => {
                          this.uninstall(app, filteredApps);
                        }
                      "
                      @favUpdated="refresh"
                      :app="app"
        ...
        <a-row>
          <a-col :span="4" v-for="app in searchResult">
            <AppItem
              @uninstall="
                () => {
                  this.uninstall(app, searchResult);
                }
              "
              :app="app"
            ></AppItem>

<template>
  ...
  <a-form :model="formState" style="padding: 20px" :label-col="{ span: 5 }" :wrdevApper-col="{ span: 19 }">
    <a-form-item label="作者名称" :rules="{ message: '应用名称，长度大于1，小于16', min: 1, max: 16 }">
      <a-input v-model:value="devApp.author" placeholder="输入作者名称" />
    </a-form-item>
    <a-form-item label="作者官网" :rules="{ message: '应用名称，长度大于1，小于16', min: 1, max: 16 }">
      <a-input v-model:value="devApp.site" placeholder="输入官网地址" />
    </a-form-item>
    <a-form-item label="是否开源：">
      <template #extra>
        <DebugTip tip="os_summary" />
      </template>
      <a-switch v-model:checked="devApp.open_source" />
    </a-form-item>
    <a-form-item v-if="devApp.open_source" label="开源代码地址：">
      <template #extra>
        <DebugTip tip="open_source" />
      </template>
      <a-input v-model:value="devApp.csv_url" />
    </a-form-item>
    <a-form-item label="开源说明" :rules="{ message: '' }">
      <a-textarea v-model:value="devApp.os_summary" placeholder="开源说明" />
      <template #extra>
        <DebugTip tip="os_summary" />
      </template>
    </a-form-item>

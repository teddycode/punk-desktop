<template>
  ...
  >
    ...
    <div style="padding: 20px">
      <a-list class="demo-loadmore-list" item-layout="horizontal" :data-source="devApps">
        <template #renderItem="{ item }">
          <a-list-item>
            <template #actions>
              <a @click="loadDevApp(item)" key="list-loadmore-edit"> <edit-outlined key="edit" /> 进入项目 </a>
              ...
              <a-list-item-meta>
                <template #description>
                  ...
                  <p v-if="item.assignAppsInfo.length > 0">
                    关联的应用：
                    <a-tag v-for="app in item.assignAppsInfo"
                      >{{ app.name }} <span style="color: grey">{{ app.nanoid }}</span></a-tag
                    >
                  </p>
                </template>
                <template #title>
                  <a @click="loadDevApp(item)"
                    >{{ item.name }} <span style="font-size: 12px; color: #999">{{ item.nanoid }}</span></a
                  >
                </template>
                <template #avatar>
    ...
    deleteDevApp(app) {
      Modal.confirm({
        ...
        onOk: () => {
          ...
          this.devApps.splice(
            this.devApps.findIndex((devApp) => devApp === app),
            1,
          );

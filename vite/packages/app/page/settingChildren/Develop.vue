<template>
  <h3>开发者模式
    <span style="float:right;font-size: 14px">当前状态 <span :style="{background:this.debugMod?'green':'red'}"
                                                             style="display: inline-block;border-radius: 100%;width: 10px;height: 10px;"/>&nbsp; </span>
  </h3>
  <div style="padding-bottom:  10px">
    进入开发者模式
    <a-tooltip title="仅非系统应用可开启调试模式">
      <a-switch :checked="debugMod" :disabled="app.isSystemApp && !allowSystemDebug" @change="switchToDevMod"/>
    </a-tooltip>
    <router-link :to="{path:'/allDevapps'}">
      <a-button style="float: right">查看其他开发中的应用</a-button>
    </router-link>
  </div>
  <a-alert v-if="existsDevApp" style="margin-top: 10px">
  ...
  <div style="padding: 10px;box-shadow: 0 0 3px #a4a4a4;border-radius: 4px;margin-top: 20px;background: white">
    ...
    <p>
      如果此应用是您自己开发的应用，可以勾选此选项，启用后设置页面将切换到开发者模式，并载入此应用关联的开发中的应用，同时将为您解锁更多开发者工具，方便您开发和调试应用。</p>

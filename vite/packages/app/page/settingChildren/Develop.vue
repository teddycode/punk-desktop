<template>
  <h3>
    ...
    <span style="float: right; font-size: 14px"
      >当前状态
      <span
        :style="{ background: this.debugMod ? 'green' : 'red' }"
        style="display: inline-block; border-radius: 100%; width: 10px; height: 10px"
      />&nbsp;
    </span>
  </h3>
  <div style="padding-bottom: 10px">
    进入开发者模式
    <a-tooltip title="仅非系统应用可开启调试模式"
      ><a-switch @change="switchToDevMod" :checked="debugMod" :disabled="app.isSystemApp && !allowSystemDebug"
    /></a-tooltip>
    <router-link :to="{ path: '/allDevapps' }"
      ><a-button style="float: right">查看其他开发中的应用</a-button></router-link
    >
  </div>
  <a-alert style="margin-top: 10px" v-if="existsDevApp">
    <template #description>
      当前应用已绑定开发中的应用 「<strong>{{ existsDevApp.name }}（ID：{{ existsDevApp.nanoid }}）</strong
      >」，可开启进入对应开发应用。
    </template>
  </a-alert>
  <div style="padding: 10px; box-shadow: 0 0 3px #a4a4a4; border-radius: 4px; margin-top: 20px; background: white">
    ...
    <p>
      如果此应用是您自己开发的应用，可以勾选此选项，启用后设置页面将切换到开发者模式，并载入此应用关联的开发中的应用，同时将为您解锁更多开发者工具，方便您开发和调试应用。
    </p>

    <h4>开发者模式有哪些额外的功能？</h4>

    <p>
      允许您为应用添加更多的设置，包括修改默认主题色、应用包名、应用标题、应用起始url 允许您使用<span
        @dblclick="enableAllowSystemDebug"
        >开发者工具</span
      >，包括导出应用配置文件，上架应用。
    </p>
  </div>

  ...
  <a-form :model="userSetting" style="padding: 20px" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
    <a-form-item :rules="{ initialValue: ['A', 'B'] }" id="optimize" label="体验优化" style="margin-bottom: 0">
      <a-row class="setting-check-boxes">
        <a-col :span="8">
          <a-checkbox name="autoRun" v-model:checked="userSetting.optimize.autoRun"> 默认启动 </a-checkbox>
        </a-col>
        <a-col :span="8">
          <a-checkbox name="keepRunning" v-model:checked="userSetting.optimize.keepRunning"> 保持运行 </a-checkbox>
        </a-col>
        <!--                        <a-col :span="8">-->

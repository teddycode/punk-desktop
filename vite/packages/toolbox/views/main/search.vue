<template>
  <div style="height: 60px" class="xt-bg-2 xt-text flex items-center">
    <template v-if="useTool">
      <XtIcon @click="back()" class="mx-2" icon="xiangzuo" type="" w="26" h="26" size="24"></XtIcon>
      <div class="xt-active-bg-2 xt-base-btn px-2 w-auto" style="height: 44px">
        <img :src="'/tools/' + useTool.img + '.png'" style="width: 32px; height: 32px" class="mr-2" alt="" />
        <div style="white-space: nowrap">{{ useTool.name }}</div>
      </div>
    </template>

    <XtInput :focus="true" v-model="search" placeholder="试试输入、粘贴、拖放内容到此处" @enter="searchEnter()">
      <template #suffix v-if="search">
        <XtIcon @click="clear()" type="" class="xt-text" icon="guanbi" w="24" h="24" size="24"></XtIcon>
      </template>
    </XtInput>
  </div>
</template>

<template>
  <div class="xt-border h-full xt-bg-2 xt-text text-base flex flex-col">
    ...
    <div class="flex justify-between p-2">
      ...
      <XtBaseIcon icon="shanchu" style="font-size: 20px" @click="delCalculator()"></XtBaseIcon>
      <div class="absolute left-1/2 -translate-x-1/2 res-copy" style="bottom: 22px">点击复制</div>
    </div>
  </div>
</template>
...
export default {
  computed: {
    ...mapWritableState(calculator, ['computeList', 'countList', 'calculators', 'selectIndex']),
  },
  watch: {
    computeList: {
      handler() {
  ...
  methods: {
    ...
    handleBlur(index) {
      if (this.computeList[index] === '' && index !== this.computeList.length - 1) {

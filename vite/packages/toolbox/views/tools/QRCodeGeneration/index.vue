<template>
  <div class="w-full h-full flex">
    <div class="flex-1 flex flex-col mr-3">
      ...
      <div class="flex mb-2">
        <XtButton class="mr-3" style="width: 100%" @click="clear()">清除</XtButton>
        <XtButton type="theme" style="width: 100%" @click="newQRCode()">生成</XtButton>
      </div>
      <div class="flex">
      ...
      <a-slider id="test" v-model:value="size" :min="90" :max="220" :disabled="disabled" />
    </div>
    <div class="flex-1">
      <div v-if="src" class="w-full h-full xt-bg-2 xt-border rounded-xl flex flex-col p-3">
        <div class="flex-grow flex mb-3 xt-base-btn">
          <img :src="src" alt="" />
        </div>

        <XtButton type="theme" style="width: 100%" @click="download()">下载</XtButton>
      </div>
      <div
        v-else
  ...
  methods: {
    analysis(path) {
      ...
      image.onload = function () {
        ...
        const imageData = canvas.getImageData(0, 0, canvasWidth, canvasHeight).data;

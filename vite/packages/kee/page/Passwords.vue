<template>
  <div class="password-header" style="-webkit-app-region: drag">
    ...
    <div class="password-search" style="-webkit-app-region: none">
      <a-input v-model:value="searchKey" placeholder="搜索 网址、备注、标题、用户名">
        <template #prefix>
          <search-outlined @click="searchClick" style="color: rgba(0, 0, 0, 0.45); font-size: 20px; cursor: pointer" />
        </template>
      </a-input>
    </div>
  ...
  <a-layout style="height: calc(100vh - 45px)">
    <a-layout-sider theme="light" style="padding: 20px; border-right: 1px solid rgba(230, 230, 230, 1)">
        ...
        <div class="header-container">
          <div class="current-header">
            ...
            <div class="current-switch">
              <a-switch v-model:checked="this.siteCard.isRoot" checked-children="全站" un-checked-children="子站" />
              <UpOutlined
      ...
      <vue-custom-scrollbar :settings="settings" style="position: relative" :style="{ height: this.listHeight }">
          ...
          <template #renderItem="{ item, index }">
            <div @mouseenter.stop="passwordItemsHover(item)" @mouseleave.stop="passwordItemRemove(item)">
              <a-list-item :class="currentIndex === item.id ? 'active-list' : ''" @click="leftDescription(item)">
                <!-- 判断鼠标悬浮时打开并填充按钮显示 -->
                <a-list-item-meta
                  :class="{ 'is-open-fill': item.showTip, 'no-open-fill': !item.showTip }"
                ...
                >
                  ...
                  <template #avatar>
                    <ColorImg
                      ...
                      :width="18"
                      :height="18"
                      :color="this.getColor(item.originData)"
                    ></ColorImg>
                  </template>
                </a-list-item-meta>
  ...
  >
    <div class="password-filter-container">
      <a-list item-layout="horizontal" :data-source="selectMenuList">
        ...
        <a-list-item
          @click="showImport"
          style="font-size: 14px; color: royalblue; cursor: pointer"
          class="other-password-drawer"
        >
  ...
  <a-modal v-model:visible="importVisible" title="导入密码" width="80%" centered @ok="doImport">
    ...
    <div style="padding: 10px">
      ...
      <template v-else-if="importStep === 1">
        <p>
          当前有 <strong style="font-size: 24px">{{ innerPasswords.length }}</strong> 条密码可供导入。点击下一步导入。
        </p>
        <p>
      ...
      <template v-else-if="importStep === 2">
        ...
        >
          <template #extra>
            ...
            <template v-if="existAction === 'jump'"> 跳过：{{ importResult.jumpCount }} <br /> </template>
            <template v-else> 更新：{{ importResult.pushCount }} <br /> </template>
          </template>
        </a-result>
      </template>
    </div>
    <template #footer>
      ...
      <a-button key="submit" type="primary" :loading="loading" @click="doImportNext()">
        <template v-if="importStep !== 2"> 下一步 </template>
...
export default {
    ...
    if (params.type === 'url') {
      ...
      if (this.$route.params.uuid && this.$route.params.target === 'remark' && !window.redirected) {
        ...
        this.leftDescription(
          this.displayPasswords.find((pwd) => {
  ...
  methods: {
    ...
    doImportPasswords() {
      this.importResult = this.importPasswords(
        this.innerPasswords,
        this.importGroup ? this.importGroupName : undefined,
        this.existAction,
        this.importDelete ? 'delete' : 'none',
      );
      return true;
    },
    // 搜索触发做的事情
    ...
    // 鼠标移入开始
    passwordHover() {
      document.querySelector('.password-select').style = 'background:rgba(80, 139, 254, 0.1);';
    },
    ...
    // 鼠标移出开始
    passwordRemove() {
      document.querySelector('.password-select').style = 'background:rgba(255, 255, 255, 1);';
    },

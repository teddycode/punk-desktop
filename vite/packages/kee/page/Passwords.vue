<template>
  <div class="password-header" style="-webkit-app-region:drag">
    ...
    >
      <a-row class="password-select-container" style="-webkit-app-region:none">
    ...
    <div class="password-button" style="-webkit-app-region:none">
      ...
      <span style=" color: rgba(255, 255, 255, 1); padding-left:8px;" @click="createPwd">新建密码</span>
    </div>
  </div>
  <a-divider style="height: 1px; background-color: rgba(230, 230, 230, 0.1);margin: 0  !important;"/>
  <a-layout style="height: calc(100vh - 45px);">
    <a-layout-sider style="padding: 20px;border-right: 1px solid rgba(230, 230, 230, 1);" theme="light">
      <div v-if="filterInfo.type==='tab'" class="current-container">
        <div class="header-container">
          <div class="current-header">
            ...
            <div class="header-content">
              ...
              <span v-else style="color: grey">当前页面</span>
            </div>
          ...
          <!-- 通过最右侧图标控制内容显示和隐藏 -->
          <div v-if="contentControlShow == true" class="current-content">
            <span>{{ currentTab.title }}</span>
            <span class="current-website">{{ currentTab.url }}</span>
          </div>
        </div>
      </div>
      <vue-custom-scrollbar :settings="settings" :style="{height:this.listHeight}"
                            style="position:relative;"
      >
        <a-list :data-source="state.displayPasswords" item-layout="horizontal">
          <template #renderItem="{ item,index }">
            <div @mouseenter.stop="passwordItemsHover(item)" @mouseleave.stop="passwordItemRemove(item)">
              <a-list-item :class="currentIndex===item.id ? 'active-list':''" @click="leftDescription(item)">
                <!-- 判断鼠标悬浮时打开并填充按钮显示 -->
                <a-list-item-meta :class="{'is-open-fill':item.showTip,'no-open-fill':!item.showTip}"
                                  :description="item.username || '无用户名'">
                  <template #title>
                    <a>{{ item.title || '未命名' }}</a>
                  </template>
                  <template #avatar>
                    <ColorImg :color="this.getColor(item.originData)" :height="18" :src="item.icon"
                              :width="18"></ColorImg>
                  </template>
                </a-list-item-meta>
                <a-dropdown :trigger="['hover']">
                  <div v-show="item.showTip" class="open-fill" @click.stop="openFillClick(item)">打开并填充</div>
                  <template #overlay>
                    <a-menu>
                      <a-menu-item key="0">
  ...
  <a-drawer v-model:visible="sideDrawerVisible" :width="216"
            class="filter-list-container" placement="left"
            @close="sideDrawerVisible = false"
  >
    <div class="password-filter-container">
      <a-list :data-source="selectMenuList" item-layout="horizontal">
        <template #renderItem="{ item }">
          <a-list-item :class="selectDrawerIndex == item.id ? 'active-drawer':''" class="drawer-item-list">
            <div v-if="item.id == 1001" class="my-password-drawer">
              <a-dropdown :trigger="['click']" overlayClassName="select-pwd" placement="bottomLeft" width="200">
                <span class="ant-dropdown-link" @click.prevent>
                  <component :is="item.icon"
                             style="font-size:16px;padding-right: 12px;color: rgba(0, 0, 0, 0.5) !important;"/>
                ...
                <template #overlay>
                  <a-menu class="my-password-drawer-dropdown">
                    <a-menu-item v-for="item in myDbList" :key="item.id">
            ...
            <div v-else class="other-password-drawer">
              <component :is="item.icon"
                         style="font-size:16px;padding-right: 12px;color: rgba(0, 0, 0, 0.5) !important;"/>
          ...
          <a-divider v-if="item.divider==1"
                     style="height: 1px; background-color: rgba(230, 230, 230, 1);margin: 8px 0 !important;"/>
        </template>
        <a-list-item class="drawer-main-application-open" @click="openMainApplication">
          <span class="drawer-main-app-icon">
            <img alt="" src="../../../public/img/lock-app.svg">
          </span>
          <span class="drawer-open-main-text">主应用中打开</span>
        </a-list-item>
        <a-list-item class="other-password-drawer" style="font-size:14px; color:royalblue;cursor:pointer;"
                     @click="showImport">
          <ApiFilled style="margin-right: 10px;margin-left: -4px"/>
          导入内置密码库密码
        </a-list-item>
      </a-list>
    </div>
  ...
  >
    <a-steps :current="importStep">
      ...
      <a-step description="" title="导入密码"/>
      <a-step description="" title="导入完成"/>
    </a-steps>
    <div style="padding: 10px">
      ...
      <template v-else-if="importStep===1">
        <p>当前有 <strong style="font-size: 24px">{{ innerPasswords.length }}</strong> 条密码可供导入。点击下一步导入。
        </p>
        <p>
    ...
    <template #footer>
      ...
      <a-button key="submit" :loading="loading" type="primary" @click="doImportNext()">

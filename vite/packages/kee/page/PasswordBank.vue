  ...
  <div @click="unselectBankItem" class="password-bank-container">
    ...
    <div class="collapse-addon" style="text-align: left; width: 340px; margin-top: -10px">
      <a-collapse ghost>
        <a-collapse-panel class="addon" style="color: #999" key="1" header="更多选项">
          附加凭证（如果有）
          <a @click="selectKey" type="primary" size="small">
            <key-outlined />
            选择秘钥文件</a
          >
          <div v-if="keyPath" style="position: relative">
            <a-tag title="{{keyPath[0]}}">
              ...
              <a
                title="去除秘钥"
                @click="removeKey"
                style="color: red; position: absolute; right: 0px; top: 0"
                class="remove-icon"
                size="small"
              >
                <close-outlined />
    ...
    <div class="password-bank-list-container">
      <vue-custom-scrollbar :settings="settings" style="position: relative; height: 185px">
        ...
        >
          <div
            class="password-bank-list-item"
            :class="bankIndex == 'inner' ? 'bank_active' : ''"
            key="inner"
            @click.stop="setInnerDb"
          >
        ...
        <a-tooltip
          :overlay-style="{ 'min-width': '500px' }"
          :mouseEnterDelay="0.5"
          placement="bottom"
          v-for="(item, index) in dbList"
        >
          <template #title> 保存位置：<br />{{ item.path }} </template>

          <div
            class="password-bank-list-item"
            :class="bankIndex == item.id ? 'bank_active' : ''"
            :key="item.id"
            @click.stop="selectBankItem(item)"
          >
            <img src="/img/lock.svg" alt="" />
            <span class="name">{{ item.text }}</span>
            <span title="删除记录" @click="removeBank(index)" class="remove-icon" size="small">
              <close-outlined
            /></span>
          </div>
        </a-tooltip>
...
export default {
  ...
  methods: {
    ...
    selectKey() {
      ...
      if (keyPath) this.keyPath = keyPath;
    },
    removeKey() {
      this.keyPath = '';
    },
...
.password-bank-container {
  ...
  .password-bank-unlock {
    ...
    height: 64px;
    line-height: 64px;
    background: linear-gradient(60deg, rgba(45, 74, 253, 0.59) 0%, rgba(20, 99, 250, 1) 100%);
    border-radius: 8px;
  }

  ...
  getters: {
    displayPasswords() {
      ...
      this._passwords = this._passwords.filter((pwd) => {
        if (pwd.originData && pwd.originData.parentGroup.uuid.id === this.currentDb.kdbx.meta.recycleBinUuid.id)
          return false; //排除已被删除的密码
        let result =
          testWords(pwd.title, this.searchKey) ||
  ...
  actions: {
    ...
    changeEntry(uuid, data, cb) {
      passwordModel
        ...
        .saveCredential(data.domain, data.username, data.password, data.title, uuid, data.notes);
      if (cb) {
        cb(data);
      }
    },
    ...
    createEntry(cb) {
      ...
      let entry = passwordModel
        .getActivePasswordManager()
        .createCredential(password.domain, password.username, password.password, password.name);

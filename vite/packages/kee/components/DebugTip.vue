<template>
  <a-tooltip :title="getTip.title">
    <div class="tip-hover" style="">
      <CodeTwoTone />
      {{ getTip.message }}
    </div>
  </a-tooltip>
...
const tipMap = {
  debug_url: {
    title: '调试入口',
    message: '仅开启调试模式可见，如果需要生效，还需同时启用调试入口开关。',
  },
  default_opt: {
    title: '初始配置',
    message: '初始体验优化，仅安装时生效，后续用户也可以通过还原重置。',
  },
  use_debug_url: {
    title: '启用调试入口',
  ...
  package: {
    title: '包名',
    message:
      '包名是应用唯一可读身份，一般由三段组成。 【前缀】.【公司名】.【应用名】，只有注册了包名的应用，才能够上架市场。包名不注册是不可以直接上架的。在上架的时候会要求唯一包名。',
  },
  logo: {
    title: 'logo',

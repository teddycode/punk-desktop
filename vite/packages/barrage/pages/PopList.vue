...
<template>
      ...
      >
        <a-row>
          <a-col flex="60px"
            ><a-avatar
              class="link"
              @click="openSpace(currentBarrageData.uid)"
              size="large"
              :src="currentBarrageData.avatar"
            ></a-avatar>
          </a-col>
          <a-col flex="1">
            <p style="font-weight: bold">
              <a class="link" @click="openSpace(currentBarrageData.uid)">{{ currentBarrageData.nickname }}</a>
            </p>
        ...
        <template #footer="">
          <div style="text-align: center">
            <a-button
              :loading="supporting"
              :danger="currentBarrageData.is_like"
              @click="doSupport(this.currentBarrageData.nanoid)"
              class="btn-margin"
            >
              <span class="link" style="">
                <transition :name="currentBarrageData.is_like ? 'zoom' : ''" mode="out-in">
                  ...
                  <like-filled
                    style="color: red"
                    id="surrpotIco"
                    v-if="currentBarrageData.is_like"
                    key="like"
                  ></like-filled>
            ...
            <span
              class="btn-margin"
              v-if="
                this.currentBarrageData.uid === this.user.user_info.uid || [4].indexOf(this.user.user_info.uid) > -1
              "
            >
              <a-button type="danger" @click="doDelete(this.currentBarrageData.nanoid)">
                <span v-if="this.currentBarrageData.uid === this.user.user_info.uid"> <delete-outlined /> 撤回 </span>
                <span v-else> <delete-outlined /> 删除 </span>
              </a-button>
            </span>
          </div>
        </template>
  ...
  <div class="window-frame">
    <div id="danmuWrapper" class="barrage-container" style="height: calc(100vh - 30px); margin-top: 10px"></div>
    <div
      id="controller"
      class="operation"
      ...
      <a-popover v-model:visible="inputPopVisible" trigger="click">
        <template #content>
          <div style="width: 350px; -webkit-app-region: no-drag" :style="{ height: this.user ? '100px' : '130px' }">
            <div>
              <img style="width: 22px; vertical-align: top" src="../assets/hot.svg" /> 发弹幕 &nbsp;
              <a-switch
                v-if="false"
                v-model:checked="channelType"
                size="small"
                checked-children="团队频道"
                un-checked-children="公共频道"
              ></a-switch>
            ...
            <div style="margin-top: 10px; margin-bottom: 15px">
              <a-input
                ...
                spellcheck="false"
                @visibleChange="toggleInput"
                id="inputArea"
                class="scroller-wrapper"
                style="resize: none; overflow: hidden !important"
                :allowClear="true"
                :maxlength="100"
                :bordered="false"
                placeholder="发一条弹幕吧~"

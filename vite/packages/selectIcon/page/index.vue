  ...
  <div
    ...
    <div class="top-icon flex">
      <div class="type-select flex">
        <span :class="selIndex == 1 ? 'active':''" @click="onSelChange(1)">Emojis</span>
        <span :class="selIndex == 2 ? 'active':''" @click="onSelChange(2)">Icons</span>
        <span v-if="this.isCustom" :class="selIndex == 3 ? 'active':''" @click="onSelChange(3)">自定义</span>
      </div>
      <div class="flex items-center pointer" @click="clearAvatar">移除</div>
    </div>
    <div v-if="selIndex != 3" class="action-search pl-3 pr-3 pt-3 flex" style="justify-content: space-between;">
      <a-input
      ...
      <a-tooltip>
        ...
        <xt-button :h="40" :w="40" class="flex justify-center items-center ml-3"
                   style="background: #2A2A2A;border-radius: 10px;" @click="onRandom">
          <Icon :icon="icons.arrowSync20Filled" class="pointer flex items-center" height="20" width="20"/>
        </xt-button>
      </a-tooltip>
      <!-- 颜色选择 -->
      <div v-show="selIndex == 2" class="flex justify-center items-center pointer ml-3"
           style="width: 40px;height:40px;background: #2A2A2A;border-radius: 10px;position: relative;"
           @click="isShowBgColor=!isShowBgColor">

        <a-tooltip>
          <template #title>颜色选择</template>
          <!-- <div class="circle" :style="{background:bgColor[selBgColor]}" ></div> -->
          <div :style="{background:bgColor[selBgColor]}" class="circle"></div>
        </a-tooltip>

        <div v-show="isShowBgColor" class="select-color" style="flex-wrap: wrap;z-index: 100;">
          <div class="ml-4 mt-2">自定义</div>
          <div :class="selBgColor == 6 ? 'sel-active':''" class="flex justify-center items-center mt-1 ml-3"
    ...
    <!-- 搜索结果 -->
    <div v-if="searchValue" class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap"
         style="height: 290px;">
      <div v-for="(item,index) in searchIcon" :key="index" :class="selectIcon == index ? 'sel-active':''"
           class="flex justify-center items-center mt-2 ml-2 pointer"
           style="width: 40px;height:40px;border-radius: 10px;" @click="onSelectIcon(index,'emoji',item.name)">
        <a-tooltip>
    ...
    <!-- emojis -->
    <div v-else-if="this.selIndex == 1" class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap"
         style="height: 290px;">
      <div v-for="(item,index) in emojisList" :key="index" :class="selectIcon == index ? 'sel-active':''"
           class="flex justify-center items-center mt-2 ml-2 pointer"
           style="width: 40px;height:40px;border-radius: 10px;" @click="onSelectIcon(index,'emoji',item.name)">
        <a-tooltip>
          <template #title>{{ item.alias }}</template>
          <a-avatar :alt="item.alias" :src="'https://a.apps.vip/icons/iconSelect/emoji/'+item.name+'.svg'" height="32"
                    width="32"></a-avatar>
        </a-tooltip>
      </div>
    </div>
    <!-- icon -->
    <div v-else-if="this.selIndex == 2" class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap"
         style="height: 290px;">
      <div v-for="(item,index) in iconList" :key="index" :class="selectIcon == index ? 'sel-active':''"
           class="flex justify-center items-center mt-2 ml-2 pointer overflow-hidden"
           style="width: 40px;height:40px;border-radius: 10px;" @click="onSelectIcon(index,'icon',item.name)">
        <a-tooltip>
          <template #title>{{ item.alias }}</template>
          <div>
    ...
    <div v-else class="flex pl-1 pr-1 items-center flex-wrap flex-col" style="height:330px;">
      ...
      <a-avatar v-else :size="64" :src="avatarUrl" shape="square" style="margin-top: 98px;"></a-avatar>

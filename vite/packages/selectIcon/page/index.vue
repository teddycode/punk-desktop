<template>
  ...
  >
    ...
    <div class="action-search pl-3 pr-3 pt-3 flex" style="justify-content: space-between" v-if="selIndex != 3">
      ...
      <div
        v-show="selIndex == 2"
        class="flex justify-center items-center pointer ml-3"
        style="width: 40px; height: 40px; background: var(--secondary-bg); border-radius: 10px; position: relative"
      ...
      >
        ...
        <div
          ...
          style="flex-wrap: wrap; z-index: 100; color: var(--primary-text)"
        >
          <div class="ml-4 mt-2">自定义</div>
          <div
            style="width: 40px; height: 40px; border-radius: 10px"
            class="flex justify-center items-center mt-1 ml-3"
            :class="selBgColor == 6 ? 'sel-active' : ''"
          >
          ...
          <div class="flex flex-wrap">
            <div
              v-for="(item, index) in bgColor.slice(0, 6)"
              class="flex justify-center items-center pointer mt-2 ml-3"
              style="width: 40px; height: 40px; border-radius: 10px"
              :class="selBgColor == index ? 'sel-active' : ''"
    ...
    <div
      v-if="searchValue"
      class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap"
      style="height: 290px"
    >
      <div
        v-for="(item, index) in searchIcon"
        :key="index"
        @click="onSelectIcon(index, 'emoji', item.name)"
        class="flex justify-center items-center mt-2 ml-2 pointer"
        style="width: 40px; height: 40px; border-radius: 10px"
        :class="selectIcon == index ? 'sel-active' : ''"
      >
        <a-tooltip v-if="goodVisible">
          <template #title>{{ item.alias }}</template>
          <a-avatar
            v-show="selIndex == 1"
            :src="'https://a.apps.vip/icons/iconSelect/emoji/' + item.name + '.svg'"
            :alt="item.alias"
            width="32"
            height="32"
          ></a-avatar>
        ...
        <a-tooltip v-else>
          ...
          <a-avatar
            v-show="selIndex == 1"
            :src="'https://a.apps.vip/icons/goodSelect/' + item.name + '.svg'"
            :alt="item.alias"
            width="32"
            height="32"
          ></a-avatar>
    ...
    <!-- emojis -->
    <div
      v-else-if="this.selIndex == 1"
      class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap"
      style="height: 290px"
    >
      <div
        v-for="(item, index) in emojisList"
        :key="index"
        @click="onSelectIcon(index, 'emoji', item.name)"
        class="flex justify-center items-center mt-2 ml-2 pointer"
        style="width: 40px; height: 40px; border-radius: 10px"
    ...
    >
      <div
        ...
        :key="index"
        @click="onSelectIcon(index, 'icon', item.name)"
    ...
    <div v-else class="flex pl-1 pr-1 items-center flex-wrap flex-col" style="height: 330px">
      ...
      <div
        v-if="!avatarUrl"
        class="pointer flex justify-center items-center"
  ...
  methods: {
    ...
    onSelectIcon(n, type, url) {
      ...
      if (this.selIndex == 1) {
        ...
        this.$emit(
          'getAvatar',
          'https://a.apps.vip/icons/iconSelect/icon/' + url + '.svg?color=' + this.bgColor[this.selBgColor],
        );
      }
    ...
    onRandom() {
      ...
      if (this.selIndex == 1) {
        if (this.goodVisible) {
          this.$emit(
            'getAvatar',
            'https://a.apps.vip/icons/iconSelect/emoji/' + this.emojisList[randomNum].name + '.svg',
          );
        } else {
        ...
        this.$emit(
          'getAvatar',
          'https://a.apps.vip/icons/iconSelect/icon/' +
            this.iconList[randomNum].name +
            '.svg?color=' +
            this.bgColor[this.selBgColor],
        );
      }
    },
    ...
    changeBgColor(n) {
      ...
      if (this.selIndex == 2) {
        this.$emit(
          'getAvatar',
          'https://a.apps.vip/icons/iconSelect/icon/' +
            this.iconList[this.selectIcon].name +
            '.svg?color=' +
            this.bgColor[this.selBgColor],
        );
      }
    ...
    clearAvatar() {
      ...
      this.$emit(
        'getAvatar',
        this.defaultAvatar || 'https://jxxt-1257689580.cos.ap-chengdu.myqcloud.com/jmPD-I__T-SMyc-LMzn',
      );
    },
    // 确认
    changeAvatar() {

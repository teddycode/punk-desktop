  ...
  <ul class="space-list">
    <a-dropdown v-for="(space, index) in spaces" :trigger="['contextmenu']">
      <li
        ...
        :class="{ active: this.activeSpace.nanoid === space.nanoid }"
        @click="setActive(space)"
      >
        <div class="card">
          <a-row>
            <a-col :span="4">
              <a-avatar :size="24" shape="square" :src="getSpaceIcon(space)"> </a-avatar>
            </a-col>
            <a-col :span="20">
          ...
          <div style="margin-bottom: 5px" v-if="this.currentSpace.spaceId === space.nanoid">
            <a-tag color="green"> 当前设备使用中 </a-tag>
          </div>
          <div v-if="space.client_id">
            <div v-if="space.client_id !== this.user.clientId">
              <a-tag color="red" v-if="!this.offLine(space.sync_time)"> 其他设备使用中 </a-tag>
              <a-tag v-else> 其他设备离线使用中 </a-tag>
            </div>
            <div>
  ...
  >
    ...
    <p>
      <a-input
        ...
        @keyup.enter="doRenameSpace"
        v-model:value="spaceRename"
        placeholder="新空间名"
      ></a-input>
    </p>
    <p></p>
  ...
  methods: {
    ...
    deleteSpace(space) {
      ...
      Modal.confirm({
        title: '删除空间确认',
        content:
          '是否确认删除 【' + space.name + '】 ？空间一旦被删除将无法恢复，所有标签组的设置将丢失。请务必确认后果。',
        centered: true,
        okText: '我已知晓后果，仍要删除',
        cancelText: '取消',
        onOk: async () => {

<template>
  <div id="titleBar" @contextmenu="showMenu" style="display: flex; justify-content: flex-start; flex-direction: row">
    <div
    ...
    <div id="title" style="flex: auto; line-height: 30px; text-align: center">
      <img
        ...
        id="appLogo"
      /><span
        :title="'网址：' + encodeURI(this.app.url) + '（双击复制）'"
        style="color: white; margin-left: 10px; font-size: 14px"
        id="appName"
        @dblclick="copyAddress"
        >{{ app.name }}</span
      >
    </div>
    ...
    <div id="windows-control" v-if="platform === 'win32'" style="width: 140px; display: flex">
      <div class="windows-caption-buttons">
        ...
        <div
          class="element caption-maximize"
          @click="maximize"
          v-show="!(this.windowIsMaximized || this.windowIsFullscreen)"
        >
          <svg>
  ...
  <div
    style="background-color: white; position: fixed; top: 30px; left: 0; right: 0; bottom: 0; -webkit-app-region: none"
  >
    <div
      hidden
      style="
...
export default {
  ...
  mounted() {
    ...
    ipc.on('init', (event, args) => {
      ...
      document.body.style.backgroundColor = args.app.user_theme_color
        ? args.app.user_theme_color
        : args.app.theme_color;

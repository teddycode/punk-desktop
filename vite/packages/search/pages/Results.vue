<template>
  ...
  <div v-if="openFirst" class="recent-open flex flex-direction justify-between"></div>
  <div v-else class="content flex justify-center list-hook">
    <div v-if="contentLoading" key="loading">
      <a-spin>
        <a-icon slot="indicator" spin style="font-size: 40px" type="loading"></a-icon>
      </a-spin>
    </div>
    <ul v-else>
      <li
        v-for="(item, index) in compSearchResult"
        :key="index"
        :class="{ 'li-ready': itemReadyedIndex === index }"
        class="flex justify-start align-center"
        @click="click(item)"
      >
        <img :src="item.icon" onerror="this.src='../../icons/default.svg'" style="width: 30px; height: 30px" />
        <div class="flex flex-direction justify-around align-start" style="padding-left: 20px; height: 100%">
          <div
            class="text-black-lg sg-omit-sm"
            v-html="item.title.replaceAll(searchWord, `<strong style='color:orangered'>${searchWord}</strong>`)"
          ></div>
          <div
            class="text-grey-sm sg-omit-sm"
            v-html="item.attached.replaceAll(searchWord, `<strong style='color:orangered'>${searchWord}</strong>`)"
          ></div>
        </div>
      </li>
      <div

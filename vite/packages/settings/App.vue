...
export default defineComponent({
  ...
  mounted() {
    ...
    settings.get('autoRun', (value) => {
      if (value !== undefined) this.autoStart = value;
    });
    settings.get('closeExit', (value) => {
      if (value !== undefined) this.closeExit = value;
    });
    settings.get('askCloseExit', (value) => {
      if (value !== undefined) this.askCloseExit = value;
    });
        ...
        >
          ...
          <a-tab-pane :forceRender="true" key="Feature">
            ...
            <div class="settings-container" id="additional-settings-container">
              ...
              <div class="setting-section">
                <a-checkbox @change="changeAutoStart" v-model:checked="autoStart"
                  >开机自启动（浏览器启动加速）</a-checkbox
                >
                <p class="tip">
                ...
                <div style="margin-top: 5px">
                  <a-checkbox
                    :disabled="this.platform !== 'win32'"
                    @change="changeAskCloseExit"
                    v-model:checked="askCloseExit"
                    >每次关闭主窗体的时候都询问</a-checkbox
                  >
                </div>
                <a-radio-group
            ...
            <div class="settings-container" id="browser-gesture">
              ...
              <div class="setting-section">
                ...
                <p>
                  ...
                  <span class="gesture-tip"
                    >按住Ctrl（Mac是Command），同时双指捏合或者双指拉大，可调节网页缩放比例</span
                  >
                </p>
              </div>
            </div>
          ...
          <a-tab-pane :forceRender="true" key="Default">
            ...
            <div class="settings-container" id="browser-settings-container">
              ...
              <div class="setting-section">
                ...
                <button
                  style="border-radius: 4px; border: solid 1px #ccc; padding: 5px"
                  id="button-default-browser"
                  @click="setDefault"
                ></button>
              </div>
            </div>
            ...
            <div class="settings-container" id="browser-tab-container">
              ...
              <div class="setting-section" id="tab-choose">
                <div class="setting-option" style="display: flex; align-items: center">
                  ...
                  <label for="tstab-input">想天桌面（官方尝鲜版）</label>
          ...
          <a-tab-pane :forceRender="true" key="Password">
            ...
            <div class="settings-container" id="password-autofill-container">
              ...
              <div class="setting-section">
                当前密码管理器：
                <a-avatar v-if="currentPasswordManager.icon" shape="square" :src="currentPasswordManager.icon" />
                <a-avatar v-else style="background-color: #1890ff">内</a-avatar> {{ currentPasswordManager.alias }}

                <div
                  hidden
                  v-if="currentPasswordManager.name !== 'file'"
                  style="
                ...
                >
                  <div>
                    <a-avatar src="/kee/kee.svg" :size="30"></a-avatar>
                ...
                >
                  <a-input-group style="float: right" compact>
                    <a-input
                      placeholder="请选择密码库"
                      disabled
                      :value="currentPasswordDb.filePath"
                      style="width: 400px"
                    >
                      <template #addonBefore> 密码库位置 </template>
                    </a-input>
                    ...
                    <a-list :grid="{ gutter: 20, column: 2 }" :data-source="passwordManagers">
                      <template #renderItem="{ item }">
                        <a-list-item>
                          <a-card
                            :style="{
                              'border-color': currentPasswordManager.name === item.name ? '#3a8ee6' : '#f1f1f1',
                              background: currentPasswordManager.name === item.name ? 'rgba(58,142,230,0.03)' : 'white',
                            }"
                            hoverable
                            size="small"
                          >
                            <template #actions>
                              ...
                              <a
                                v-else-if="currentPasswordManager.name === item.name"
                                style="color: #1890ff"
                                @click="setPasswordManager(item)"
                                ><check-circle-outlined /> 当前使用中</a
                              >
                              <a v-else="currentPasswordManager.name === item.name" @click="setPasswordManager(item)"
                                ><check-circle-outlined /> 设置</a
                              >
              ...
              <h3>
                内置密码库内的密码（任何使用此浏览器的用户均可切换到内置密码库，重要密码建议使用独立密码库）
                <a-button @click="importPwd" style="float: right" size="small" type="primary"
                  ><import-outlined /> 导入密码</a-button
                >
              </h3>
              <Passwords />
            </div>

    ...
    <div class="box-content">
      <div v-if="currentSpace.name">
        ...
        <div style="font-size: 12px; color: grey">
          <span v-if="currentSpace.spaceType === 'cloud'"> 云空间 </span>
          <span v-else>类型：本机空间</span>
          <div style="font-size: 12px; color: grey">
            {{ currentSpace.space.count_task }} 标签组 {{ currentSpace.space.count_tab }}标签
          </div>
          <div>最后同步：<span class="time" v-html="friendlyDate(currentSpace.space.sync_time)"></span></div>
        </div>
      </div>
      <span v-else>
        <a-empty
          style="margin: 0; color: rgba(0, 0, 0, 0.25); margin-bottom: 0; padding-top: 17px"
          :description="null"
          :image="simpleImage"
        ></a-empty>
      </span>
...
<script lang="ts">
import { CommentOutlined, CloudSyncOutlined, SwapOutlined } from '@ant-design/icons-vue';

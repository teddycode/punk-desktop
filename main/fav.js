...
async function initFav() {
  ...
  if (!fs.existsSync(defaultStorePath)) {
    ...
    function initLib(defaultStorePath) {
      ...
      const commonFolder = ['书签', '图片', '视频', '音频', '密码', '笔记'];
      const jobFolder = {
        develop: [],
        pm: [],
        designer: [],
  ...
  ipc.on('getFavContent', async (event, args) => {
    ...
    if (content.type === 'img') {
      ...
      localCacheManager
        .fetchContentWithType(content.src, fullPath)
        .then((header) => {
          let ext = header.substr(header.lastIndexOf('/') + 1);
          if (ext === 'svg+xml') {
            ext = 'svg';
          ...
          if (content.alt !== '') {
            ...
            newFileName =
              content.title.length > 20 ? content.title.substr(0, 20) + '.' + ext : content.title + '.' + ext;
          } else {
          ...
          while (fs.existsSync(path.join(defaultStorePath, testFileName))) {
            ...
            testFileName =
              ...
              '-' +
              i.toString() +
              newFileName.substr(newFileName.lastIndexOf('.'));
          }
  ...
  async function showAddPage() {
    ...
    popWindow = await popManager.openPop(
      'favSaveToFolder',
      url,
      {},
      { preload: __dirname + '/pages/fav/preload.js' },
      options,
    );

...
function installDevPlugin (plugin) {
  sendMessage({
    type: 'loading',
    config: {
      content: '正在后台安装插件，请勿关闭浏览器。如果长时间无法成功，请挂梯子后重试。',
      key: 'install',
      duration: 5
    }
  })
  installExtension(plugin).then((ext) => {
    sendMessage({
      type: 'success',
      config: {
        content: '安装插件成功，插件生效需如此操作：①关闭原调试工具，②按F5刷新调试页面，③按F12再次打开调试工具。',
        key: 'install',
        duration: 10
      }
    })
    ...
    .catch((err) => {
      sendMessage({
        type: 'error',
        config: {
          content: '安装插件失败，失败原因：' + err + '，请检查网络后再试，必要情况下请挂梯子。',
          key: 'install'
        }
      })
    }).finally(() => {

...
function makeId(length, possible = 'abcdefghijklmnopqrstuvwxyz') {
  let id = '';
...
async function doInstallCrx(manifestPath, crxInfo) {
  ...
  const manifest = JSON.parse(jsonStrip(await promises.readFile(manifestPath, 'utf8')));
  await promises.writeFile(manifestPath, JSON.stringify(manifest, null, 2)); //重新写回没有注释的版本到文件中，在复制过去，以便于后续读取不出错
  ...
  ipc.on('openExtShop', async (e, a) => {
    if (a.name === 'chrome') {
      ...
      const files = dialog.showOpenDialogSync({
        userScriptWindow,
        filters: [{ name: '扩展插件', extensions: ['crx'] }],
        properties: ['openFile'],
      });
      if (!!!files) {
        return;
...
const extensionManager = {
  ...
  async remove(baseName) {
    ...
    require('fs-extra')
      .remove(path.join(this.extensionsPath, baseName))
      .then(() => {
        renderPage.sendIPC('extensionList', 'reload');
        sendMessage({
          type: 'success',

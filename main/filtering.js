...
function initFilterList() {
  ...
  fs.readFile(
    path.join(__dirname, 'ext/filterLists/easylist+easyprivacy-noelementhiding.txt'),
    'utf8',
    function (err, data) {
      if (err) {
        return;
  ...
  fs.readFile(path.join(app.getPath('userData'), 'customFilters.txt'), 'utf8', function (err, data) {
    if (!err && data) {
      parser.parse(data, parsedFilterData);
    }
  });

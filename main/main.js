...
function createWindowWithBounds(bounds) {
  ...
  mainWindow = new BrowserWindow({
    ...
    x: bounds.x,
    y: bounds.y,
    minWidth: process.platform === 'win32' ? 600 : 320, // controls take up more horizontal space on Windows
    minHeight: 350,
  ...
  async function askCloseExit() {
    ...
    if (askCloseExit === true) {
      let choose = await dialog.showMessageBox(mainWindow, {
        ...
        title: '退出选择',
        detail:
          '完全退出之后，将无法累计在线时长、无法接收消息，还将增加打开浏览器的耗时。选『节能后台运行』，则保留到托盘菜单，且以最低内存运行。建议后台运行。',
        checkboxLabel: '不再询问',
        buttons: ['完全退出（同时关闭工作台）', '节能后台运行（保留工作台）'],
      });
  ...
  function checkClipboard() {
    ...
    if (
      latestClipboardContent !== clipboardContent &&
      (latestClipboardContent.startsWith('http://') || latestClipboardContent.startsWith('https://'))
    ) {

    ...
    if (!knownProtocols.includes(url.split(':')[0])) {
      ...
      if (externalApp) {
        ...
        if (view.webContents.getURL()) {
          view.webContents
            .executeJavaScript('confirm("' + l('openExternalApp').replace('%s', sanitizedName) + '")')
...
ipc.on('setView', function (e, args) {
  ...
  if (windowManager) {
    windowManager.syncAttachedBounds();
  }
  if (args.focus) {
...
ipc.on('enableEmulation', function (e, data) {
  if (
    viewMap[data.id].webContents.getURL().startsWith('file://') ||
    viewMap[data.id].webContents.getURL().startsWith('ts://')
  ) {
    return;
  }
  mobileMod.add({

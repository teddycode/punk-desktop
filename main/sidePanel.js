...
global.SidePanel = class SidePanel {
  ...
  isDestroyed() {
    if (this._sidePanel == null || typeof this._sidePanel == 'undefined') return true;
  ...
  setMouseIgnore() {
    ...
    if (BrowserWindow.getFocusedWindow() != null && sidePanel.get().isFocused()) {
      //如果有任意一个window还有焦点，则聚焦到mainwindow
      //有窗体还有焦点 且 只有左侧栏
  ...
  setMouseEnable() {
    ...
    if (BrowserWindow.getFocusedWindow() != null && mainWindow.isFocused())
      //如果有任意一个window还有焦点，且主窗体有焦点，则取得焦点，则聚焦到sidepanel
      this._sidePanel.focus();
...
ipc.on('addTask', function (event, data) {
  ...
  if (sidePanel != null) sidePanel.addItem(item);
...
ipc.on('captureDeskScreen', (event, args) => {
  ...
  event.sender.capturePage(args.bounds).then((data) => {
    ...
    fs.writeFile(
      path.resolve(userDataPath + '/desk/screen' + args.desk.id + '.jpg'),
      capturedImage.toJPEG(50),
      (err) => {
        if (!err) {
  ...
  ipc.on('toolbar.speedup', () => {
    ...
    switch (ask) {
      case 0:
        ...
        if (closedApp > 0) sendMessage({ type: 'success', config: { content: '已为您关闭' + closedApp + '个应用。' } });
        break;
      case 1:

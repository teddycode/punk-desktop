...
app.whenReady().then(async () => {
    ...
    try {
        ...
        if (await userModel.isLogged()) {
            const schedule = await guideModel.getSchedule()
    ...
    ipc.on('shareTask', async (event, arg) => {
        sidePanel.get().webContents.send('message', {
            type: 'loading',
            config: {content: '正在生成分享链接。', key: 'shareTask'}
        })
        try {
            ...
            sidePanel.get().webContents.send('message', {
                type: 'error',
                config: {content: '分享失败!', key: 'shareTask'}
            })
            dlog.error(err)
        }
    })
    ...
    ipc.on('guideMigration', (event, args) => {
        ...
        ipc.on('afterMigration', async (event, args) => {
            ...
            if (global.fromRender && !global.fromRender.guide.isDestroyed()) {
                const data = await guideModel.getSchedule()
    ...
    async function calcGuideScedule() {
        const guideSchedule = await guideModel.getSchedule()
        const noobGuideObj = JSON.parse(JSON.stringify(guideSchedule.modules.noobGuide))
        const featureObj = JSON.parse(JSON.stringify(guideSchedule.modules.feature))
        const totalChildrenObj = Object.assign(noobGuideObj, featureObj)
        const percentage = Math.floor((Object.entries(totalChildrenObj).filter(v => v[1] === true).length) / (Object.keys(totalChildrenObj).length) * 100)
        return percentage
    }
    ...
    async function afterGuide(module, key) {
        ...
        if (global.fromRender && !global.fromRender.guide.isDestroyed()) {
            ...
            const percent = await calcGuideScedule()

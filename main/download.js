...
function sendIPCToDownloadWindow(action, data) {
  ...
  getDownloadWindow();
  setTimeout(() => {
    if (downloadWindow && !downloadWindow.isDestroyed()) downloadWindow.webContents.send(action, data || {});
  }, 20);
...
function listenForDownloadHeaders(ses) {
  ses.webRequest.onHeadersReceived(function (details, callback) {
    if (details.resourceType === 'mainFrame' && details.responseHeaders) {
      ...
      var typeHeader =
        details.responseHeaders[Object.keys(details.responseHeaders).filter((k) => k.toLowerCase() === 'content-type')];
      var attachment = isAttachment(
        details.responseHeaders[
          Object.keys(details.responseHeaders).filter((k) => k.toLowerCase() === 'content-disposition')
        ],
      );

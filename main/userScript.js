...
app.whenReady().then(() => {
  ...
  ipc.on('importScript', (event, args) => {
    ...
    if (existsCount > 0) {
      ...
      sendMessage({
        type: 'success',
        config: { content: '成功导入' + imported + '个脚本。新导入的脚本需要重启浏览器方可生效。' },
      });
    }
  ...
  ipc.on('viewCode', (event, args) => {
    ...
    const viewCodeWindow = new BrowserWindow({
      frame: true,
      backgroundColor: 'white',
      parent: mainWindow,
      center: true,
  ...
  ipc.on('importPwd', (event, args) => {
    const files = dialog.showOpenDialogSync({
      userScriptWindow,
      filters: [{ name: '密码导出文件', extensions: ['csv'] }],
      properties: ['openFile'],
    });
    if (!files) {
      return;
      ...
      csv()
        ...
        .then((json) => {
          ...
          function convertFromSafari(item) {
            const domain = item.URL.replace('http://', '').replace('https://', '').replace('/', ''); // 清理掉http和https
            return {
              domain: domain,
              username: item.Username,
              password: item.Password,
          ...
          try {
            ...
            json.forEach((item) => {
              let account = {};
              const isSafari = typeof item.OTPAuth !== 'undefined';
              const isFirefox = typeof item.formActionOrigin !== 'undefined';
              if (isSafari) {
                // 是safari
                from = 'Safari';
          ...
          if (importedPwds.length === 0) {
            ...
            event.reply('importPwdSuccess', {
              imported: importedPwds.length,
              importedPwds: importedPwds,
              from: from,
            });
          }
        })

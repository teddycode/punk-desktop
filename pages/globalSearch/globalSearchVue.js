...
const globalSearch = new Vue({
  ...
  methods: {
    ...
    bindKeys() {
      ...
      window.onkeydown = (e) => {
        if (e.keyCode === 40) {
          ...
          e.keyCode === 13 &&
          (Object.keys(this.itemReadyedItem).length > 0 || Object.keys(this.recentReadyedItem).length > 0)
        ) {
          //回车按键
          if (this.openFirst) {
    ...
    async getAllApps() {
      this.apps = await saAppModel.getAllApps({ order: 'last_execute_time', limit: 6 });
    },

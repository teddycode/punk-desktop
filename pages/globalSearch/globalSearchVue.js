...
const globalSearch = new Vue({
  ...
  methods: {
      ...
      window.onkeyup = async (e) => {
        if (e.keyCode === 13) {
          ...
          this.searchResult = this.searchResult.concat(
            ...
            this.searchTask(e.target.value)
          )
          this.itemReadyedIndex = 0
          this.itemReadyedItem = this.searchResult[0]
          this.contentLoading = false
        ...
        if (e.keyCode === 40) {
          ...
          if (this.openFirst) {
            ...
            if (this.recentReadyedIndex + 1 < this.recentOpenedHistory.length) {
              ...
              this.recentReadyedItem =
                this.recentOpenedHistory[this.recentReadyedIndex]
              document.getElementsByClassName('recent-list-hook')[0].scrollTop =
            ...
            if (this.itemReadyedIndex + 1 < this.compSearchResult.length) {
              ...
              this.itemReadyedItem =
                this.compSearchResult[this.itemReadyedIndex]
              document.getElementsByClassName('list-hook')[0].scrollTop =
                this.calculateHeight()
            } else {
              this.itemReadyedIndex = 0
          ...
          if (this.openFirst) {
            //如果是在初始页
            if (
              this.recentReadyedIndex + 1 <= this.recentOpenedHistory.length &&
              this.recentReadyedIndex !== 0
            ) {
              this.recentReadyedIndex = this.recentReadyedIndex - 1
              this.recentReadyedItem =
            ...
            if (
              this.itemReadyedIndex + 1 <= this.compSearchResult.length &&
              this.itemReadyedIndex !== 0
            ) {
              this.itemReadyedIndex = this.itemReadyedIndex - 1
              this.itemReadyedItem =
          ...
          e.keyCode === 13 &&
          (Object.keys(this.itemReadyedItem).length > 0 ||
            Object.keys(this.recentReadyedItem).length > 0)
        ) {
          //回车按键
          if (this.openFirst) {
    ...
    async searchApp (word) {
      let apps = await saAppModel.find(word, { order: 'last_execute_time' })
      return apps.map((v) => {
        return {
          title: v.name,
          icon: v.logo,
    ...
    searchTab (word) {
      ...
      return mapTabs.filter(
        (v) =>
          v.title.toLowerCase().includes(word) ||
          tools.execDomain(v.url).toLowerCase().includes(word) ||
          tools.pinyinMatch(v.title, word)
      )
    },
  },
  async mounted () {

  ...
  methods: {
    ...
    guide(a) {
      const stepsList = [
          ...
          buttons: [
            {
              action: function () {
                window.location.href =
                  'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/zp48yn';
              },
              secondary: true,
              text: '了解更多',
            },
        ...
        {
          text: `<div>在这里创建或选择您想要的空间</div>`,
          attachTo: { element: '.guideSpace', on: 'right' },
          buttons: [
            {
              action: function () {
                window.location.href =
                  'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/gg7vro';
              },
              secondary: true,
              text: '了解更多',
            },
        ...
        {
          text: `<div>点击这里可以创建你的团队</div>`,
          attachTo: { element: '#guideTeam', on: 'right' },
          buttons: [
            {
              action: function () {
                window.location.href =
                  'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/rls0pi';
              },
              secondary: true,
              text: '了解更多',
            },
    ...
    guideApplySecond() {
      const applySecondShepherd = new Shepherd.Tour({
        ...
        steps: [
          {
            text: '点击这里可以管理你安装的所有应用',
            attachTo: { element: '#guideApplySecond', on: 'right' },
            buttons: [
              {
                action: function () {
                  window.location.href =
                    'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/mmcfd7';
                },
                text: '了解更多',
                classes: 'button1',
              },
            ...
            buttons: [
              {
                action: function () {
                  window.location.href =
                    'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/sv8ozw';
                },
                text: '了解更多',
                classes: 'button2',
              },
            ...
            buttons: [
              {
                action: function () {
                  window.location.href =
                    'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/yglkui';
                },
                secondary: true,
                text: '了解更多',
              },
    ...
    async uninstallApp(args) {
      let app = await standAloneAppModel.get({ appid: args });
      standAloneAppModel.uninstall(app.nanoid).then(
        (success) => {
    ...
    openAppCircle(args) {
      window.location.href = `tsb://app/redirect/?package=com.thisky.com&url=${api.getUrl(
        api.API_URL.user.CIRCLE,
      )}?id=${args}`;
    },
    myApps() {
    ...
    confirmChangeSpace(space, type) {
      ...
      if (type !== 'cloud') {
        this.$confirm({
          title: '切换到本地空间',
          content:
            '是否更改当前空间，更改空间将重载浏览器，可能导致您网页上未保存的内容丢失，请确认已经保存全部内容。切换本地空间并不会更改当前登录帐号。',
          centered: true,
          okText: '我已保存，切换空间',
          cancelText: '取消',
          onOk: async () => {
        ...
        if (space.isOtherUsing) {
          if (space.online) {
            this.$confirm({
              title: '此空间正忙',
              content:
                '此空间正在被其他设备使用，如若切换到此空间，可能造成其他设备未同步的标签组丢失。是否仍然要强行切换？这将导致该设备上的浏览器强制下线。',
              centered: true,
              okText: '我已明确，切换空间',
              cancelText: '取消',
              onOk: async () => {
            ...
            this.$confirm({
              ...
              content: appVue.$createElement('div', {}, [
                appVue.$createElement(
                  'p',
                  {},
                  '此空间正在被其他设备使用，但是系统检测到此设备可能已经因为网络或者其他原因而离线。',
                ),
                appVue.$createElement(
                  'p',
                  {},
                  '所以此设备上可能存在未保存的标签组。如果切换到此空间，可能造成未保存的内容丢失。',
                ),
                appVue.$createElement('p', {}, '建议到此设备商重新连接后正常关闭浏览器，以防止数据冲突。'),
                appVue.$createElement(
                  'p',
                  {},
                  '如果您确认已无法恢复此设备的连接，则可切换到此设备在离线前最后一次保存的空间。',
                ),
          ...
          this.$confirm({
            title: '切换到云端空间',
            content:
              '是否切换到云端空间？切换到云端空间后会同时更换当前帐号到此帐号。请务必确认您网页上的内容已经保存。否则可能丢失未保存内容。',
            centered: true,
            okText: '我已保存，切换空间',
            cancelText: '取消',
            onOk: async () => {
    ...
    changeSpace(id, type) {
      ...
      } else ipc.send('changeSpace', { spaceId: id, spaceType: type });
    },
    openUserWindow() {
    ...
    openBottom(action) {
      if (action === 'help') {
        window.location.href =
          'tsb://app/redirect/?package=com.thisky.helper&url=https://www.yuque.com/tswork/browser/tmpomo';
        return;
      }
      if (action === 'setting') {
        postMessage({
    ...
    async logout() {
      if (this.currentSpace.spaceType === 'cloud') {
        this.$confirm({
          title: '退出账号确认',
          content:
            '您当前正在使用云空间，一旦注销，将会退出当前空间，请确认是否要退出当前空间？注意：退出账号不会删除登录凭证，如需删除账号信息，请在切换用户面板删除账号。',
          centered: true,
...
ipc.on('closeApp', function (event, args) {
  appVue.$refs.sidePanel.apps.forEach((app) => {
    if (app.nanoid === args.nanoid) {
      ...
      if (appIndex > -1) appVue.$refs.sidePanel.runningApps.splice(appIndex, 1);
      appVue.$refs.sidePanel.sortApps();
    }
  });
});

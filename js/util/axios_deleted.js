...
function execRefreshToken(config) {
  axios
    .post(`${api.NODE_API_URL.USER.REFRESH_TOKEN}`, {
...
axios.interceptors.request.use(
  async (config) => {
    ...
    if (
      (config.hasOwnProperty('expireInfo') && config.expireInfo && config.expireInfo.token.length > 0) ||
      config.url.includes('refreshBrowserToken')
    ) {

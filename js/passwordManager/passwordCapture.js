...
const passwordCapture = {
  ...
  showCaptureBar: function (username, password) {
    ...
    passwordCapture.description.innerHTML = l('passwordCaptureSavePassword').replace(
      '%s',
      tabIcon + `${passwordCapture.currentDomain}`,
    );
    ...
    if ($toolbar.expanded) {
      if (document.querySelector('#third-toolbar').hidden === true) {
        document.getElementById('password-capture-bar').style.top =
          document.getElementById('toolbar').offsetTop + 40 + 'px';
      } else {
        document.getElementById('password-capture-bar').style.top =
          document.getElementById('toolbar').offsetTop + 80 + 'px';
      }
    } else {

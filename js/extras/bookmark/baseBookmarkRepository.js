...
class baseBookmarkRepository {
  static htmlDeal(data) {
    ipc.send('message', {
      type: 'loading',
      config: { content: 'html书签导入中...', duration: 7, key: 'baseBookmarkImport' },
    });
    ...
    function recurCalc(dt, levelParam = 1, parentParam = null, typeParam = null, nameParam = null, urlParam = null) {
      ...
      let url = urlParam;
      if (
        dt.firstChild.nodeName === 'H3' &&
        dt.firstChild.attributes[2] &&
        dt.firstChild.attributes[2].name === 'personal_toolbar_folder'
      ) {
        //说明是书签栏 根root dt
        return;
      }
      ...
      if (
        dt.childNodes.length === 2 &&
        dt.firstChild.nodeName === 'A' &&
        dt.parentNode.previousElementSibling.innerText === 'Bookmarks'
      ) {
        ...
        if (
          dt.parentNode.previousElementSibling.nodeName === 'H3' &&
          dt.parentNode.previousElementSibling.attributes[2] &&
          dt.parentNode.previousElementSibling.attributes[2].name === 'personal_toolbar_folder'
        ) {
  ...
  static importHtml(bookmark) {
    try {
      ...
      setTimeout(() => {
        ipc.send('message', {
          type: 'success',
          config: { content: 'Html文件生成书签成功', key: 'baseBookmarkImport' },
        });
        ipc.send('reloadFav');
      ...
      ipc.send('message', {
        type: 'error',
        config: { content: `Html文件生成书签失败!${error}!`, key: 'baseBookmarkImport' },
      });
    }
  }

...
function fullTextQuery(tokens) {
  return db.transaction('r', db.places, function* () {
    ...
    const tokenMatches = yield Dexie.Promise.all(
      tokens.map((prefix) => db.places.where('searchIndex').equals(prefix).primaryKeys()),
    );
    ...
    .then(function (queryResults) {
      ...
      for (let i = 0; i < dl; i++) {
        ...
        const mappedArr = snippetIndex.map((w) =>
          searchWords.includes(stemmer(w.toLowerCase().replace(nonLetterRegex, ''))) ? 1 : 0,
        );
        ...
        for (let i2 = 0; i2 < mappedArr.length; i2++) {
          ...
          if (
            ...
            (currentScore > 0 && currentScore === maxScore && indexBegin - maxBegin <= 1)
          ) {
            maxBegin = indexBegin;
            maxEnd = indexEnd;
